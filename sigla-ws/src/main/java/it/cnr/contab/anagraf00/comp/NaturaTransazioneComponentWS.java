...
import it.cnr.contab.docamm00.intrastat.bulk.Natura_transazioneBulk;
 ...
 */
@Stateless
@WebService(endpointInterface="it.cnr.contab.anagraf00.ejb.NaturaTransazioneComponentSessionWS")
...
public class NaturaTransazioneComponentWS  {
	
public  NaturaTransazioneComponentWS() {
}

public java.util.ArrayList<NaturaTransazione> cercaNaturaTransazione(
		  String query,
		  Integer esercizio,
		  String dominio,
		  Integer numMax,
...
try {
	java.util.ArrayList<NaturaTransazione> listaNaturaTransazione=new ArrayList<NaturaTransazione>();
	List lista=null;
	if(user== null)
		user="IIT";
	if(ricerca== null)
	...
	if(query== null){
		...
		try {
			lista=(((FatturaAttivaSingolaComponentSession)it.cnr.jada.util.ejb.EJBCommonServices.createEJB("CNRDOCAMM00_EJB_FatturaAttivaSingolaComponentSession",FatturaAttivaSingolaComponentSession.class)).findListaNaturaTransazioneWS(userContext,query, dominio, ricerca));
		...
		if (lista != null && !lista.isEmpty()){
    		for (Iterator i = lista.iterator();i.hasNext()&&num< new Integer(numMax).intValue();){
    			Natura_transazioneBulk db_nt = (Natura_transazioneBulk)i.next();
    			NaturaTransazione nt=new NaturaTransazione();
    			nt.setEsercizio(db_nt.getEsercizio());
    			nt.setId(db_nt.getId_natura_transazione());
    			nt.setCodice(db_nt.getCd_natura_transazione());
    			nt.setDescrizione(db_nt.getDs_natura_transazione());
    			listaNaturaTransazione.add(nt);
    			num++;
    		} 
		}
		return listaNaturaTransazione;
