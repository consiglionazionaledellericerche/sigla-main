...
public  ModalitaErogazioneComponentWS() {
}
...
try {
	...
	if(query== null){
		...
		try {
			listaModalita=(((FatturaAttivaSingolaComponentSession)it.cnr.jada.util.ejb.EJBCommonServices.createEJB("CNRDOCAMM00_EJB_FatturaAttivaSingolaComponentSession",FatturaAttivaSingolaComponentSession.class)).findListaModalitaErogazioneWS(userContext,query, dominio, ricerca));
		...
		if (listaModalita != null && !listaModalita.isEmpty()){
    		for (Iterator i = listaModalita.iterator();i.hasNext()&&num< new Integer(numMax).intValue();){
    			Modalita_erogazioneBulk modero = (Modalita_erogazioneBulk)i.next();
