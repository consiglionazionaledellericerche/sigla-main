...
import it.cnr.contab.client.docamm.ModalitaTrasporto;
 ...
 */
@Stateless
@WebService(endpointInterface="it.cnr.contab.anagraf00.ejb.ModalitaTrasportoComponentSessionWS")
...
public class ModalitaTrasportoComponentWS  {
	
public  ModalitaTrasportoComponentWS() {
}

public java.util.ArrayList<ModalitaTrasporto> cercaModalitaTrasporto(
		  String query,
		  Integer esercizio,
		  String dominio,
		  Integer numMax,
...
try {
	java.util.ArrayList<ModalitaTrasporto> listaModalitaTrasporto=new ArrayList<ModalitaTrasporto>();
	List lista=null;
	if(user== null)
		user="IIT";
	if(ricerca== null)
	...
	if(query== null){
		...
		try {
			lista=(((FatturaAttivaSingolaComponentSession)it.cnr.jada.util.ejb.EJBCommonServices.createEJB("CNRDOCAMM00_EJB_FatturaAttivaSingolaComponentSession",FatturaAttivaSingolaComponentSession.class)).findListaModalitaTrasportoWS(userContext,query, dominio, ricerca));
		...
		if (lista != null && !lista.isEmpty()){
    		for (Iterator i = lista.iterator();i.hasNext()&&num< new Integer(numMax).intValue();){
    			Modalita_trasportoBulk db_mt = (Modalita_trasportoBulk)i.next();
    			ModalitaTrasporto mt=new ModalitaTrasporto();
    			mt.setEsercizio(db_mt.getEsercizio());
    			mt.setCodice(db_mt.getCd_modalita_trasporto());
    			mt.setDescrizione(db_mt.getDs_modalita_trasporto());
    			listaModalitaTrasporto.add(mt);
    			num++;
    		} 
		}
		return listaModalitaTrasporto;
