...
public  CondizioneConsegnaComponentWS() {
}
...
try {
	...
	if(query== null){
		...
		try {
			lista=(((FatturaAttivaSingolaComponentSession)it.cnr.jada.util.ejb.EJBCommonServices.createEJB("CNRDOCAMM00_EJB_FatturaAttivaSingolaComponentSession",FatturaAttivaSingolaComponentSession.class)).findListaCondizioneConsegnaWS(userContext,query, dominio, ricerca));
		...
		if (lista != null && !lista.isEmpty()){
    		for (Iterator i = lista.iterator();i.hasNext()&&num< new Integer(numMax).intValue();){
    			Condizione_consegnaBulk db_cc = (Condizione_consegnaBulk)i.next();
