...
public class FatturaPassivaTrovatiComponentWS {
	@EJB FatturaPassivaComponentSession fatturaPassivaComponentSession; 
	@EJB CompensoComponentSession compensoComponentSession;
	
	@RolesAllowed({ "WSUserRole", "BrevettiRole" })
	public java.util.ArrayList<FatturaPassiva> ricercaFatturaPassiva(
			String user, Long esercizio, String cds, String uo, Long pg,
			boolean byKey) throws Exception {
		...
		try {
			...
			if (byKey)
				fattura = fatturaPassivaComponentSession
						.ricercaFatturaByKey(userContext, esercizio, cds, uo,
								pg);
			else
				fattura = fatturaPassivaComponentSession
						.ricercaFatturaTrovato(userContext, esercizio, cds, uo,
								pg);
	...
	@RolesAllowed({ "WSUserRole", "BrevettiRole" })
	public java.util.ArrayList<FatturaPassiva> ricercaFatturaPassivaByKey(
			String user, Long esercizio, String cds, String uo, Long pg)
			throws Exception {
	...
	/**
	 * Restituisce le fatture legate al progressivo "trovato" con informazioni
	 * complete
	 * 
	 * @param user
	 * @param trovato
	 ...
	 */
	@RolesAllowed({ "WSUserRole", "BrevettiRole" })
	public java.util.ArrayList<FatturaPassiva> ricercaFatturePassive(
	 ...
	 */
	@RolesAllowed({ "WSUserRole", "BrevettiRole" })
	public java.util.ArrayList<FatturaPassivaBase> ricercaFatturePassiveBase(
			String user, Long trovato) throws Exception {
		return ricercaFatture(user, trovato, Boolean.TRUE);
	}

	private java.util.ArrayList ricercaFatture(String user, Long trovato,
		...
		try {
			java.util.List<it.cnr.contab.docamm00.docs.bulk.Fattura_passivaBulk> fatture = fatturaPassivaComponentSession
					.ricercaFattureTrovato(userContext, trovato);
			for (Iterator<Fattura_passivaBulk> i = fatture.iterator(); i
					.hasNext();) {
	...
	private java.util.ArrayList ricercaCompensi(String user, Long trovato,
			Boolean base) throws Exception {
		UserContext userContext = new WSUserContext(user, null, new Integer(
				java.util.Calendar.getInstance().get(java.util.Calendar.YEAR)),
				null, null, null);
		...
		if (trovato == null)
			throw new SOAPFaultException(faultIdTrovatoNullo());
		try {
			List<CompensoBulk> compensi = compensoComponentSession
					.ricercaCompensiTrovato(userContext, trovato);
	...
	@RolesAllowed({ "WSUserRole", "BrevettiRole" })
	public Compenso ricercaCompenso(String user, Long esercizio, String cds,
			String uo, Long pg) throws Exception {
		return ricercaCompenso(user, esercizio, cds, uo, pg, false);
	}

	public Compenso ricercaCompensoByKey(String user, Long esercizio,
			String cds, String uo, Long pg) throws Exception {
		return ricercaCompenso(user, esercizio, cds, uo, pg, true);
	}

	public java.util.ArrayList<Compenso> ricercaCompensi(String user,
			Long trovato) throws Exception {
	...
	private void caricaFattura(UserContext userContext,
		...
		if (base.booleanValue()) {
			...
			BulkList<Fattura_passiva_rigaBulk> dett = fattura
					.getFattura_passiva_dettColl();
			for (Iterator<Fattura_passiva_rigaBulk> j = dett.iterator(); j
					.hasNext();) {
				Fattura_passiva_rigaBulk det = (Fattura_passiva_rigaBulk) j
						.next();
				...
				ritorno.setDs_fattura_passiva(fattura.getDs_fattura_passiva());
				ritorno.setNr_fattura_fornitore(fattura
						.getNr_fattura_fornitore());
				ritorno.setDt_fattura_fornitore(fattura
						.getDt_fattura_fornitore());
				ritorno.setPartita_iva(fattura.getPartita_iva());
				ritorno.setCodice_fiscale(fattura.getCodice_fiscale());
				...
				ritorno.setDt_pagamento_fondo_eco(fattura
						.getDt_pagamento_fondo_eco());
				ritorno.setStato_pagamento_fondo_eco(fattura
						.getStato_pagamento_fondo_eco());

				if (det.getObbligazione_scadenziario() != null
						&& det.getObbligazione_scadenziario().getObbligazione() != null) {
					ObbligazioneBulk obbl = det.getObbligazione_scadenziario()
							.getObbligazione();
					ritorno.setPg_obbligazione_impegno(obbl
							.getPg_obbligazione());
					ritorno.setEsercizio_obbligazione_impegno(obbl
							.getEsercizio());
					ritorno.setDt_emissione_obbligazione_impegno(obbl
							.getDt_registrazione());
				}
				...
				if (det instanceof Fattura_passiva_rigaIBulk) {
					...
					if (mans != null && !mans.isEmpty()) {
						...
						ritorno.setDt_emissione_mandato(manr.getMandato()
								.getDt_emissione());
					}
				}
	...
	private void impostaImportiRiga(Fattura_passiva_rigaBulk det,
			FatturaPassiva ritorno) {
	...
	private void caricaCompenso(UserContext userContext,
		...
		if (base.booleanValue()) {
			...
			ritorno.setCd_unita_organizzativa(compenso
					.getCd_unita_organizzativa());
			...
			ritorno.setDt_pagamento_fondo_eco(compenso
					.getDt_pagamento_fondo_eco());
			ritorno.setStato_pagamento_fondo_eco(compenso
					.getStato_pagamento_fondo_eco());

			if (compenso.getObbligazioneScadenzario() != null
					&& compenso.getObbligazioneScadenzario().getObbligazione() != null) {
				ObbligazioneBulk obbl = compenso.getObbligazioneScadenzario()
						.getObbligazione();
	...
	private SOAPFault faultChiaveCompensoNonCompleta() throws SOAPException {
		return generaFault("004",
				"Identificativo Compenso non valido e/o incompleto");
	}
