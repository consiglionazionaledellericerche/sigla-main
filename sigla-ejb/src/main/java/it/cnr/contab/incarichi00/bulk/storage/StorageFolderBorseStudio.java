...
public class StorageFolderBorseStudio extends StorageFolderContrattiModel {
	...
	public String getCMISPath() {
		...
		if (cmisPath!=null) {
			cmisPath = cmisPath.concat(SiglaStorageService.SUFFIX).concat("Borsa di Studio "+this.getEsercizio().toString()+Utility.lpad(this.getPg_repertorio().toString(),10,'0'));
		}
		return cmisPath;
	}
