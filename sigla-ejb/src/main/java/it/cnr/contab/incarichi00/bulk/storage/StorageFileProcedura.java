...
public class StorageFileProcedura extends StorageFile implements StorageTypeName {
	...
	public String getStorageAlternativeParentPath() {
		...
		if (cmisPath != null)
			cmisPath = cmisPath.concat(SiglaStorageService.SUFFIX).concat((String)Incarichi_archivioBulk.getTipo_archivioKeys().get(this.getIncaricoProceduraArchivio().getTipo_archivio()));
		return cmisPath;
	}
