 ...
 * Creation date: (10/09/2002 16:36:21)
 * @author: CNRADM
 */
public class ListaMessaggiBP extends it.cnr.jada.util.action.SelezionatoreListaBP {
...
public GestioneLoginComponentSession createComponentSession() throws BusinessProcessException {
	return (GestioneLoginComponentSession)createComponentSession("CNRUTENZE00_NAV_EJB_GestioneLoginComponentSession",GestioneLoginComponentSession.class);
}
public it.cnr.jada.util.jsp.Button[] createNavigatorToolbar() {
	...
	toolbar[i++] = new it.cnr.jada.util.jsp.Button(it.cnr.jada.util.Config.getHandler().getProperties(getClass()),"Toolbar.remove");
	toolbar[i++] = new it.cnr.jada.util.jsp.Button(it.cnr.jada.util.Config.getHandler().getProperties(getClass()),"Toolbar.removeAll");
	return toolbar;
}
/**
 * Insert the method's description here.
 * Creation date: (11/09/2002 17:18:46)
...
protected void init(it.cnr.jada.action.Config config,it.cnr.jada.action.ActionContext context) throws it.cnr.jada.action.BusinessProcessException {
	setBulkInfo(it.cnr.jada.bulk.BulkInfo.getBulkInfo(it.cnr.contab.messaggio00.bulk.MessaggioBulk.class));
...
public void refresh(ActionContext context) throws it.cnr.jada.action.BusinessProcessException {
	try {
		setIterator(context,it.cnr.jada.util.ejb.EJBCommonServices.openRemoteIterator(context,createComponentSession().listaMessaggi(context.getUserContext(),server_url)));
		reset(context);
	} catch(Throwable e) {
...
public it.cnr.jada.util.action.Selection saveSelection(it.cnr.jada.action.ActionContext context) throws BusinessProcessException {
	setModel(context,null);
 ...
 * Insert the method's description here.
 * Creation date: (11/09/2002 17:18:46)
 * @param newServer_url java.lang.String
 */
public void setServer_url(java.lang.String newServer_url) {
	server_url = newServer_url;
}
/* 
 * Nel momento in cui l'utente naviga sul record viene memorizzato nella tabella Messaggio_Visionato
 */
public int setFocus(ActionContext actioncontext) throws BusinessProcessException {
	...
	if (!messaggio.getVisionato().booleanValue()) {
		try {
			CRUDMessaggioComponentSession messaggioComponentSession = (CRUDMessaggioComponentSession)createComponentSession("CNRMESSAGGIO00_EJB_CRUDMessaggioComponentSession",CRUDMessaggioComponentSession.class);
			messaggioComponentSession.setMessaggioVisionato(actioncontext.getUserContext(), messaggio);
