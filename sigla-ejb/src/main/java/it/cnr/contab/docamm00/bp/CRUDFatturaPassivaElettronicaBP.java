...
public class CRUDFatturaPassivaElettronicaBP extends AllegatiCRUDBP<AllegatoFatturaBulk, DocumentoEleTestataBulk> implements FatturaPassivaElettronicaBP {
    ...
    private boolean isAdminFatturaElettronica;
    ...
    protected void initialize(ActionContext actioncontext)
        ...
        try {
            ...
            setEsercizioAperto(EJBCommonServices.createEJB("CNRCONFIG00_EJB_EsercizioComponentSession", EsercizioComponentSession.class).isEsercizioAperto(actioncontext.getUserContext()));
            ...
            isAdminFatturaElettronica = EJBCommonServices.createEJB(
                    "CNRUTENZE00_NAV_EJB_GestioneLoginComponentSession",
                    GestioneLoginComponentSession.class
            ).controllaAccessoSenzaUO(actioncontext.getUserContext(), "AMMFATTURDOCELADMIN");
    ...
    protected OggettoBulk cancellaLogicamente(AllegatoGenericoBulk allegato) {
        AllegatoFatturaBulk allegatoFatturaBulk = (AllegatoFatturaBulk)allegato;
    ...
    protected Boolean isDaCancellareLogicamente(AllegatoGenericoBulk allegato) {
        if (isComunicazioneNonRegistabilita(allegato) && isAdminFatturaElettronica &&
                Optional.ofNullable(allegato)
                        .filter(AllegatoFatturaBulk.class::isInstance)
                        .map(AllegatoFatturaBulk.class::cast)
                        .filter(allegatoFatturaBulk -> !Optional.ofNullable(allegatoFatturaBulk.getDataCancellazione()).isPresent())
                        .isPresent()
        ) {
            return true;
        }
        return super.isDaCancellareLogicamente(allegato);
    }

    @Override
    protected Boolean isPossibileCancellazione(AllegatoGenericoBulk allegato) {
        if (isComunicazioneNonRegistabilita(allegato)) {
			if (isAdminFatturaElettronica &&
                    Optional.ofNullable(allegato)
                            .filter(AllegatoFatturaBulk.class::isInstance)
                            .map(AllegatoFatturaBulk.class::cast)
                            .filter(allegatoFatturaBulk -> !Optional.ofNullable(allegatoFatturaBulk.getDataCancellazione()).isPresent())
    ...
    protected Boolean isPossibileModifica(AllegatoGenericoBulk allegato) {
        ...
        if(!allegato.isToBeUpdated() && Optional.ofNullable(allegato)
                .filter(AllegatoFatturaBulk.class::isInstance)
                .map(AllegatoFatturaBulk.class::cast)
                .map(allegatoFatturaBulk -> Optional.ofNullable(allegatoFatturaBulk.getDataCancellazione()).isPresent()).isPresent())
            return false;
    ...
    protected String getRowDetailStyle(AllegatoGenericoBulk allegatoGenericoBulk) {
        AllegatoFatturaBulk allegatoFatturaBulk = (AllegatoFatturaBulk)allegatoGenericoBulk;

        StringBuffer style = new StringBuffer();
        if (Optional.ofNullable(allegatoFatturaBulk.getDataCancellazione()).isPresent())
