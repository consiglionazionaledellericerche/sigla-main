...
//ndc
	try {
		...
		it.cnr.jada.util.RemoteIterator ri = findNoteDiCreditoFor(context, (Fattura_attiva_IBulk)bp.getModel());
		...
		else return select(context, ri, it.cnr.jada.bulk.BulkInfo.getBulkInfo(Nota_di_credito_attivaBulk.class), "default", "doApriNdCSelezionata");
	} catch (Throwable e) {
		return handleException(context, e);
	}
}
...
//ndd
	try {
		...
		it.cnr.jada.util.RemoteIterator ri = findNoteDiDebitoFor(context, (Fattura_attiva_IBulk)bp.getModel());
		ri = it.cnr.jada.util.ejb.EJBCommonServices.openRemoteIterator(context, ri);
		if (ri == null || ri.countElements() == 0){
			it.cnr.jada.util.ejb.EJBCommonServices.closeRemoteIterator(context, ri);
		...
		else return select(context, ri, it.cnr.jada.bulk.BulkInfo.getBulkInfo(Nota_di_debito_attivaBulk.class), "default", "doApriNdDSelezionata");
	} catch (Throwable e) {
		return handleException(context, e);
	}
}
