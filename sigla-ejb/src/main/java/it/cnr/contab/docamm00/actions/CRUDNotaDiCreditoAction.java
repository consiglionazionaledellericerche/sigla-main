...
public Forward doAddToCRUDMain_Dettaglio(ActionContext context) {
    ...
    try {
	    ...
	    bp.getDettaglio().getSelection().clearSelection();
	    fillModel(context);
	    it.cnr.jada.util.RemoteIterator ri = ((FatturaPassivaComponentSession)bp.createComponentSession()).cercaFatturaPerNdC(context.getUserContext(), (Nota_di_creditoBulk)bp.getModel());
...
private Forward basicDoRicercaAccertamento(
	    ...
	    try {
	        ...
	        filtro.setHasDocumentoCompetenzaCOGESoloInAnnoCorrente(
		        ...
		        nc.getDateCalendar(nc.getDt_a_competenza_coge()).get(java.util.Calendar.YEAR) == nc.getEsercizio().intValue());
	        if (models == null || models.isEmpty())
