...
private java.util.Vector basicAddDetailsTo(
	...
	if (!notaDiCredito.isEditable() || target.isViewing()) {
		if (!bp.getDettaglio().getSelection().isEmpty())
			target.setErrorMessage("Uno o pi� dettagli non sono stati aggiunti perch� la nota di credito � gi� stata pagata o annullata o riportata!");
	} else {
		for (java.util.Iterator i = bp.getDettaglio().getSelection().iterator(); i.hasNext();) {
			...
			if (!dettaglio.STATO_INIZIALE.equals(dettaglio.getStato_cofi())) {
				...
				try {
					...
					target.setErrorMessage("Uno o pi� dettagli non sono stati aggiunti o per mancanza di disponibilt� o perch� il bene contenuto � di tipo sconto/abbuono!");
				}
			}
		}
	}
...
private java.util.Vector basicAddDetailsTo(ActionContext context, CRUDNotaDiDebitoBP target)
	...
	if (!notaDiDebito.isEditable() || target.isViewing()) {
		if (!bp.getDettaglio().getSelection().isEmpty())
			target.setErrorMessage("Uno o pi� dettagli non sono stati aggiunti perch� la nota di debito � gi� stata pagata o annullata o riportata!");
	} else {
		for (java.util.Iterator i = bp.getDettaglio().getSelection().iterator(); i.hasNext();) {
			Fattura_passiva_rigaIBulk dettaglio = (Fattura_passiva_rigaIBulk)bp.getDettaglio().getDetails().get(((Integer)i.next()).intValue());
...
private void basicDoOnIstituzionaleCommercialeChange(ActionContext context, Fattura_passivaBulk fattura)
	...
	try {
		...
		if (fattura.getFornitore() != null && fattura.getFornitore().getCrudStatus() == it.cnr.jada.bulk.OggettoBulk.NORMAL) {
			...
			((it.cnr.jada.util.action.CRUDBP)context.getBusinessProcess()).setMessage("Attenzione: il fornitore non � pi� valido. Selezionare un altro fornitore!");
		}*/
	} catch (Throwable t) {
...
public Forward doApriNotaDiCredito(ActionContext context) {
	...
	try {
		...
		else return select(context, ri, it.cnr.jada.bulk.BulkInfo.getBulkInfo(Nota_di_creditoBulk.class), "default", "doApriNdCSelezionata");
	} catch (Throwable e) {
		return handleException(context, e);
	}
}
...
public Forward doApriNotaDiDebito(ActionContext context) {
	...
	try {
		...
		else return select(context, ri, it.cnr.jada.bulk.BulkInfo.getBulkInfo(Nota_di_debitoBulk.class), "default", "doApriNdDSelezionata");
	} catch (Throwable e) {
		return handleException(context, e);
	}
}
 ...
 */
 ...
 public Forward doOnModalitaPagamentoChange(ActionContext context) {
	...
	try {
		...
		CRUDFatturaPassivaBP bp = (CRUDFatturaPassivaBP)getBusinessProcess(context);
		Fattura_passivaBulk fattura = (Fattura_passivaBulk)bp.getModel();
		//se c'� almeno un dettaglio non faccio pi� modificare la modalit� di pagamento
