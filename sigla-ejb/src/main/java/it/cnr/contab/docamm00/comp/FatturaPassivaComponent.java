	...
	if (original != null) {
		...
		//ora eseguito anche se la sola autofattura � stampata sui registri IVA
			...
			if (!original.getDt_registrazione().equals(fatturaPassiva.getDt_registrazione()))
				throw new it.cnr.jada.comp.ApplicationException("Attenzione: non � possibile modificare la data registrazione della " + fatturaPassiva.getDescrizioneEntita() + " o della sua autofattura (se esiste) quando lo stato IVA � B o C");
			...
			if (!original.getNr_fattura_fornitore().equalsIgnoreCase(fatturaPassiva.getNr_fattura_fornitore()) ||
				...
				throw new it.cnr.jada.comp.ApplicationException("Attenzione: non si possono modificare campi relativi alla " + fatturaPassiva.getDescrizioneEntita() + " del fornitore (sezionali, importi totali o riferimenti) quando la fattura o la relativa autofattura (se esiste) � in stato IVA B o C");

			if (!original.getCd_divisa().equals(fatturaPassiva.getCd_divisa()) ||
				original.getCambio().compareTo(fatturaPassiva.getCambio()) != 0)
				throw new it.cnr.jada.comp.ApplicationException("Attenzione: non si possono modificare i campi della valuta o del cambio della " + fatturaPassiva.getDescrizioneEntita() + " quando essa o la sua autofattura (se esiste) � in stato IVA B o C");
				
			if (!original.getCd_terzo().equals(fatturaPassiva.getCd_terzo()))
				throw new it.cnr.jada.comp.ApplicationException("Attenzione: non si possono modificare campi relativi al fornitore della " + fatturaPassiva.getDescrizioneEntita() + " quando essa o la sua autofattura (se esiste) � in stato IVA B o C");
		}
