...
public Forward doAddToCRUDMain_Modalita_pagamento(ActionContext context) {
	try {
		...
		RemoteIterator ri = it.cnr.jada.util.ejb.EJBCommonServices.openRemoteIterator(context,((TerzoComponentSession)bp.createComponentSession()).cercaModalita_pagamento_disponibiliByClause(context.getUserContext(),(TerzoBulk)bp.getModel()));
		int count = ri.countElements();
		if (count == 0) {
			bp.setMessage("Nessuna modalità di pagamento trovata");
			it.cnr.jada.util.ejb.EJBCommonServices.closeRemoteIterator(context,ri);
		} else {
			SelezionatoreListaBP selezionatore = select(context,ri,BulkInfo.getBulkInfo(Rif_modalita_pagamentoBulk.class),null,"doBringBackModalita_pagamento");
...
public Forward doAddToCRUDMain_Modalita_pagamento_Banche(ActionContext context) {
	try {
		CRUDTerzoBP bp = (CRUDTerzoBP)getBusinessProcess(context);
		TerzoBulk terzo = (TerzoBulk)bp.getModel();
		Modalita_pagamentoBulk modalita = (Modalita_pagamentoBulk)bp.getCrudModalita_pagamento().getModel();
		boolean modalitaPerCessione = modalita.isPerCessione();
		boolean modalitaAnnullata = modalita.isAnnullata();

		if (modalitaAnnullata)
			throw new it.cnr.jada.comp.ApplicationException("Attenzione: non è possibile aggiungere nuovi dettagli poichè la modalità di pagamento prescelta è stata Annullata.");

		TerzoComponentSession terzoComponent = (TerzoComponentSession)bp.createComponentSession();

		if (modalitaPerCessione){

			RemoteIterator iterator = terzoComponent.cercaBanchePerTerzoCessionario(context.getUserContext(), modalita);
			...
			if (iterator.countElements() == 0) {
				...
				throw new it.cnr.jada.comp.ApplicationException("Attenzione: il Terzo Delegato indicato non ha Banche utilizzabili.");
			}
			SelezionatoreListaBP banche_selezionate = select(context,iterator,BulkInfo.getBulkInfo(BancaBulk.class),modalita.getRif_modalita_pagamento().getTi_pagamento(),"doBringBackBanchePerCessionario");
			banche_selezionate.setMultiSelection(true);
			return banche_selezionate;
						
		} else {
			getController(context, "main.Modalita_pagamento.Banche").add(context);
			((BancaBulk)getController(context, "main.Modalita_pagamento.Banche").getModel()).setNazioniIban(terzoComponent.findNazioniIban(context.getUserContext(), ((BancaBulk)getController(context, "main.Modalita_pagamento.Banche").getModel())));
			return context.findDefaultForward();
		}
	...
	/**
	 * Assucia una termine di pagamento, tipo {@link Termini_pagamentoBulk }, a un terzo, tipo {@link TerzoBulk }.
	 *
	...
	public Forward doAggiungiTermini_pagamento(ActionContext context) {
		try {
			...
			for (java.util.Iterator i = bp.getCrudTermini_pagamento_disponibili().iterator();i.hasNext();)
...
public Forward doBringBackBanchePerCessionario(ActionContext context) {
	try {
		...
		TerzoComponentSession terzoComponent = (TerzoComponentSession)bp.createComponentSession();
		TerzoBulk terzo = (TerzoBulk)bp.getModel();
		Modalita_pagamentoBulk modalita_pagamento = (Modalita_pagamentoBulk)bp.getCrudModalita_pagamento().getModel();
		HookForward caller = (HookForward)context.getCaller();
		java.util.List banche = (java.util.List)caller.getParameter("selectedElements");
		if (banche != null){
			for (java.util.Iterator i = banche.iterator(); i.hasNext();){
				...
				nuova_banca.setNazioniIban(terzoComponent.findNazioniIban(context.getUserContext(), nuova_banca));
				
				java.util.List coll = (java.util.List)terzo.getBanchePerTipoPagamento().get(nuova_banca.getChiave());
				if (coll == null)
...
public Forward doBringBackModalita_pagamento(ActionContext context) {
	try {
		...
		TerzoBulk terzo = (TerzoBulk)bp.getModel();
		HookForward caller = (HookForward)context.getCaller();
		java.util.Collection coll = (java.util.Collection)caller.getParameter("selectedElements");
		if (coll != null) {
			for (java.util.Iterator i = coll.iterator();i.hasNext();) {
				Modalita_pagamentoBulk modalita_pagamento = new Modalita_pagamentoBulk();
				modalita_pagamento.setRif_modalita_pagamento((Rif_modalita_pagamentoBulk)i.next());
				modalita_pagamento.setTerzo(terzo);
				modalita_pagamento.setUser(context.getUserContext().getUser());
 ...
 */
public Forward doBringBackSearchFind_comune_sede(ActionContext context,TerzoBulk terzo,ComuneBulk comune) throws java.rmi.RemoteException {
	...
	if (comune != null) {
		...
		if( comune.getPg_comune() != null && !("".equals(comune.getPg_comune())) ) {
			try {
				terzo = ((TerzoComponentSession)bp.createComponentSession()).setComune_sede(context.getUserContext(),terzo,comune);
...
public Forward doBringBackSearchFind_unita_organizzativa(ActionContext context,TerzoBulk trz,Unita_organizzativaBulk uo) {
 ...
 * @param optionBP <code>OptionBP</code> l'oggetto che contiene le informazioni relative alla risposta dell'utente
 *
 * @return Forward
 */

public Forward doConfirmSalva(ActionContext context, OptionBP optionBP) throws it.cnr.jada.comp.ApplicationException{
...
public Forward doRemoveFromCRUDMain_Modalita_pagamento(ActionContext context) 
	...
	CRUDTerzoBP bp = (CRUDTerzoBP)getBusinessProcess(context);
	Modalita_pagamentoBulk mod_pag = (Modalita_pagamentoBulk)bp.getCrudModalita_pagamento().getModel();
	java.util.List selected_mod = bp.getCrudModalita_pagamento().getSelectedModels(context);
	TerzoBulk terzo = (TerzoBulk)bp.getModel();	
	boolean more_mod_pag = false;
	
	try{
		// Sono state selezionate più Modalità di Pagamento: controlla che non ve ne siano create da Migrazione.
		if (selected_mod != null && selected_mod.size() >0){
			for (java.util.Iterator i = selected_mod.iterator(); i.hasNext();){
				Modalita_pagamentoBulk sel_modalita = (Modalita_pagamentoBulk)i.next();
				if (sel_modalita.getUtcr() != null && sel_modalita.getUtcr().compareTo("$$$$$MIGRAZIONE$$$$$")==0){
					throw new it.cnr.jada.comp.ApplicationException("Attenzione: la  Modalità di Pagamento " + sel_modalita.getCd_modalita_pag() + " non è cancellabile\npoichè creata da una operazione di migrazione.");	
				}
			}
		} else if (mod_pag != null){	

			// Controlla che la mOdalità di Pagamento NON sia stata creata da Migrazione
			String ti_pagamento = mod_pag.getRif_modalita_pagamento().getTi_pagamento();			
			if (mod_pag.getUtcr() != null && mod_pag.getUtcr().compareTo("$$$$$MIGRAZIONE$$$$$")==0){
				throw new it.cnr.jada.comp.ApplicationException("Attenzione: la  Modalità di Pagamento " + mod_pag.getCd_modalita_pag() + " non è cancellabile\npoichè creata da una operazione di migrazione.");	
			}
					
			// Verifica l'esistenza di un'altra Modalità di Pagamento che abbia lo stesso ti_pagamento
			for (java.util.Iterator i = terzo.getModalita_pagamento().iterator();i.hasNext();) {
				...
				if ( m!= mod_pag && ti_pagamento.equals(m.getRif_modalita_pagamento().getTi_pagamento())){
					more_mod_pag = true;
				}
			}

			/* NON esistono altre Mod. di Pagamento dello stesso tipo di quella che si sta tentando 
			 *	di cancellare: il sistema controlla che, tra le banche collegate alla Mod. Pag. 
			 *	da cancellare NON ve ne siano che sono state create da procedure di MIGRAZIONE, 
			 *	(ORIGINE == 'S'): in tal caso, l'operazione sarà bloccata.
			*/
			if (!more_mod_pag && verifyBanche_StipendiFor(terzo, mod_pag)){
				throw new it.cnr.jada.comp.ApplicationException("Attenzione: questa Modalità di Pagamento non è cancellabile poichè una o più banche ad essa associate sono state create da una operazione di migrazione.");
			}
		}
 ...
 * @see it.cnr.jada.util.action.CRUDAction#doElimina
 */
public Forward doRemoveFromCRUDMain_Modalita_pagamento_Banche(ActionContext context) 
	throws ValidationException, BusinessProcessException{

	CRUDTerzoBP bp = (CRUDTerzoBP)getBusinessProcess(context);
	try{
		// Sono state selezionate + banche da cancellare: controlla che tra queste NON ci siano banche da MIGRAZIONE
		if (bp.getCrudBanche().getSelection(context).size()>1){ 
			java.util.List selected_banche = bp.getCrudBanche().getSelectedModels(context);
			if (selected_banche != null && selected_banche.size() >0){
				for (java.util.Iterator i = selected_banche.iterator(); i.hasNext();){
					BancaBulk banca = (BancaBulk)i.next();
					if (banca.isOrigineStipendi()){
						throw new it.cnr.jada.comp.ApplicationException("Attenzione: la banca " + banca.getIntestazione() + " non è cancellabile,\nin quanto creata da una operazione di migrazione.");
					}
				}
			}
		} else {
			...
			if (banca != null && banca.isOrigineStipendi()){
				throw new it.cnr.jada.comp.ApplicationException("Attenzione: la banca selezionata non è cancellabile,\nin quanto creata da una operazione di migrazione.");
			}
		}
		getController(context,"main.Modalita_pagamento.Banche").remove(context);
	...
	public Forward doRimuoviTermini_pagamento(ActionContext context) {
		try {
			fillModel(context);
			CRUDTerzoBP bp = (CRUDTerzoBP)context.getBusinessProcess();
			for (java.util.Iterator i = bp.getCrudTermini_pagamento().iterator();i.hasNext();)
				((OggettoBulk)i.next()).setUser(context.getUserContext().getUser());
 ...
 *	Controlla, per ogni modalità di pagamento indicata, se sono state speficicati i dati
 *	relativi alla banca di riferimento.
 *	Se una modalità risulta sprovvista dei dati necessari, viene visualizzato un messaggio
 *	che indica che quella modalità non verrà salvata e chiede se si vuole continuare.
 *	La risposta dell'Utente viene gestita dal metodo doConfirmSalva(ActionContext).
 *
 * @param context la <code>ActionContext</code> che ha generato la richiesta
 *
 * @return Forward
 ...
 * Creation date: (11/11/2002 11.29.43)
 * @return boolean
 * @param param it.cnr.contab.anagraf00.core.bulk.BancaBulk
 */
private boolean existBancaInCollection(java.util.List bancheColl, BancaBulk addBanca) {

	for (java.util.Iterator i = bancheColl.iterator(); i.hasNext();){
		BancaBulk existBanca = (BancaBulk)i.next();
			
		if ( (existBanca.getPg_banca_delegato().compareTo(addBanca.getPg_banca()) == 0)
				&& (existBanca.getCd_terzo_delegato().compareTo(addBanca.getTerzo().getCd_terzo())==0) ){
					return true;
				}
	}
 ...
 * @param param it.cnr.contab.anagraf00.core.bulk.BancaBulk
 */
private boolean verifyBanche_StipendiFor(TerzoBulk terzo, Modalita_pagamentoBulk mod_pag) {

	for (java.util.Iterator i = terzo.getBanche().iterator();i.hasNext();) {
		BancaBulk banca = (BancaBulk)i.next();		
		if (mod_pag.getChiavePerBanca().equals(banca.getChiave()) && banca.isOrigineStipendi()) {
			return true;
		}
	}
