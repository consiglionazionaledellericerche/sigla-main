...
public class CRUDTerzoBP extends SimpleCRUDBP {
	private final SimpleDetailCRUDController crudTelefoni                       = new SimpleDetailCRUDController("Telefoni",TelefonoBulk.class,"telefoni",this);

	private final SimpleDetailCRUDController crudFax                            = new SimpleDetailCRUDController("Fax",TelefonoBulk.class,"fax",this);

	private final SimpleDetailCRUDController crudEmail                          = new SimpleDetailCRUDController("Email",TelefonoBulk.class,"email",this);

	private final SimpleDetailCRUDController crudContatti                       = new SimpleDetailCRUDController("Contatti",ContattoBulk.class,"contatti",this);

	private final SimpleDetailCRUDController crudTermini_pagamento              = new SimpleDetailCRUDController("Termini_pagamento",Rif_termini_pagamentoBulk.class,"rif_termini_pagamento",this);

	private final SimpleDetailCRUDController crudTermini_pagamento_disponibili  = new SimpleDetailCRUDController("Termini_pagamento_disponibili",Rif_termini_pagamentoBulk.class,"rif_termini_pagamento_disponibili",this);

	private final SimpleDetailCRUDController crudModalita_pagamento             = new SimpleDetailCRUDController("Modalita_pagamento",Modalita_pagamentoBulk.class,"modalita_pagamento",this);

	//private final SimpleDetailCRUDController crudModalita_pagamento_disponibili = new SimpleDetailCRUDController("Modalita_pagamento_disponibili",Rif_modalita_pagamentoBulk.class,"rif_modalita_pagamento_disponibili",crudTerzi);

	private final SimpleDetailCRUDController crudBanche                         = new SimpleDetailCRUDController("Banche",BancaBulk.class,"banche",crudModalita_pagamento) {
		public java.util.List getDetails() {
			if (getTerzo() == null) return null;
			return getTerzo().getBanche((Modalita_pagamentoBulk)crudModalita_pagamento.getModel());
		}
		...
		public void add(ActionContext actioncontext, OggettoBulk oggettobulk) throws BusinessProcessException {
			if (oggettobulk instanceof BancaBulk) {
				...
				try{
					bancaClone = (BancaBulk)Utility.createAbiCabComponentSession().caricaStrutturaIban(actioncontext.getUserContext(), (BancaBulk)oggettobulk);
				} catch(Exception e) {
	...
	public void validaRiga(ActionContext actioncontext,
			OggettoBulk oggettobulk) throws ValidationException {
		try {
	...
	public CRUDTerzoBP(String function,AnagraficoBulk anagrafico) throws BusinessProcessException {
		this(function);
		this.anagrafico = anagrafico;
	}
public CRUDTerzoBP(String function,Unita_organizzativaBulk unita_organizzativa) throws BusinessProcessException {
	this(function);
	this.unita_organizzativa = unita_organizzativa;
}
public void edit(it.cnr.jada.action.ActionContext context,OggettoBulk bulk,boolean doInitializeForEdit) throws it.cnr.jada.action.BusinessProcessException {
 ...
 * Creation date: (05/08/2002 17:11:01)
...
public OggettoBulk getBringBackModel() {
	...
	if (tb == null)
		throw new MessageToUser("E' necessario selezionare un terzo",ERROR_MESSAGE);
 ...
 */
 ...
 public final it.cnr.jada.util.action.SimpleDetailCRUDController getCrudBanche() {
	...
	/**
	 * Restituisce il CRUDController relativo ai termini di pagamento disponibili.
	 *
	 * @return it.cnr.jada.util.action.SimpleDetailCRUDController
	 */
...
public String getFreeSearchSet() {
	...
	return 
		getAnagrafico().isStrutturaCNR() ? "terzoStrutturaCNR" : "terzoPersonaFisica";
}
...
/**
 * Insert the method's description here.
 * Creation date: (08/08/2002 16:52:03)
...
protected void initialize(it.cnr.jada.action.ActionContext context) throws it.cnr.jada.action.BusinessProcessException {
	try {
		if (unita_organizzativa == null) {
			...
			TerzoBulk terzo = ((it.cnr.contab.anagraf00.ejb.TerzoComponentSession)createComponentSession()).cercaTerzoPerUnitaOrganizzativa(context.getUserContext(),unita_organizzativa);
			if (terzo == null) {
				if (isEditable())
					reset(context,terzo = ((it.cnr.contab.anagraf00.ejb.TerzoComponentSession)createComponentSession()).inizializzaTerzoPerUnitaOrganizzativa(context.getUserContext(),unita_organizzativa));
				else
					throw new MessageToUser("Non esiste nessun terzo per l'unità organizzativa "+unita_organizzativa.getCd_unita_organizzativa());
			} else {
				edit(context,terzo);
...
public OggettoBulk initializeModelForFreeSearch(ActionContext context,OggettoBulk bulk) throws BusinessProcessException {
	TerzoBulk terzo = (TerzoBulk)super.initializeModelForFreeSearch(context,bulk);
...
public OggettoBulk initializeModelForInsert(ActionContext context,OggettoBulk bulk) throws BusinessProcessException {
	TerzoBulk terzo = (TerzoBulk)super.initializeModelForInsert(context,bulk);
	terzo.setAnagrafico(anagrafico);
	return terzo;
}
@Override
public OggettoBulk initializeModelForEdit(ActionContext actioncontext,OggettoBulk oggettobulk) throws BusinessProcessException {
	TerzoBulk terzo = (TerzoBulk)super.initializeModelForEdit(actioncontext, oggettobulk);

	BulkList bancheList = new BulkList();
	for (Iterator i=terzo.getBanche().iterator();i.hasNext();) {
		BancaBulk banca = (BancaBulk)i.next();
		try{
			BancaBulk bancaClone = (BancaBulk)Utility.createAbiCabComponentSession().caricaStrutturaIban(actioncontext.getUserContext(), banca);
...
public OggettoBulk initializeModelForSearch(ActionContext context,OggettoBulk bulk) throws BusinessProcessException {
	TerzoBulk terzo = (TerzoBulk)super.initializeModelForSearch(context,bulk);
	terzo.setAnagrafico(anagrafico);
	return terzo;
}
/**
...
public boolean isCrudModalita_pagamentoVisible() {
	...
	//Modalita_pagamentoBulk modalita_pagamento = (Modalita_pagamentoBulk)getCrudModalita_pagamento().getModel();
...
public boolean isDeleteButtonEnabled() {
	...
	TerzoBulk terzo = (TerzoBulk)getModel();
	if (terzo != null && (terzo.isDipendente() || terzo.isAnagraficoScaduto()))
		return false;
...
public boolean isInputReadonly() {
	return super.isInputReadonly() || ((TerzoBulk)getModel()).isTerzo_speciale();
}
...
public boolean isPerCessione() {
	...
	if (getCrudModalita_pagamento().getModel() != null
			&& ((Modalita_pagamentoBulk)getCrudModalita_pagamento().getModel()).isPerCessione()){
				return true;
	}
...
public OggettoBulk removeBanca(OggettoBulk bulk,int index) {
	return ((TerzoBulk)getModel()).removeFromBanche((BancaBulk)bulk,index);
}
...
public void validaBancaPerCancellazione(ActionContext context, BancaBulk banca) throws ValidationException{
	if (banca.getFl_cancellato().booleanValue())
		throw new ValidationException("La banca selezionata è già stata cancellata");
}
