...
public class AnnullamentoBP extends it.cnr.jada.util.action.BulkBP implements it.cnr.jada.util.action.SelectionListener
{
...
public AnnullamentoBP() {
		super("Tn");
//		super();
}
public AnnullamentoBP(String function) {
	super(function+"Tn");
//	super(function);	
}
/**
 * Ricerca di documenti contabili idonei all'annullamento
 * 
 *
 ...
 */
public it.cnr.jada.util.RemoteIterator cercaDocDaAnnullare(ActionContext context,V_obb_acc_xxxBulk model) throws it.cnr.jada.action.BusinessProcessException 
{
	try 
	{
		RicercaDocContComponentSession sessione = (RicercaDocContComponentSession) createComponentSession();
		return it.cnr.jada.util.ejb.EJBCommonServices.openRemoteIterator(context,sessione.cercaPerAnnullamento(context.getUserContext(),model));
	} catch(Exception e) {
...
public void clearSelection(it.cnr.jada.action.ActionContext context) throws it.cnr.jada.action.BusinessProcessException {
	...
	try 
	{
//		((V_obb_acc_xxxBulk)getModel()).setPg_call(null);
		((RicercaDocContComponentSession)createComponentSession()).clearSelectionPerAnnullamento(context.getUserContext(), (V_obb_acc_xxxBulk)getModel());
...
/**
 * richiama la stored procedure che effettua l'annullamento massivo sui documenti
 ...
 * @throws BusinessProcessException	
 */
public void confermaAnnullamento(ActionContext context) throws it.cnr.jada.action.BusinessProcessException 
{
	try 
	{
		RicercaDocContComponentSession sessione = (RicercaDocContComponentSession) createComponentSession();
		sessione.callAnnullamentoDocCont(context.getUserContext(), ((V_obb_acc_xxxBulk)getModel()).getPg_call());
	} catch(Exception e) {
		throw handleException(e);
	}
}
...
public void initializeSelection(it.cnr.jada.action.ActionContext context) throws it.cnr.jada.action.BusinessProcessException {
	...
	try 
	{
		V_obb_acc_xxxBulk doc = ((RicercaDocContComponentSession)createComponentSession()).initializeSelectionPerAnnullamento(
...
public void selectAll(it.cnr.jada.action.ActionContext context) throws it.cnr.jada.action.BusinessProcessException {
	try 
	{
		((RicercaDocContComponentSession)createComponentSession()).selectAllPerAnnullamento(
						context.getUserContext(),(V_obb_acc_xxxBulk)getModel());
	} catch(Exception e) {
		throw handleException(e);
	} 
...
public java.util.BitSet setSelection(it.cnr.jada.action.ActionContext context, it.cnr.jada.bulk.OggettoBulk[] bulks, java.util.BitSet oldSelection, java.util.BitSet newSelection) throws it.cnr.jada.action.BusinessProcessException {
	...
	try 
	{
		((RicercaDocContComponentSession)createComponentSession()).setSelectionPerAnnullamento(
