 ...
 * @author: Paola sala
 */
public class CRUDMissioneDiariaBP extends it.cnr.jada.util.action.SimpleCRUDBP {
/**
 * CRUDMissioneDiariaBP constructor comment.
 */
public CRUDMissioneDiariaBP() {
	super();
}
/**
 * CRUDMissioneDiariaBP constructor comment.
 ...
 */
public CRUDMissioneDiariaBP(String function) {
	super(function);
}
/**
...
public void basicEdit(ActionContext context,OggettoBulk bulk, boolean doInitializeForEdit) throws it.cnr.jada.action.BusinessProcessException {
	...
	if (!isViewing()){
		Missione_diariaBulk aDiaria = (Missione_diariaBulk)getModel();
		if(aDiaria.getDataFineValidita()!=null && aDiaria.getDataFineValidita().compareTo(CompensoBulk.getDataOdierna())<=0){
			setStatus(VIEW);		
		}
	}
}
