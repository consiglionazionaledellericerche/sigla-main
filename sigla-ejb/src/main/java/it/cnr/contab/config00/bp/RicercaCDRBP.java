...
public void eseguiRicerca(it.cnr.jada.action.ActionContext context)throws BusinessProcessException{
	if(uo== null){
		...
		try {
			setCDR(((CDRComponentSession)it.cnr.jada.util.ejb.EJBCommonServices.createEJB("CNRCONFIG00_EJB_CDRComponentSession",CDRComponentSession.class)).findListaCDRWS(context.getUserContext(),uo,query, dominio, ricerca));
