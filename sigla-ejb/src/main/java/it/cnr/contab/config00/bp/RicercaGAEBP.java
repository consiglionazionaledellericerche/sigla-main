...
public void eseguiRicerca(it.cnr.jada.action.ActionContext context)throws BusinessProcessException{
	...
	if(cdr== null){
		...
		try {
			GAE_old=(((Linea_attivitaComponentSession)it.cnr.jada.util.ejb.EJBCommonServices.createEJB("CNRCONFIG00_EJB_Linea_attivitaComponentSession",Linea_attivitaComponentSession.class)).findListaGAEWS(context.getUserContext(), getCdr(),getTipo(), getQuery(), getDominio(), getRicerca(),getFiltro()));
			setGAE(GAE_old);
			int num=0;
