...
public class MacroAction extends FormAction{
    ...
    public Forward doDefault(ActionContext actioncontext){
		try {
			...
			//Se il mode � H allora va in sola visualizzazione altrimenti in modifica			
			...
			setErrorMessage(actioncontext,"Password scaduta da pi� di sei mesi.");
			return actioncontext.findForward("password_scaduta");
		} catch(it.cnr.contab.utente00.nav.comp.UtenteNonValidoException e) {
			setErrorMessage(actioncontext,"Utente non pi� valido");
			return actioncontext.findDefaultForward();
		} catch(it.cnr.contab.utente00.nav.comp.UtenteInDisusoException e) {
			setErrorMessage(actioncontext,"Utente non utilizzato da pi� di sei mesi.");
			return actioncontext.findDefaultForward();
		} catch(Throwable e) {
