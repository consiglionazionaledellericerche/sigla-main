...
public class RiepilogativiIvaCentroDefinitivoBP extends StampaRegistriIvaBP {

	private int status = INSERT;
public RiepilogativiIvaCentroDefinitivoBP() {
	this("");
}
public RiepilogativiIvaCentroDefinitivoBP(String function) {
	super(function+"Tr");
}
 ...
 */
public Riepilogativi_iva_centro_definitivoVBulk createEmptyModelForSearch(ActionContext context) throws BusinessProcessException {
 ...
 */
public Riepilogativi_iva_centro_definitivoVBulk createNewBulk(ActionContext context) throws BusinessProcessException {
	try {
		Riepilogativi_iva_centro_definitivoVBulk bulk = new Riepilogativi_iva_centro_definitivoVBulk();
		bulk.setUser(context.getUserInfo().getUserid());
		bulk = (Riepilogativi_iva_centro_definitivoVBulk)bulk.initializeForSearch(this,context);
		...
		bulk.setTipi_sezionali(
			createComponentSession().selectTipi_sezionaliByClause(
														...
														null));
		return (Riepilogativi_iva_centro_definitivoVBulk)aggiornaRegistriStampati(context, bulk);
	} catch(Throwable e) {
		throw handleException(e);
	}
}
