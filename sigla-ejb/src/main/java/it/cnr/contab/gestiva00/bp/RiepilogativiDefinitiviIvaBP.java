...
public class RiepilogativiDefinitiviIvaBP extends StampaRegistriIvaBP {

	private int status = INSERT;
public RiepilogativiDefinitiviIvaBP() {
	this("");
}
public RiepilogativiDefinitiviIvaBP(String function) {
	super(function+"Tr");
}
/**
 * Invocato per creare un modello vuoto da usare su una nuova richiesta di ricerca.
 */
public Riepilogativi_definitivi_ivaVBulk createEmptyModelForSearch(ActionContext context) throws BusinessProcessException {
 ...
 */
public Riepilogativi_definitivi_ivaVBulk createNewBulk(ActionContext context) throws BusinessProcessException {
	try {
		Riepilogativi_definitivi_ivaVBulk bulk = new Riepilogativi_definitivi_ivaVBulk();
		bulk.setUser(context.getUserInfo().getUserid());
		bulk= (Riepilogativi_definitivi_ivaVBulk)bulk.initializeForSearch(this,context);
		...
		return (Riepilogativi_definitivi_ivaVBulk)aggiornaRegistriStampati(context, bulk);
	} catch(Throwable e) {
		throw handleException(e);
	}
}
