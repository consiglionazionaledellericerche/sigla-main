   ...
   try{
		...
		for(Iterator i=dettagliRev.iterator();i.hasNext();){
	    	V_mandato_reversaleBulk bulk=(V_mandato_reversaleBulk) i.next();
	    		currentReversale=(Reversale)recuperaDatiReversaleFlusso(context.getUserContext(), bulk);
		...
		for(Iterator i=dettagliMan.iterator();i.hasNext();){
	    	V_mandato_reversaleBulk bulk=(V_mandato_reversaleBulk) i.next();
	    		currentMandato=recuperaDatiMandatoFlusso(context.getUserContext(), bulk);
...
private Mandato recuperaDatiMandatoFlusso(UserContext userContext,
		V_mandato_reversaleBulk bulk)throws ComponentException, RemoteException, BusinessProcessException {
	try{
		DistintaCassiereComponentSession componentDistinta =((DistintaCassiereComponentSession)createComponentSession());
		BancaBulk bancauo=componentDistinta.recuperaIbanUo(userContext,bulk.getUo());
...
//				infoben.setTipoPagamento("COMPENSAZIONE");
			// Classificazioni
			List listClass = componentDistinta.findDocumentiFlussoClass(userContext, bulk);
			...
			if(docContabile.getTiDocumento().compareTo(MandatoBulk.TIPO_REGOLAM_SOSPESO)==0){
				...
				List listSosp = componentDistinta.findDocumentiFlussoSospeso(userContext, bulk);
			...
			if(bulk.getIm_ritenute().compareTo(BigDecimal.ZERO)!=0){
				List list_rev = componentDistinta.findReversali(userContext, bulk);
...
//		if(docContabile.getTiDocumento().compareTo(ReversaleBulk.TIPO_INCASSO)==0 && bulk.getPg_documento_cont_padre().compareTo(bulk.getPg_documento_cont())==0)
		...
		List listClass = componentDistinta.findDocumentiFlussoClassReversali(userContext, bulk);
		VDocumentiFlussoBulk oldDoc=null;
		for(Iterator c=listClass.iterator();c.hasNext();){
			VDocumentiFlussoBulk doc=(VDocumentiFlussoBulk)c.next();
