...
public class ConsRiscontriEtrBP extends ConsultazioniBP {
	public java.util.Vector addButtonsToToolbar(java.util.Vector listButton){
		Button button_etr = new Button(Config.getHandler().getProperties(getClass()), "Toolbar.dettagli_etr");
		button_etr.setSeparator(true);
		listButton.addElement(button_etr);
		return listButton;
	}
	...
	protected void init(it.cnr.jada.action.Config config,ActionContext context) throws BusinessProcessException {
			   ...
			   String etr = SospesoBulk.TIPO_ENTRATA;
			  ...
			  if(!isUoEnte(context))	 {					
					...
					clauses.addClause("AND", "ti_entrata_spesa",SQLBuilder.EQUALS,etr);
			  }
			else 
			clauses.addClause("AND", "esercizio", SQLBuilder.EQUALS, esercizio);
			clauses.addClause("AND", "ti_sospeso_riscontro",SQLBuilder.EQUALS,ris);
