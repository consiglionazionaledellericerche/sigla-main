...
public BigDecimal getIm_associato_siope(){
	...
	for (Iterator i = getReversale_siopeColl().iterator(); i.hasNext();) totale = totale.add(Utility.nvl(((Reversale_siopeBulk)i.next()).getImporto()));
...
public BigDecimal getIm_associato_cup(){
	BigDecimal totale = Utility.ZERO;
	for (Iterator i = getReversaleCupColl().iterator(); i.hasNext();) totale = totale.add(((ReversaleCupBulk)i.next()).getImporto());
	return Utility.nvl(totale);
}
