...
private void aggiornaIm_pagato_incassato (UserContext userContext,Sospeso_det_etrBulk det_etr, V_mandato_reversaleBulk v_man_rev) throws ComponentException
{
	try
	{
		...
		if ( det_etr.STATO_ANNULLATO.equals( det_etr.getStato()) )
...
private void aggiornaIm_pagato_incassato (UserContext userContext,Sospeso_det_uscBulk det_usc, V_mandato_reversaleBulk v_man_rev) throws ComponentException
{
	...
	{
		BigDecimal im_sospeso;
		if ( det_usc.STATO_ANNULLATO.equals( det_usc.getStato()) )
...
private void aggiornaStatoDocContabile (UserContext userContext,Sospeso_det_etrBulk dettaglio, V_mandato_reversaleBulk v_man_rev) throws ComponentException
{
	try
	{
		...
		if(  ( v_man_rev.getIm_pagato_incassato().compareTo( v_man_rev.getIm_documento_cont()) == 0  && 
			   dettaglio.STATO_ANNULLATO.equals( dettaglio.getStato()) ) ||
			 ( !dettaglio.STATO_ANNULLATO.equals( dettaglio.getStato()) && 
...
public OggettoBulk creaConBulk (UserContext userContext,OggettoBulk bulk) throws ComponentException
{
	try
	{
		...
		//sospeso	
		if( sospeso.TI_SOSPESO.equals( sospeso.getTi_sospeso_riscontro() ) )
		{
			...
			if ( !sospeso.getCd_cds().equals( ente.getCd_unita_organizzativa() ))
			{
				...
				figlio.setStato_sospeso( sospeso.STATO_SOSP_ASS_A_CDS );
			}
		...
		if( sospeso.getTi_sospeso_riscontro().equals( sospeso.TI_RISCONTRO ) )
...
{
	if( sospeso.getTi_entrata_spesa().equals( sospeso.TIPO_ENTRATA) )
		creaDettaglioSospesoEtr( userContext, sospeso);
	else if( sospeso.getTi_entrata_spesa().equals( sospeso.TIPO_SPESA) )
...
private void creaDettaglioSospesoEtr (UserContext userContext,SospesoBulk sospeso) throws ComponentException
{
	try
	{
		...
		det_etr.setTi_sospeso_riscontro( sospeso.TI_RISCONTRO );
...
private void creaDettaglioSospesoUsc (UserContext userContext,SospesoBulk sospeso) throws ComponentException
{
	try
	{
		...
		det_usc.setTi_entrata_spesa( sospeso.TIPO_SPESA );
		det_usc.setTi_sospeso_riscontro( sospeso.TI_RISCONTRO );
...
public OggettoBulk inizializzaBulkPerModifica (UserContext aUC,OggettoBulk bulk) throws ComponentException
{
	...
	try
	{
		...
		else if ( sospeso.getTi_sospeso_riscontro().equals( sospeso.TI_SOSPESO ) )
		{
			...
			if ( sospeso.getTi_entrata_spesa().equals( sospeso.TIPO_ENTRATA))
			{
				...
				sql.addSQLClause( "AND", "esercizio", sql.EQUALS, sospeso.getEsercizio() );
				sql.addSQLClause( "AND", "cd_cds", sql.EQUALS, sospeso.getCd_cds() );
				sql.addSQLClause( "AND", "ti_sospeso_riscontro", sql.EQUALS, sospeso.getTi_sospeso_riscontro() );
...
public Query select(UserContext userContext, CompoundFindClause clauses, OggettoBulk bulk ) throws ComponentException, it.cnr.jada.persistency.PersistencyException
{
	if ( bulk instanceof SospesoBulk )
	{
		...
		sql.addSQLClause( "AND", "cd_sospeso_padre", sql.ISNULL, null );
		...
		if( sospeso.getV_man_rev() != null && sospeso.getV_man_rev().getPg_documento_cont() != null )
		{
			...
			if( sospeso.getV_man_rev().getCd_tipo_documento_cont().equals( Numerazione_doc_contBulk.TIPO_MAN ) )
			{
				...
				sql.addSQLClause("AND", "SOSPESO_DET_USC.PG_MANDATO", sql.EQUALS, sospeso.getV_man_rev().getPg_documento_cont()  );
			}
			else if( sospeso.getV_man_rev().getCd_tipo_documento_cont().equals( Numerazione_doc_contBulk.TIPO_REV ) )
			{
				...
				sql.addSQLClause("AND", "SOSPESO_DET_ETR.PG_REVERSALE", sql.EQUALS, sospeso.getV_man_rev().getPg_documento_cont()  );
			}
		}
		return sql;
	}
	else if ( bulk instanceof SelezionaSospesiCNRBulk )
	{
		...
		sql.addSQLClause( "AND", "fl_stornato", sql.EQUALS, "N" );
		sql.addSQLClause( "AND", "ti_sospeso_riscontro", sql.EQUALS, SospesoBulk.TI_SOSPESO );
		sql.addSQLClause( "AND", "IM_SOSPESO - IM_ASSOCIATO", sql.GREATER, new java.math.BigDecimal(0) );
		sql.addSQLClause( "AND", "IM_ASS_MOD_1210", sql.EQUALS, new java.math.BigDecimal(0) );				
		
		if ( seleziona.getTi_entrata_spesa().equals( seleziona.TIPO_ENTRATA))
			sql.addSQLClause( "AND", "ti_entrata_spesa", sql.EQUALS, SospesoBulk.TIPO_ENTRATA );
		else if ( seleziona.getTi_entrata_spesa().equals( seleziona.TIPO_SPESA))
			sql.addSQLClause( "AND", "ti_entrata_spesa", sql.EQUALS, SospesoBulk.TIPO_SPESA );			
		...
		if ( seleziona.getRicercaSospesiAssegnati().booleanValue())
		{
			...
			sql.addClause( "AND", "cd_cds_origine", sql.EQUALS, ((CNRUserContext)userContext).getCd_cds() );
		...
		if ( seleziona.getRicercaSospesiInSospeso().booleanValue() )
		{
			if (nrClausoleStato == 0 )
				...
				sql.openParenthesis("OR");			
			sql.addClause( "AND", "stato_sospeso", sql.EQUALS, SospesoBulk.STATO_SOSP_IN_SOSPESO );
...
public SQLBuilder selectV_man_rev_for_searchByClause( UserContext userContext, SospesoBulk bulk, V_mandato_reversaleBulk v_man_rev, CompoundFindClause clauses) throws ComponentException, it.cnr.jada.persistency.PersistencyException 
{
	...
	*/
	if ( bulk.getTi_entrata_spesa() != null && ( bulk.getTi_entrata_spesa().equals( bulk.TIPO_ENTRATA )) ) // sospeso di tipo entrata: ricerca reversali
		sql.addClause( "AND", "cd_tipo_documento_cont", sql.EQUALS, Numerazione_doc_contBulk.TIPO_REV );
	else if( bulk.getTi_entrata_spesa() != null && ( bulk.getTi_entrata_spesa().equals( bulk.TIPO_SPESA )) ) // sospeso di tipo spesa: ricerca mandati
...
*/
		...
		if ( sospeso.TI_RISCONTRO.equals( sospeso.getTi_sospeso_riscontro()))
			if ( sospeso.TIPO_SPESA.equals( sospeso.getTi_entrata_spesa()))
...
private void verificaRiscontroEntrataCNR (UserContext userContext,SospesoBulk sospeso) throws ComponentException
{
	try
	{
		...
		sql.addSQLClause( "AND", "REVERSALE_RIGA.cd_cds", sql.EQUALS, sospeso.getV_man_rev().getCd_cds())	;
		sql.addSQLClause( "AND", "REVERSALE_RIGA.esercizio", sql.EQUALS, sospeso.getV_man_rev().getEsercizio())	;
		sql.addSQLClause( "AND", "REVERSALE_RIGA.pg_reversale", sql.EQUALS, sospeso.getV_man_rev().getPg_documento_cont());
...
private void verificaSospesoRiscontro (UserContext aUC,SospesoBulk sospeso) throws ComponentException
{
	try
	{
		...
		if( sospeso.TI_RISCONTRO.equals( sospeso.getTi_sospeso_riscontro()) )
		{
		   ...
		   if( (sospeso.getTi_entrata_spesa() != null) && (sospeso.getTi_entrata_spesa().equals( sospeso.TIPO_ENTRATA )) )
			{
				if ( sospeso.getDettaglio_etr() == null ||
