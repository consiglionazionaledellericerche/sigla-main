...
public java.util.List findCapitoliDiEntrataCds( AccertamentoBulk accertamento ) throws IntrospectionException,PersistencyException 
{
	...
	sql.addClause(FindClause.AND,"ti_appartenenza",SQLBuilder.EQUALS, accertamento.getTi_appartenenza());
	sql.addClause(FindClause.AND,"ti_gestione",SQLBuilder.EQUALS,accertamento.getTi_gestione());
	sql.addClause(FindClause.AND,"cd_voce",SQLBuilder.EQUALS,accertamento.getCd_voce());
