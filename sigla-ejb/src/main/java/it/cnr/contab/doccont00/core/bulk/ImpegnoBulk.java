...
/*	if ( !Numerazione_doc_contBulk.TIPO_IMP_RES.equals( getCd_tipo_documento_cont()))
	{
		*/
		if ( //  data obbligazione != data scadenza && data_obbligazione >= data_scadenza
			...
			throw new ValidationException( "Non è possibile inserire una data scadenza antecedente a quella di registrazione dell'Impegno su bilancio Ente." );
		...
		if (gc.get(java.util.GregorianCalendar.YEAR) < getEsercizio().intValue())
			throw new ValidationException( "Non è possibile inserire una data scadenza antecedente all'esercizio di creazione dell'Impegno su bilancio Ente." );
/*	}	*/

	// controlli su campo IMPORTO	
	if (  getIm_obbligazione() == null ) 
		throw new ValidationException( "L' IMPORTO dell'Impegno su bilancio Ente non può essere nullo." );
