...
-- il campo IMP_FISC della tabella CNR_ANADIP ? inserito nel campo IRPEF_LORDO_DIPENDENTI della tabella MONTANTI
...
-- modificata data di cessazione dell'inquadramento da creare (nuovo) gi? chiuso
--
-- Date: 19/03/2003
-- Version: 1.29
...
-- Date: 13/05/2003
-- Version: 2.4
-- Nel calcolo montanti ? stata inserita la vista v_stipendi_montanti
--
-- Date: 15/05/2003
-- Version: 2.5
...
-- perch? ? presente nell'indirizzo  aAnaDip.ind_resid e se questo cambia non viene aggiornato il numero civico
-- quindi pu? accadere che ci siano 2 diversi numeri civici.
...
procedure caricaAnagDipendenti(aAnnoRif Number, aMeseRif Number, aUtente varchar2) is
 ...
 --Dbms_Output.put_line('caricaAnagDipendenti');
 	  ...
 	  -- La tabella CNR_ANADIP in questa fase deve essere gi? caricata con i dati
...
-- Flavia -- modifica: ALLINEA_MONTANTE era fuori dal loop, ? stata spostata perch? sono cambiati i parametri di input
...
--Dbms_Output.put_line('CREAINQUADRAMENTO'||' matricola '||aAnaDip.matricola);
	  ...
	  if lContinuaEsecuzione then
		 ...
		 if ( trunc(aAnadip.data_assunzione) > trunc(aAnadip.profilo_prec_fine) ) then
					 ibmutl200.logInf(gPgLog ,'Matricola ' || aAnaDip.matricola , 'La data di assunzione ? maggiore della data fine profilo precedente', '');
		 end if;
...
-- Non prendiamo quelli che hanno stipendio a zero.
			  ...
			  -- allora per avere una distribuzione scalare INPS, IRPEF pi? precisa divideremo
...
--29/05/2006 un rapporto viene allineato solo quando la data cessazione della tabella cnr_anadip ? valorizzata
	-- altrimenti se ? valorizzata solo profilo_prec_fine deve essere allineato solo l'inquadramento
	...
	exception
        ...
        End If;
			  ...
			  -- Se esiste il rapporto ma ? cambiato il pg_rif_inquadramento deve essere inserito un nuovo record
			 ...
			 begin
				select * into lInquadramento
			  	...
			  	-- Se la data di fine validita del rapporto ? diversa dalla data di fine validita
...
--Dbms_Output.put_line('MODIFICABANCA'||' matricola '||aAnaDip.matricola);
	  ...
	  if lNumBanche > 0 then
         ...
         -- altrimenti se ? impostato a N  non bisogna fare niente.
         -- inizio
							...
							if pg_esistente is not null and flag = 'Y' and Nvl(aAnaDip.iban_pag,'0')!='0' then
                                ...
                                begin
										    ...
										    begin
													...
													--dbms_output.put_line('DA AGG - cd_terzo ='|| aTerzo.cd_terzo||' ha pi? di un conto');
         ...
         -- fine la banca esiste ed ? stata aggiornata!
...
--	  else -- lInsBanca
	...
	*/
		 ...
		 -- se non ? cos? allora inseriamo la banca, altrimenti NO
...
--FLAVIA--
		-- recuperiamo il record precedente. se ? cambiato il numero di conto e il precedente non ? stato reso inutilizzabile
		-- ? necessario impostare il fl_cancellato = 'Y'
...
--Dbms_Output.put_line('chkPresenzaAnag'||' matricola '||aAnaDip.matricola);
	...
	-- ? stata aggiunta la condizione che la dt_fine_rapporto sia null a causa delle duplicazioni
	--  dt_fine_rapporto is null indica che l'anagrafico ? valido
 	  select a.*
	  into aAnagrafico
...
--Dbms_Output.put_line('INS_ANAGRAFICO'||' anag '||aAnagrafico.cd_anag);
 ...
 when dup_val_on_index then
 	  ibmutl200.logInf(gPgLog ,'Matricola ' || aAnaDip.matricola , ' risulta gi? inserita la modalita di pagamento ' || aModPagamento.CD_MODALITA_PAG, '');
 end;
...
/* da testare  nuovo aggiornamento per i rapporti chiusi che vengono ripristinati! */
		  -- se un rapporto era stato chiuso ma il dipendente ? rientrato in servizio
		  -- viene aggiornata la data di fine validit? del rapporto.
		  ...
		  Dbms_Output.put_line('aggiorno il rapporto con la nuova data di fine validit? - '||' matricola '||aAnaDip.matricola);
		  ...
		  Dbms_Output.put_line('data di fine validit? - '||nvl(aAnaDip.data_cessazione,to_date('31122200','ddmmyyyy')));
		  end if;
...
procedure UPD_INQUADRAMENTO (aAnaDip cnr_Anadip%rowtype
 ...
 --Dbms_Output.put_line('UPD_INQUADRAMENTO'||' matricola '||aAnaDip.matricola);
	  ...
	  if  lContinuaEsecuzione then
		  ...
		  ibmutl200.logInf(gPgLog ,'Matricola ' || aAnaDip.matricola , 'Impossibile inserire l''inquadramento la data di inizio rapporto non ? valida', '');
		  end;
	  end if;
 ...
 begin
 	  ...
 	  for lVStipMon in (select * from v_stipendi_montanti
 	  		    ...
 	  		    and matricola = aAnaDip.matricola)
	  loop
		  -- 1) Verifichiamo se il dipendente in esame ? contenuto in cnr_anadip
