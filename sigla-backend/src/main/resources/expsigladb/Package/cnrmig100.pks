...
begin
  ...
  /* ribaltamento_log viene cancellata quindi non ? attendibile fare questo controllo
	begin
...
begin
	...
	begin
		...
		if isRibaltamentoAltroEffettuato(aEs, aPgEsec) then
		   stato_fine := 'E';
		   aMessage := 'Lo script di ribaltamento ? gi? stato eseguito con successo per l''esercizio '|| aEs ||': non ? possibile eseguirlo nuovamente.';
		   endLogRibaltamentoAltro(aEs, aPgEsec, stato_fine, aMessage);
		end if;
		if Not isRibaltamentoPDGPEffettuato(aEs, aPgEsec) then
		   stato_fine := 'E';
		   aMessage := 'Non ? stato ancora eseguito il ribaltamento per il PDGP per l''esercizio '|| aEs ||': non ? possibile proseguire.';
		   endLogRibaltamentoAltro(aEs, aPgEsec, stato_fine, aMessage);
		end if;
	...
	for aElVoce in (select * from ELEMENTO_VOCE
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'ELEMENTO_VOCE con PK('||aEs||', '
					||aElVoce.TI_APPARTENENZA||', '
					||aElVoce.TI_GESTIONE||', '
					||aElVoce.CD_ELEMENTO_VOCE||') gi? esistente','','');
			stato_fine := 'W';
		end;
	end loop;
	...
	for aCatGV in (select * from CATEGORIA_GRUPPO_VOCE
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'CATEGORIA_GRUPPO_VOCE con PK ('||aCatGV.CD_CATEGORIA_GRUPPO||', '
					 ...
					 ||aCatGV.TI_APPARTENENZA||', '
					 ||aCatGV.TI_GESTIONE||', '
					 ||aCatGV.CD_ELEMENTO_VOCE||') gi? esistente','','');
			stato_fine := 'W';
		end;
	end loop;
	  ...
	  loop
		...
		exception
		   when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'CLASSIFICAZIONE_VOCI con PK ('||new_id||') gi? esistente','','');
			stato_fine := 'W';
		   when OTHERS then
	...
	Loop
		...
		Exception when dup_val_on_index then
			ibmutl200.logwar(aPgEsec,'Codice SIOPE con PK('||aEs||', '
							     	       ||aCodSiope.TI_GESTIONE||', '
								       ||aCodSiope.CD_SIOPE||') gi? inserito','','');
			stato_fine := 'W';
		End;
	End Loop;
	...
	Loop
		...
		Exception when dup_val_on_index then
			ibmutl200.logwar(aPgEsec,'Associazione Tipologia ISTAT - Codici SIOPE con PK('||aAssIstatSiope.PG_TIPOLOGIA||', '
							     	       				      ||aEs||', '
							     	                                      ||aAssIstatSiope.TI_GESTIONE_SIOPE||', '
								                                      ||aAssIstatSiope.CD_SIOPE||') gi? inserita','','');
			stato_fine := 'W';
		End;
	End Loop;
	...
	For aAssEVSiope in (select * from ASS_EV_SIOPE
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'ASS_EV_SIOPE con PK('||aEs||', '
					...
					||aEs||', '
					||aAssEVSiope.TI_GESTIONE_SIOPE||', '
					||aAssEVSiope.CD_SIOPE||') gi? esistente','','');
			stato_fine := 'W';
		end;
	end loop;
		...
		Exception when dup_val_on_index then
			ibmutl200.logwar(aPgEsec,'Associazione Contributi/ritenute - Codici SIOPE con PK('||aEs||', '
							     	       				      ||aAssContrRitSiope.CD_CONTRIBUTO_RITENUTA||', '
							     	                                      ...
							     	                                      ||aAssContrRitSiope.CD_SIOPE_S||', '
							     	                                      ||aAssContrRitSiope.TI_GESTIONE_SIOPE_E||', '
								                                      ||aAssContrRitSiope.CD_SIOPE_E||') gi? inserita','','');
			stato_fine := 'W';
		End;
	End Loop;
...
-- fine spostamento in ribaltamento  altro rospuc
		...
		begin
			...
			where ESERCIZIO = aEsPrec;
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'FIRME per l''esercizio base '||aEs||' gi? esistenti','','');
			stato_fine := 'W';
		end;

		-- Ribaltamento ASS_INCARICO_ATTIVITA sull'esercizio contabile destinazione
		-- a partire dall'esercizio precedente
		-- archivio gi? caricato anche negli anni successivi
		...
		begin
			...
			where ESERCIZIO = aEsPrec;
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'ASS_INCARICO_ATTIVITA per l''esercizio base '||aEs||' gi? esistente','','');
			stato_fine := 'W';
		end;
		...
		for aCC in (select * from causale_coge
			...
			begin
				 ...
				 values (aEs,
					 ...
					 1);
			exception when DUP_VAL_ON_INDEX then
				ibmutl200.logwar(aPgEsec,'Causale coge con PK('||aEs||', '||aCC.cd_causale_coge||') gi? inserita','','');
				stato_fine := 'W';
			end;
		end loop;
 		...
 		for aNaz in (select * from NAZIONE_BLACKLIST
			...
			exception when dup_val_on_index then
				ibmutl200.logwar(aPgEsec,'Nazione inserita nella BLACKLIST con PK('||aEs||', '
													   ||aNaz.CD_CATASTALE||','||aNaz.CD_NAZIONE||') gi? inserita','','');
				stato_fine := 'W';
			end;
 		end loop;
...
begin
	...
	begin
		...
		if isRibaltamentoPDGPEffettuato(aEs, aPgEsec) then
		   stato_fine := 'E';
		   aMessage := 'Lo script di ribaltamento per il PDGP ? gi? stato eseguito con successo per l''esercizio '|| aEs ||': non ? possibile eseguirlo nuovamente.';
		   endLogRibaltamentoPerPDGP(aEs, aPgEsec, stato_fine, aMessage);
		end if;
		if isRibaltamentoAltroEffettuato(aEs, aPgEsec) then
		   stato_fine := 'E';
		   aMessage := 'Lo script di ribaltamento ? gi? stato eseguito con successo per l''esercizio '|| aEs ||': non ? possibile eseguire quello per il PDGP.';
		   endLogRibaltamentoPerPDGP(aEs, aPgEsec, stato_fine, aMessage);
		end if;
		...
		begin
			 ...
			 where exists (select 1 from esercizio_base
			 	   		   where esercizio = aEs);
			 stato_fine := 'E';
			 aMessage := 'Esercizio base '||aEs||' gi? esistente: impossibile ribaltare';
			 endLogRibaltamentoPerPDGP(aEs, aPgEsec, stato_fine, aMessage);
		exception when NO_DATA_FOUND then
			 null;
		end;
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'Esercizio base '||aEs||' gi? esistente','','');
			stato_fine := 'W';
		end;
		...
		begin
			...
			where ESERCIZIO = aEsPrec;
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'Parametri CNR per l''esercizio base '||aEs||' gi? esistenti','','');
			stato_fine := 'W';
		end;
		...
		begin
			...
			values (cnrctb020.getCDCDSENTE(aEs)
				   ...
				   ,0);
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'Esercizio contabile '||aEs||' per l''ente gi? esistente','','');
			stato_fine := 'W';
		end;
		...
		begin
			...
			insert into TIPO_VARIAZIONE (ESERCIZIO,CD_TIPO_VARIAZIONE,DS_TIPO_VARIAZIONE,TI_TIPO_VARIAZIONE,
						     FL_UTILIZZABILE_ENTE,FL_UTILIZZABILE_AREA,FL_UTILIZZABILE_CDS,
  						     ...
  						     FL_VISTO_DIP_SALDO_MODULO,FL_VISTO_DIP_CLASSIFICAZIONE)
			...
			where ESERCIZIO = aEsPrec;
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'TIPO_VARIAZIONE per l''esercizio base '||aEs||' gi? esistente','','');
			stato_fine := 'W';
		end;
		...
		for aConfCNR in (select * from CONFIGURAZIONE_CNR
			...
			exception when DUP_VAL_ON_INDEX then
				ibmutl200.LOGWAR(aPgEsec,'CONFIGURAZIONE_CNR con PK('||aEs||', '
					         ||aConfCNR.CD_UNITA_FUNZIONALE||', '
						 ||aConfCNR.CD_CHIAVE_PRIMARIA||', '
						 ||aConfCNR.CD_CHIAVE_SECONDARIA||') gi? esistente','','');
				stato_fine := 'W';
			end;
		end loop;
...
begin
	...
	for aClassE in (select * from CLASSIFICAZIONE_ENTRATE
		...
		exception
		   when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'CLASSIFICAZIONE_ENTRATE con PK ('||aEsDest||', '||aClassE.CODICE_CLA_E||') gi? esistente','','');
			aStato := 'W';
		   when OTHERS then
	...
	for aClassS in (select * from CLASSIFICAZIONE_SPESE
		...
		exception
		   when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'CLASSIFICAZIONE_SPESE con PK ('||aEsDest||', '||aClassS.CODICE_CLA_S||') gi? esistente','','');
			aStato := 'W';
		   when OTHERS then
	...
	lock table parametri_livelli in exclusive mode nowait;
	   ...
	   insert into PARAMETRI_LIVELLI (ESERCIZIO,LIVELLI_ENTRATA,
		...
		where ESERCIZIO = aEsOrig;
	Exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'Parametri Livelli per l''esercizio base '||aEsDest||' gi? esistenti','','');
			aStato := 'W';
	End;
...
--			 ibmutl200.LOGINF(aPgEsec,'cnrctb001.creaEsplVociEsercizio('||aEsDest||','||aCds.cd_unita_organizzativa||','||cgUtente||')','','');
		...
		exception
		when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'Esercizio contabile '||aEsDest||' per il CDS '||aCds.cd_unita_organizzativa||' gi? definito','','');
			aStato := 'W';
		end;
...
--			 ibmutl200.LOGINF(aPgEsec,'Impostazione percentuali copertura per cds:'||aCds.cd_unita_organizzativa,'','');
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'Percentuali di copertura '||cnrutil.getLabelObbl()||' per il CDS '||aCds.cd_unita_organizzativa|| ' per l''esercizio '|| aEsDest||' gi? impostate','','');
			aStato := 'W';
		end;
...
--		ibmutl200.LOGINF(aPgEsec,'CNRCTB018.AGGIORNANUMERATORI('||aEsDest||','||aCds.cd_unita_organizzativa||','|| cgUtente||')','','');
	...
	for aCds in (select * from v_unita_organizzativa_valida
	   ...
	   Declare
		FL_LINEA_E	varchar2(1);
  	   ...
  	   Begin
  	   	--se la linea non ? valida, ripulisco i campi
		...
		exception
		when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'Parametri CDS per l''esercizio contabile '||aEsDest||' e per il CDS '||aCds.cd_unita_organizzativa||' gi? definiti','','');
			aStato := 'W';
		end;
	   Exception
	      When No_Data_Found Then
	...
	for aCds in (select * from v_unita_organizzativa_valida
	   ...
	   Begin
		...
		exception
		when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'Parametri di Chiusura coep per l''esercizio contabile '||aEsDest||' e per il CDS '||aCds.cd_unita_organizzativa||' gi? definiti','','');
			aStato := 'W';
		end;
	end loop;
	...
	for aAssPgiro in (select * from ASS_PARTITA_GIRO
		...
		when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'ASS_PARTITA_GIRO con PK( '||aEsDest||', '
									||aAssPgiro.TI_APPARTENENZA||', '
									||aAssPgiro.TI_GESTIONE||', '
									||aAssPgiro.CD_VOCE||') gi? esistente','','');
			aStato := 'W';
	...
	for aInterf in (select * from LIQUID_CORI_INTERF_CDS
		...
		begin
			...
			values (aInterf.cd_cds,
					...
					aInterf.tipo);
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'Cds '||aInterf.cd_cds||' gi? esistente per l''esercizio '||aEsDest,'','');
			aStato := 'W';
		end;
	end loop;
			...
			values (aEsDest,
			        aEsterna.cd_unita_organizzativa,
				      aEsterna.tipo,
					    ...
					    1);
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'UO '||aEsterna.cd_unita_organizzativa||' gi? esistente per l''esercizio '||aEsDest,'','');
			aStato := 'W';
		end;
	end loop;
			...
			values (aEsDest,
			        aDati.CD_DIPARTIMENTO,
				      ...
				      aDati.peso,
					    ...
					    1);
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'Dipartimento peso gi? esistente per l''esercizio '||aEsDest,'','');
			aStato := 'W';
		end;
	end loop;
	...
	for aDati In(Select * From  ASS_UO_AREA
		...
		exception
		when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'Associazione UO-AREA per l''esercizio contabile '||aEsDest||' e per la UO '||aDati.cd_unita_organizzativa||' gi? definiti','','');
			aStato := 'W';
		end;
	end loop;
	...
	for aDati In(Select * From  ASS_DIPARTIMENTO_AREA
		...
		exception
		when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'Associazione DIPARTIMENTO_AREA per l''esercizio contabile '||aEsDest||' e per il dipartimento '||aDati.CD_DIPARTIMENTO||' gi? definita','','');
			aStato := 'W';
		end;
	end loop;
end;
...
begin
	...
	for aVoceEP in (select * from VOCE_EP
		...
		begin
			...
			values (aEsDest,
					...
					NULL);
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'Voce E/P '||aVoceEP.CD_VOCE_EP||' gi? esistente per l''esercizio '||aEsDest,'','');
			aStato := 'W';
		end;
	end loop;
	...
	for aAssEVEP in (select * from ASS_EV_VOCEEP
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'ASS_EV_VOCEEP con PK ('||aEsDest||', '
															 ...
															 ||aAssEVEP.TI_GESTIONE||', '
															 ||aAssEVEP.CD_ELEMENTO_VOCE||', '
															 ||aAssEVEP.CD_VOCE_EP||') gi? inserita','','');
			aStato := 'W';
		end;
	end loop;
	...
	lock table parametri_livelli_ep in exclusive mode nowait;
	   ...
	   insert into PARAMETRI_LIVELLI_EP (ESERCIZIO,LIVELLI_ECO,
		...
		where ESERCIZIO = aEsOrig;
	Exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'Parametri Livelli EP per l''esercizio base '||aEsDest||' gi? esistenti','','');
			aStato := 'W';
	End;
	...
	From CLASSIFICAZIONE_VOCI_EP;
	  ...
	  loop
		...
		exception
		   when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'CLASSIFICAZIONE_VOCI_EP con PK ('||new_id||') gi? esistente','','');
			aStato := 'W';
		   when OTHERS then
	...
	for aAssAnagEP in (select * from ASS_ANAG_VOCE_EP
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'ASS_ANAG_VOCE_EP con PK (' ||aEsDest||', '
																 ...
																 ||aAssAnagEP.TI_ENTITA||', '
																 ||aAssAnagEP.ENTE_ALTRO||','
																 ||aAssAnagEP.CD_CLASSIFIC_ANAG||') gi? inserita','','');
			aStato := 'W';
		end;
	end loop;
*/
	...
	for aAssCatVoceEp in (select * from categoria_gruppo_voce_ep
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'CATEGORIA_GRUPPO_VOCE_EP con PK (' ||aAssCatVoceEp.CD_CATEGORIA_GRUPPO||', '
																 ||aEsDest||', '
																 ||aAssCatVoceEp.SEZIONE||') gi? inserita','','');
			aStato := 'W';
		end;
	end loop;
	...
	for aAssContoGruppoEp in (select * from CNR_ASS_CONTO_GRUPPO_EP
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'CNR_ASS_CONTO_GRUPPO_EP con PK (' ||aEsDest||', '
																 ||aAssContoGruppoEp.CD_PIANO_GRUPPI||', '
																 ||aAssContoGruppoEp.CD_GRUPPO_EP||', '
																 ||aAssContoGruppoEp.CD_VOCE_EP||') gi? inserita','','');
			aStato := 'W';
		end;
	end loop;
...
begin
	...
	exception when DUP_VAL_ON_INDEX then
		ibmutl200.LOGWAR(aPgEsec,'LIQUID_CORI con PK('||cnrctb020.getCDCDSENTE(aEsDest)||','||aEsOrig||','||uoEnte.cd_unita_organizzativa||',0) gi? inserita','','');
		aStato := 'W';
	end;
	...
	for aGrCR in (select * from GRUPPO_CR
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'Gruppo CORI '||aGrCR.CD_GRUPPO_CR||' gi? inserito per l''esercizio '||aEsDest,'','');
			aStato := 'W';
		end;
	end loop;
	...
	for	aGrCRDet in (select * from GRUPPO_CR_DET
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'GRUPPO_CR_DET con PK ('||aEsDest||', '
					||aGrCRDet.CD_GRUPPO_CR||', '
					||aGrCRDet.CD_REGIONE||', '
					||aGrCRDet.PG_COMUNE||') gi? inserito','','');
			aStato := 'W';
		end;
	end loop;
	...
	for	aGrCRUO in (select * from GRUPPO_CR_UO
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'GRUPPO_CR_UO con PK ('||aEsDest||', '
					||aGrCRUO.CD_GRUPPO_CR||', '
					||aGrCRUO.CD_UNITA_ORGANIZZATIVA||') gi? inserito','','');
			aStato := 'W';
		end;
	end loop;
	...
	for	aTiCR in (select * from TIPO_CR_BASE
		...
		begin
			 ...
			 values (aEsDest,
				...
				1);
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'Contributo ritenuta '||aTiCR.CD_CONTRIBUTO_RITENUTA||' gi? definito per esercizio '||aEsDest,'','');
			aStato := 'W';
		end;
	end loop;
	...
	For	i In 1..15 Loop
		begin
			 ...
			 values (aEsDest,
				 ...
				 1);
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'Mese '||i||' gi? definito per esercizio '||aEsDest,'','');
			aStato := 'W';
		end;
	End Loop;
	...
	for	aStip in (select * from STIPENDI_COFI_CORI_REG
		...
		begin
			 ...
			 values (aEsDest,
				...
				1);
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'Contributo ritenuta '||aStip.CD_CONTRIBUTO_RITENUTA||' di tipo '||aStip.TI_ENTE_PERCIPIENTE||' gi? definito per esercizio '||aEsDest,'','');
			aStato := 'W';
		end;
	end loop;
End;
	...
	for aAssCoriEV in (select * from ASS_TIPO_CORI_EV
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'ASS_TIPO_CORI_EV con PK ('||aEsDest||', '
					...
					||aAssCoriEV.TI_GESTIONE||', '
					||aAssCoriEV.TI_ENTE_PERCEPIENTE||', '
					||aAssCoriEV.CD_CONTRIBUTO_RITENUTA||') gi? inserita','','');
			aStato := 'W';
		end;
	end loop;
	...
	for aAssCoriEP in (select * from ASS_TIPO_CORI_VOCE_EP
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'ASS_TIPO_CORI_VOCE_EP con PK ('||aEsDest||','
					||aAssCoriEP.CD_CONTRIBUTO_RITENUTA||','
					||aAssCoriEP.TI_ENTE_PERCEPIENTE||','
					||aAssCoriEP.SEZIONE||' gi? inserita','','');
			aStato := 'W';
		end;
	end loop;
...
begin
          ...
          PG_VER_REC)
			 values (id,
               ...
               aNaturaTransazione.DS_NATURA_TRANSAZIONE,
					     ...
					     1);
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'NATURA_TRANSAZIONE con PK ('||id||') gi? inserita','','');
			aStato := 'W';
		end;
	end loop;
	...
	for aModalitaTrasporto in (select * from MODALITA_TRASPORTO
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'MODALITA_TRASPORTO con PK ('||aEsDest||', '
					||aModalitaTrasporto.CD_MODALITA_TRASPORTO||') gi? inserita','','');
			aStato := 'W';
		end;
	end loop;
	...
	for aCondizioneConsegna in (select * from CONDIZIONE_CONSEGNA
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'CONDIZIONE_CONSEGNA con PK ('||aEsDest||', '
					||aCondizioneConsegna.CD_INCOTERM||') gi? inserita','','');
			aStato := 'W';
		end;
	end loop;
	...
	for aNomenclatura in (select * from NOMENCLATURA_COMBINATA
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'NOMENCLATURA_COMBINATA con PK ('||id||') gi? inserita','','');
			aStato := 'W';
		end;
	end loop;
	...
	for aModalitaErogazione in (select * from MODALITA_EROGAZIONE
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'MODALITA_EROGAZIONE con PK ('||aEsDest||', '
					||aModalitaErogazione.CD_MODALITA_EROGAZIONE||') gi? inserita','','');
			aStato := 'W';
		end;
	end loop;
	...
	for aModalitaIncasso in (select * from MODALITA_INCASSO
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'MODALITA_INCASSO con PK ('||aEsDest||', '
					||aModalitaIncasso.CD_MODALITA_INCASSO||') gi? inserita','','');
			aStato := 'W';
		end;
	end loop;
	...
	for aCodiciCPA in (select * from CODICI_CPA
		...
		exception when DUP_VAL_ON_INDEX then
			ibmutl200.LOGWAR(aPgEsec,'CODICI_CPA con PK ('||id||') gi? inserito','','');
			aStato := 'W';
		end;
	end loop;
end;
	...
	for aLimitiSpesa in (select * from LIMITE_SPESA
		  ...
		  exception when DUP_VAL_ON_INDEX then
			  ibmutl200.LOGWAR(aPgEsec,'LIMITE_SPESA con PK('||aEsDest||', '
					...
					||aLimitiSpesa.TI_GESTIONE||', '
					||aLimitiSpesa.CD_ELEMENTO_VOCE||', '
					||aLimitiSpesa.FONTE||') gi? esistente','','');
			  aStato := 'W';
		  end;
