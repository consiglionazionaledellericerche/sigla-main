...
-- Gestita la possibilit? di chiudere definitivamente la COGE di un CDS singolo
...
CREATE OR REPLACE package body CNRCTB206 is
 ...
 begin
	  for aLCds in (select * from v_unita_organizzativa_valida where fl_cds = 'Y' and esercizio = aEs) loop
       -- Se l'esercizio finanziario non ? chiuso per tutti i cds la modalita diventa di prova
       if not CNRCTB008.ISESERCIZIOCHIUSO(aEs,aLCds.cd_unita_organizzativa) then
        IBMUTL200.logErr(pg_exec, 'Non tutti gli esercizio finanziari sono chiusi: la modalit? di esecuzione rimane in prova',null,null);
		return false;
       end if;
      end loop;
      return true;
...
Dbms_Output.PUT_LINE ('others bb');
 ...
 begin
  ...
  -- Controlla che in modalit? definitiva, l'esercizio finanziario sia chiuso per tutti i CDS
...
-- se il costo/ricavo ? totalmente anno successivo
-- storna anche l'imputazione del conto sull'anno successivo
-- per toglierla totalmente dal saldo che chiuder?
...
-- se il costo/ricavo ? a cavallo tra anno corrente e anno successivo
-- storna la quota sull'anno corrente per toglierla dal saldo che chiuder?
...
Savepoint CNRCTB205_SP_002;
  ...
  -- cerca eventuali scritture gia registrate per il documento
  -- se la competenza ? fuori esercizio cerca i ratei parte 1
