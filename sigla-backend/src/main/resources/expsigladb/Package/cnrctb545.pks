...
--==================================================================================================
--
-- CNRCTB545 - package di utilit? per gestione COMPENSI
--
-- Date: 14/07/2006
-- Version: 2.7
--
...
-- Version: 1.4
--
-- Aggiunta alla tabella insiemeCoriTab il campo che memorizza la data di inizio validit? del cori
...
-- L'errore si verifica anche in altri casi quando il compenso ? stato incluso in un conguaglio, la
...
EXCEPTION
   ...
   WHEN no_data_found THEN
        IBMERR001.RAISE_ERR_GENERICO
           ('Non ? stato trovato alcun compenso asociato alla missione U.O. ' || aCdUnitaOrganizzativa ||
...
BEGIN
   -------------------------------------------------------------------------------------------------
   -- Entro per aliquota quando questa ? valorizzata. Generalmente solo IRPEF a scaglioni se
   ...
   IF aAliquota != 0 THEN
      ...
      IF chkEsisteScaglione = 0 Then
         IBMERR001.RAISE_ERR_GENERICO
                  ('L''aliquota presente nei dati Anagrafici del percipiente non ? valida.');
         --aImportoRif:=0;
         --aAliquota:=0;
      END IF;
   ...
   BEGIN
      ...
      LOOP
         ...
         IF aRecTmpVPreScaglione.ti_ente_percipiente = 'E' THEN
            IF isEnte = 'Y' THEN
               IBMERR001.RAISE_ERR_GENERICO
                  ('Trovati pi? di un record riferito all''ente per il codice contributo ritenuta ' ||
         ...
         ELSE
            IF isPercipiente = 'Y' THEN
               IBMERR001.RAISE_ERR_GENERICO
                  ('Trovati pi? di un record riferito all percipiente per il codice contributo ritenuta ' ||
                   aCdContributoRitenuta);
            END IF;
            isPercipiente:='Y';
      ...
      -- Se non ? stato trovato alcun record ? sollevata un'eccezione
...
BEGIN
   ...
   -- Se il calcolo ? per date di detrazioni familiari normalizzo portando da e a rispettivamente a
   -- inizio e fine mese
...
BEGIN
   ...
   -- Abilito il calcolo solo se la matrice date ? piena
...
BEGIN
   ...
   -------------------------------------------------------------------------------------------------
   -- Non deve essere fatto nulla. Si tratta di inserimento e non ? stata chiesta la generazione della fattura.
   ...
   IF isCancella = 'Y' THEN
      ...
      IF aRecFatturaPassiva.protocollo_iva IS NOT NULL THEN
         IBMERR001.RAISE_ERR_GENERICO
            ('Impossibile eliminare una fattura  gi? stampata in modo definitivo sui registri IVA' ||
      ...
      END IF;
         ...
         IBMERR001.RAISE_ERR_GENERICO
            ('Impossibile eliminare una fattura gi? inserita nel registro unico delle fatture - ' ||
             'Fattura UO ' || aRecFatturaPassiva.cd_unita_organizzativa || ' numero ' ||
             aRecFatturaPassiva.esercizio || '/' || aRecFatturaPassiva.pg_fattura_passiva);
      END IF;
...
--    del compenso, aggiungeva un'altra fattura ad un periodo gi? chiuso nel registro.
...
BEGIN
   ...
   IF (aRecRateizzaClassificCoriOut.im_da_rateizzare < 0 OR
      ...
      IF aRecRateizzaClassificCoriOut.im_da_rateizzare < 0 THEN
         IBMERR001.RAISE_ERR_GENERICO
            ('Impossibile attivare il conguaglio in modalit? accantonamento con ' || aDescAddizionale || ' ' ||
