...
-- Il codice errore è seguito da spazio trattino spazio, poi dalla descrizione,
-- che a sua volta è seguita dal punto.
--
-- Date: 03/07/2002
-- Version: 1.4
...
-- Version: 2.6
--
-- Revisione completa per centralizzare i controlli di ammissibilità delle stampe e correzione degli
-- stessi.
--
-- Date: 10/12/2002
-- Version: 2.7
...
-- Fix errore in elaborazione della liquidazione, errato controllo ammissibilità alla stampa.
--
-- Date: 18/12/2002
-- Version: 3.2
--
...
-- Version: 3.7
--
-- Fix errore interno 3185, inserita gestione report vuoto in stampa esigibilità differita
...
-- Verificato funzionamento del client e revisione della modalità di aggiornamento della liquidazione
--
-- Date: 03/02/2003
-- Version: 3.11
--
...
-- Version: 4.0
--
-- Inserito controllo di impossibilità a procedere alla liquidazione definitiva per UO se non vi sono
...
-- Inserito controllo di ammissibilità alla liquidazione ente (tutte le uo devono aver liquidato in
-- modo definitivo
...
-- dal centro. Errore del client che non eseguiva il commit e aggiunti controlli sull'ammissibilità
...
-- di non ammissibilità della liquidazione al centro.
--
-- Date: 27/02/2003
-- Version: 4.6
--
...
-- a 26 euro. Il controllo è stato spostato su CNRCTB270
--
-- Date: 28/02/2003
-- Version: 4.8
--
-- Verifica che la liquidazione di massa salti il calcolo della liquidazione se questa è già stata
...
-- provvisoria.
-- Eliminato il controllo di ammissibilità della liquidazione di massa al fatto che sia lanciata
-- dal cds 999
--
-- Date: 18/03/2003
-- Version: 4.14
...
-- Version: 4.17
--
-- Richiesta CINECA n. 739. Revisione recupero record relativi ad iva ad esigibilità differita, si
-- vogliono includere non solo i record con data esigibilità compresa nell'intervallo di selezione
...
--
-- pre-post-name: Stampa provvisoria richiesta per un intervallo già elaborato in modo definitivo.
-- pre:           Non è possibile la stampa provvisoria di un registro IVA, riepilogativo del centro e UO (tutti i sezionali)
--                e stampa iva differita per periodi già elaborati in definitivo.
--                E' obbligatorio l'uso della funzione di ristampa
-- post:          Viene sollevata eccezione.
--
-- pre-post-name: Stampa definitiva richiesta per un intervallo già elaborato in modo definitivo.
-- pre:           Non è possibile una elaborazione definitiva IVA per un intervallo già elaborato in modo definitivo.
-- post:          Viene sollevata eccezione.
--
...
-- pre:           Non è possibile una elaborazione definitiva IVA per un intervallo temporale per cui non esiste una
...
--                     La gestione per esigibilità differita si applica solo alle fatture attive.
--
...
--                        Si determina se l'elaborazione è richiesta per tutti i sezionali o per uno solo
...
--                                  per una certa unità organizzativa.
--                                  Se l'elaborazione è riferita a riepilogativi per UO o a liquidazione
--                                       Se la tipologia di estrazione è per gruppo commerciale si estraggono i soli
--                                       sezionali di tipo commerciale
--                                       Se la tipologia di estrazione è per gruppo intra_ue si estraggono i soli sezionali
--                                       di tipo istituzionale che sono riferiti a gestione intra_ue
--                                       Se la tipologia di estrazione è per gruppo san marino senza IVA si estraggono i soli
--                                       sezionali di tipo istituzionale riferiti a gestione san marino senza iva
--                                  Se l'elaborazione è riferita a stampa riepilogo iva differita si aggiunge
...
--                        Si memorizzano i sezionali così individuati rilevando se il loro contenuto è pieno o vuoto in
...
--                     7) Decodifica nome unità organizzativa
--
--                AMMETTI ELABORAZIONE. Routine ammettiElaborazione(...).
--                     Si controlla l'ammissibilità della elaborazione richiesta (solo se non si tratta di ristampa).
--                     Elaborazione provvisoria
--                          1) In caso di stampa registri IVA, riepilogativo del centro e UO (tutti i sezionali) e stampa
--                             iva differita si controlla che lo stesso intervallo temporale non sia già stato elaborato
--                             in modo definitivo.
--                             Vedi pre-post-name: Stampa provvisoria richiesta per un intervallo già elaborato in modo definitivo
--                     Elaborazione definitiva
--                          1) Per ogni elaborazione si controlla che lo stesso intervallo temporale non sia già stato
--                             elaborato in modo definitivo.
--                             Vedi pre-post-name: Stampa definitiva richiesta per un intervallo già elaborato in modo definitivo.
...
--                          3) Se è richiesta una stampa registri definitiva
--                             - Se esiste liquidazione definitiva nel periodo corrente o in periodi successivi sollevo
--                               errore. La liquidazione controllata è quella compatibile con il tipo sezionale del registro
...
--                          5) Se è richiesta la stampa definitiva di una liquidazione ENTE si controlla, se non è la prima,
--                             che tutte le liquidazioni UO siano state fatte in modo definitivo.
--                          6) Se è richiesta la stampa definitiva di una liquidazione UO si controlla, se non è la prima,
...
--                     STAMPA REGISTRI
--                          E' prevista la possibilità di stampare un solo sezionale per volta sia in modo provvisorio
--                          che definitivo.
--                          L'elaborazione è applicabile ad ogni tipologia di registro sia commerciale che istituzionale.
...
--                     STAMPA REGISTRI
--                          E' prevista la possibilità di stampare un solo sezionale per volta.
--                          L'elaborazione è applicabile ad ogni tipologia di registro sia commerciale che istituzionale.
--                          Nel caso di elaborazione per registri commerciali si escludono dalla selezione le righe
...
-- Lancio delle delle diverse stampe IVA previa esecuzione dei controlli di ammissibilità
...
-- Controllo ammissibilità ad una elaborazione IVA
...
-- Stampa IVA ad esigibilità differita
...
BEGIN
   ...
   -- 2) Il cds richiedente è presente nella tabella LIQUID_IVA_INTERF_CDS
   -- L'esercizio di riferimento è sempre quello reale, solo la liquidazione è scritta in quello successivo
...
BEGIN
 ...
 OPEN gen_cv FOR
   ...
   -------------------------------------------------------------------------------------------------
   -- Verifico se l'elaborazione è vuota
...
close gen_cv;
   ...
   -- Controllo ammissibilità della elaborazione. La ristampa non è controllata in quanto già filtrata dal client
...
BEGIN
   ...
   IF inTipoReport = 'P' THEN
      ...
      -- Non è ammessa l'elaborazione provvisoria se esiste, nello stesso periodo, l'elaborazione definitiva
      ...
      -- La liquidazione di massa non può essere provvisoria

      IF inTipoStampa = TI_ELAB_LIQUIDAZIONE_MASSA THEN
         IBMERR001.RAISE_ERR_GENERICO
               (aMessaggio1 || aMessaggio2 || ' non è ammessa in forma provvisoria');
      END IF;

      RETURN;

   Else --Elaborazione definitiva
      -- La liquidazione di massa provvisoria non può essere definitiva
      IF inTipoStampa = TI_ELAB_LIQUIDAZIONE_MASSA_PRV THEN
         IBMERR001.RAISE_ERR_GENERICO
               (aMessaggio1 || aMessaggio2 || ' non è ammessa in forma definitiva');
      END IF;
   END IF;
   ...
   IF inTipoStampa = TI_ELAB_REGISTRO_IVA THEN
      ...
      IF aGruppoReportAltro IS NOT NULL THEN
         ...
         IF (flEsistePeriodo = 1 OR
             flEsistePeriodoDopo = 1) THEN
            IBMERR001.RAISE_ERR_GENERICO
               (aMessaggio1 || aMessaggio2 || ' non è ammessa in quanto esistono liquidazioni definitive nel periodo ' ||
         ...
         IF flEsistePeriodoPrecedente = 0 THEN
            ...
            Begin
              ...
              If conta > 0 Then --Non è la prima stampa.....quindi controllo
   ...
   -- Per le liquidazioni non ente e quella di massa si verifica che non sia già stata elaborata ---
   ...
   IF (inTipoStampa IN (TI_ELAB_LIQUIDAZIONE_MASSA, TI_ELAB_LIQUIDAZIONE_MASSA_PRV) OR
      ...
      IF (flEsistePeriodoPrecedente = 0 AND
          (flEsistePeriodoPrima = 1 OR
           flEsistePeriodoDopo = 1)) THEN
         IBMERR001.RAISE_ERR_GENERICO
            (aMessaggio1 || aMessaggio2 || ' non è ammessa in quanto la liquidazione centro, pur non risultando ' ||
             'presente in stato definitivo per il periodo precedente, è definita  in altri periodi contabili');
      END IF;
   ...
   BEGIN
      ...
      IF contaSezionali > 0 THEN
         ...
         LOOP
            ...
            -- Il report, per il sezionale in elaborazione, è vuoto
...
BEGIN
   ...
   LOOP
      ...
      -------------------------------------------------------------------------------------------
      -- Stampa del riepilogo per aliquota (solo in presenza di stampa per più sezionali
            ...
            IF inTipoReport = 'P' THEN
               ...
               BEGIN
                  ...
                  LOOP
                     ...
                     -- L'estrazione è eseguita solo per i sezionali valorizzati
...
BEGIN
   ...
   -- Recupero dati del sezionale è sempre elaborato un unico sezionale per volta ------------------
   ...
   LOOP
      ...
      ---------------------------------------------------------------------------------------------
      -- Stampa del registro riepilogativo a  rottura di unità organizzativa
...
BEGIN
   ...
   -- Nella stampa del riepilogo iva ad esigibilità differita per riepilogo si esegue anche il passo
   ...
   Else   --sembra l'unica gestita
         lda_i:=1;
          IF inTipoRegistro = 'A' Then
          -- Aggiunta sezione fatture divenute esigibile perchè trascorso 1 anno dalla data emissione fornitore
                la_i:=6;
          Else
                la_i:=5;
          End If;
   ...
   BEGIN
      ...
      LOOP
         ...
         Loop
            ...
            -- Per la liquidazione provvisoria massiva il calcolo è solo per il commerciale
...
BEGIN
   ...
   LOOP
      ...
      ----------------------------------------------------------------------------------------------
      -- Aggiornamento fatture a esigibilità differita su tabella REPORT_DETTAGLIO
...
--             WHERE  A.CD_CHIAVE_SECONDARIA = 'DESCR_CODICE_IVA_PROMISCUO_ISTITUZIONALE';
         ...
         -- Decodifica unità organizzativa
