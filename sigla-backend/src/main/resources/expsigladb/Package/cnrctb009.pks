...
--
-- La procedura verifica se l'esercizio contabile del cds pu? essere
...
--
-- pre-post-name: Controlli di chiudibilit?
-- pre: una delle seguenti precondizioni non ? verificata
--      Se il cds ? ente e non risulta che gli esercizi di tutti i suoi cds siano chiusi
--      allora viene sollevata un eccezione
--
--      Controlliamo se tutti i mandati del cds in esame risultano riscontrati
--      un mandato risulta riscontrato se ad esso ? legato un sospeso, ma ci?
...
--      una reversale risulta riscontrata se ad essa ? legato un sospeso, ma ci?
...
--      per le unit? organizzative valide.
...
begin
	 ...
	 -- perch? per tutte le scadenze pe cui IM_ASSOCIATO_DOC_CONTABILE = IM_SCADENZA
...
-- =================================================================================================
-- Conta le unit? organizzative per un dato CdS valide in un esercizio con o senza riferimento al
-- livello
-- =================================================================================================
FUNCTION contaUO
   (aCds unita_organizzativa%ROWTYPE,
...
begin
   ...
   if not (aUOVALIDA.fl_cds='Y') then
    IBMERR001.RAISE_ERR_GENERICO('L''uo specificata non ? un CDS '||aCds);
   end if;
   ...
   -- Se il cds non ? pi? valido nell'esercizio successivo esce senza eseguire ulteriori operazioni
...
begin
	 ...
	 -- Se il cds non ? pi? valido nell'esercizio successivo esce senza eseguire operazioni
	 begin
	  select * into aVUOVALIDA from v_unita_organizzativa_valida where
	             esercizio = aEs+1
			 and cd_unita_organizzativa=aCds;
