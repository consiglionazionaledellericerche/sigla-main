...
-- L'estrazione delle scritture prime ? fatto con l'esercizio del doc amministrativo
...
-- che la riga (proveniente da fattura) sia di sconto abbuono: ? il metodo di costruzione del movimento che trovando
...
-- Date: 26/08/2004
-- Version: 1.25
-- Riordinamento funzioni di utilit?
--
-- Date: 24/09/2004
-- Version: 1.26
...
-- Trova il conto associato all'entit? anagrafica specificata utilizzando anche il tipo di documento
...
-- Ritorna true se la coge ? da differire
 function checkIsCogeDifferita(aDocTst V_DOC_AMM_COGE_TSTA%rowtype) return boolean;

-- Ritorna true se l'esercizio ? quello di partenza
...
-- Verifica se la data specificata (fine periodo di competenza) ? fuori esercizio o in esercizio
-- Soleva eccezioni nel caso in cui la data di fine validit? sia in esercizio precedente ed il corrente non ? il primo esercizio in assoluto (2003)
-- Soleva eccezioni nel caso in cui la data di fine validit? sia in esercizi precedenti al precedente
...
-- e l'anticipo ? maggiore del netto percipiente
...
-- Post:
--      Se l'esercizio finanziario non ? aperto o chiuso solleva eccezione
--      Se l'esercizio economico ? chiuso definitivamente o in fase di chiusura definitiva solleva eccezione
...
--  aStringa:='P/U: U';
 ...
 begin
  ...
  if aEsercizioComp < aDocTst.esercizio - 1 then
   IBMERR001.RAISE_ERR_GENERICO('La competenza economica (data fine periodo) del documento ? errata:'||aDocTst.dt_a_competenza_coge||' per esercizio:'||aDocTst.esercizio);
  end if;
...
End;
 ...
 Dbms_Output.put_line ('inizio ricerca anticipo da '||aCdTipoDoc);
  ...
  if aCdTipoDoc = CNRCTB100.TI_COMPENSO then
   ...
   -- Se il compenso non ? associato a missione
  ...
  begin
   -- Se la missione non ? associata ad anticipo
...
Else
   IBMERR001.RAISE_ERR_GENERICO('Si sta cercando di determinare il Mandato collegato ad una Reversale ma il documento contabile non ? una Reversale.');
End If;
...
--Dbms_Output.PUT_LINE ('ENTRO FUNZIONE '||INES||' '||INGEST||' '||INAPP||' '||INELVOCE);
   If INES Is Null Or INGEST Is Null Or INAPP Is Null Or INELVOCE  Is Null Then
     IBMERR001.RAISE_ERR_GENERICO('Impossibile determinare se l''elemento voce ? associato a categoria, i dati forniti sono incompleti.');
   End If;
...
--Dbms_Output.PUT_LINE ('RITORNO FALSE');
 ...
 begin
  ...
  -- Modifica del 25/08/2003 -> se la competenza ? in esercizio precedente e l'esercizio attuale ? il primo dell'applicazione
  -- Il conto di costo/ricavo ? SP Iniziale o Fatture da emettere
...
Dbms_Output.PUT_LINE ('ENTRA IN trovaContoEp CORI');
 ...
 -- Il conto di costo/ricavo ? SP Iniziale
	...
	--  30/10/2002 Se il documento ? un compenso o fattura con IVA su partita di giro il conto da utilizzare ? quello del terzo diversi contropatita (debito)
	...
	if aDocTst.cd_tipo_documento = CNRCTB100.TI_COMPENSO then
     ...
     begin
	  ...
	  -- Se la tipologia di contributo ritenuta ? IVA o RIVALSA il conto ritornato ? quello di costo principale del compenso
...
--  1. se la voce finanziaria ? patrimoniale movimento sempre il conto di attivo ad essa legato
--  2. altrimenti, se la competenza economica del costo/ricavo della fattura a cui ? legata
--     cade in un esercizio precedente chiuso allora anzich? movimentare il costo/ricavo in rettifica deve movimentare
--     le insussistenze passive/attive

If aDocTst.TI_FATTURA = 'C' Then -- se ? una nota
...
------ nel caso CERCO IL CONTO ASSOCIATO ALLA VOCE (E LO PASSO ALLA SCRITTURA AL POSTO DI UTILE O PERDITA SU CAMBIO)
   ...
   If isCompFattEsPreChiusoADTREG (aDocTst, aDoc) Then  -- e la fattura a cui ? legata ha competenza in un esercizio chiuso
...
begin
  ...
  if aTipoDoc is not null then
   ...
   if CNRCTB100.GETSEZIONEECONOMICA(aTipoDoc) = CNRCTB200.IS_AVERE then
		 aTipoTerzo:='D'; -- Documenti attivi il terzo ? un debitore
	 else
     aTipoTerzo:='C'; -- Documenti passivi il terzo ? un creditore
   end if;
  else
...
--   end if;
   ...
   aScrittura.DS_SCRITTURA := 'Contabilizzazione '||aDocTst.CD_TIPO_DOCUMENTO||': '||aDocTst.CD_CDS||'/'||
                              ...
                              aDocTst.PG_NUMERO_DOCUMENTO;    --null; -- chi lo valorizza e come non ? chiaro
...
-- 09.08.2006 data della scrittura = data emissione mandato/reversale
   ...
   aScrittura.DS_SCRITTURA := 'Contabilizzazione '||aDocTst.CD_TIPO_DOCUMENTO_CONT||': '||aDocTst.CD_CDS||'/'||
                              aDocTst.CD_UNITA_ORGANIZZATIVA||'/'||aDocTst.ESERCIZIO||'/'||
                              aDocTst.PG_DOCUMENTO_CONT;    --null; -- chi lo valorizza e come non ? chiaro
   aScrittura.PG_SCRITTURA_ANNULLATA:=null; -- codice della scrittura annullata in effettuazione storno
   aScrittura.ATTIVA:='Y';
...
End;
 ...
 begin
  if not CNRCTB008.ISESERCIZIOAPERTOOCHIUSO(aEs, aCdCds) then
    IBMERR001.RAISE_ERR_GENERICO('L''esercizio ('||aEs||') non ? ancora aperto per il cds :'||aCdCds);
  end if;
  if (CNRCTB200.ISCHIUSURACOEPDEF(aEs, aCdCds)='Y') then
   IBMERR001.RAISE_ERR_GENERICO('L''esercizio economico ? chiuso definitivamente o in fase di chiusura per il cds: '||aCdCds);
  end if;
 end;
