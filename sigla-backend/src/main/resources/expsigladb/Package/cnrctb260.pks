...
-- Protocollo a -1 indica sezionale non definito o non compatibile
   ...
   -- Il lock sui sezionali e fatture ? fatto solo per registro definitivo
...
BEGIN
   ...
   LOOP
      ...
      ELSIF i = 6 THEN
            ...
            CLOSE gen_cv;
      ELSIF i = 7 Then
      -- RP Gestito solo per gli Acquisti Fatture divenute esigibile perch? trascorso 1 anno
            If aTipoRegistro = 'A' THEN
...
--                EXCEPTION
--                     WHEN DUP_VAL_ON_INDEX THEN
--                           Ibmerr001.RAISE_ERR_GENERICO ('Riporto dati dichiarazione di intento, gi? stampati.' );
--               END;
--                END LOOP;
--                -- close cursore
--                CLOSE gen_cv;
--             END;
--          -- se ? una ristampa considera solo quelli gi? aggiornati alla data fine periodo
--          ELSE
--             BEGIN
--               OPEN gen_cv FOR
--                     SELECT LTRIM(RTRIM(cognome || ' ' || nome || ragione_sociale)),
...
--                EXCEPTION
--                     WHEN DUP_VAL_ON_INDEX THEN
--                           Ibmerr001.RAISE_ERR_GENERICO ('Dichiarazione di intento, gi? stampata' );
--                END;
--                END LOOP;
--                -- close cursore
--                CLOSE gen_cv;
...
--
--             WHEN DUP_VAL_ON_INDEX THEN
--                  Ibmerr001.RAISE_ERR_GENERICO ('Revoca dichiarazione di intento, gi? stampata ');
--          END;
--
--       END LOOP;
--
...
Begin
   ...
   -------------------------------------------------------------------------------------------------
   -- Ciclo principale di lettura dei dati da calcolare per la liquidazione (no esigibilit? differita)
...
--                  END IF;
   ...
   -------------------------------------------------------------------------------------------------
   -- Ciclo principale di lettura dei dati da calcolare per la liquidazione (si esigibilit? differita)
   -- Si prelevano tutti i record con data esigibilit? IVA inferiore alla data di fine, la vista
...
-- =================================================================================================
-- Inserimento nella tabella REPORT DETTAGLIO del dettaglio fatture con esigibilit? differita che
...
BEGIN
   ...
   -------------------------------------------------------------------------------------------------
   -- Lettura fatture e scrittura in REPORT DETTAGLIO dell'elenco fatture ad esigibilit? differita
   ...
   -- La gestione ? attualmente attiva solo per registri IVA vendite
...
--                  data_esigibilita >= aDataInizio AND
      ...
      LOOP
         ...
         EXCEPTION
            ...
            WHEN DUP_VAL_ON_INDEX THEN
                 IBMERR001.RAISE_ERR_GENERICO
                    ('Chiave duplicata in inserimento estremi fattura ad esigibilit? differita in REPORT_DETTAGLIO '  ||
