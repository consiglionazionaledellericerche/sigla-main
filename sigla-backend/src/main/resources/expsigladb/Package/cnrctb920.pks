...
-- l'Estrazione ? fatta per U.O. ad eccezione della SAC dove si elabora l'intero CdS.
--
-- >>>>>>>>>> Date: 23/03/2004
-- >>>>>>>>>> Version: 1.4
--
...
-- |inCdUo                    |Identificativo dell'unit? organizzativa di riferimento              |
...
-- +----------------------------+----------------------------------------------------+------------+
-- |parametri_tab( 1).stringa   |Descrizione unit? organizzativa o Cds se si tratta  |     SI     |
-- |                            |di SAC                                              |            |
...
-- L'estrazione ? fatta per singola unit? organizzativa. Nel caso in cui tale UO sia di tipo SAC sono elaborate tutte
-- le unit? organizzative del Cds SAC. Sono presi in considerazione i compensi liquidati ad altri creditori nell'esercizio
...
--
--                      Se la UO ? di tipo non SAC la descrizione ? quella della UO in input.
--                      Se la UO ? di tipo SAC la descrizione ? quella del Cds SAC.
--
...
--
--                              Se la UO ? di tipo non SAC si elabora la singola unit? organizzativa.
--                              Se la UO ? di tipo SAC si elabora l'intero Cds SAC.
--
...
--
--                                 Se i giorni della data di cui sopra sono > 15 il mese di riferimento ? pari allo stesso mese.
--                                 altrimenti il mese di riferimento ? quello precedente.
...
BEGIN
   ...
   LOOP
      ...
      BEGIN
         ...
         -- Memorizzazione della descrizione dell'unit? organizzativa
...
BEGIN
   ...
   BEGIN
      ...
      LOOP
         ...
         -- Se il compenso letto ? diverso dal precedente o se il progressivo di riga ? diverso dal
         ...
         IF aRecVEstraiDatiInps.ti_ente_percipiente = memTiCoriEPPrec THEN
            IBMERR001.RAISE_ERR_GENERICO
               ('Errore in sequenza record. Un CORI ente non ? precedute da uno percipiente o viceversa ' || CHR(10) ||
	    ...
	    --            solo se ammontare ? > 0 e imponibile > 5000
	    --Nel caso 1) bisogna prendere l'imponibile cos? come sta
	    Declare
	    ...
	    Begin
	       ...
	       Where c.cd_cds = aRecVEstraiDatiInps.cd_cds_compenso
	         ...
	         Else
	            ...
	            If Nvl(im_netto_perc,0) < 0 Then --? un conguaglio
...
BEGIN
   ...
   BEGIN
      ...
      LOOP
         ...
         -- Correggo il mese mantenendolo pari a 1 se la data di riferimento ? inferiore al 16/01/inEsercizio.
         -- Correggo il mese mantenendolo pari a 12 se la data di riferimento ? maggiore del 31/12/inEsercizio.
...
BEGIN
   ...
   BEGIN
      ...
      LOOP
         ...
         -- E' la prima volta o il soggetto anagrafico letto ? diverso dal precedente --------------
