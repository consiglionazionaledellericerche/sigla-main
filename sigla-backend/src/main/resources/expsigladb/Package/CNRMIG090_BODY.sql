--------------------------------------------------------
--  DDL for Package Body CNRMIG090
--------------------------------------------------------
 ...
 lContaAnag number;
 begin
 	  -- La tabella CNR_ANAGRAFICO in questa fase deve essere già caricata con i dati
	  ...
	  for aAnag in (select * from cnr_anagrafico order by id_anagrafico, timestamp) loop
	  	  begin
			   ...
			   -- Passo 1
			   -- Controlliamo che il dipendente in esame non risulti già
			   ...
			   if chkPresenzaAnag(aAnag, lAnagraficoOld) then
			   	  -- errore
			  	  ibmutl200.logInf(gPgLog ,aAnag.id_anagrafico || ', già presente, come cd_anagrafico ' ||lAnagraficoOld.cd_anag , '','');
			   else
 ...
 begin
	  ...
	  if lNumPagamenti > 0 then
		  ...
		  loop
		  	  ...
		  	  lModValida := true;
			  -- Controlliamo se la modalita di pagamento è valida
			  begin
				   select *
				   into lRifPagamento
	  ...
	  if lContBanche > 0 then
		  ...
		  loop
			  ...
			  if lBancaSci.Ti_pagamento ='B' then
				  -- Controlliamo se la banca è valida
			 	  select count(*)
				  into lNumBanche
				  from abicab
			  ...
			  if lBancaValida then
				 ...
				 if not lBancaGiaInserita then
			  	...
			  	ibmutl200.logInf(gPgLog ,lBancaSci.id_anagrafico || ', La banca con abi' || lBancaSci.abi || ' e cab ' || lBancaSci.CAB ||' non è valida ', '', '');
			  end if;
	 	  end loop;
	  else
