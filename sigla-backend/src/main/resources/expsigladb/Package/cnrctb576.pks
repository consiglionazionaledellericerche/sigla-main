 ...
 Begin
   ...
   Loop
       	  ...
       	  If aStato != CNRCTB575.STATO_GRUPPO_CENTRO_INIZIALE Then
       	     IBMERR001.RAISE_ERR_GENERICO('Nell''Esercizio '||To_Char(aEs)||' il gruppo '||aDGruppoCentro.cd_gruppo_cr||'/'||aDGruppoCentro.cd_regione||'/'||aDGruppoCentro.pg_comune||' non ? in stato iniziale.');
       	  End If;
       	  If aDaEsPrec != 'N' Then
       	     IBMERR001.RAISE_ERR_GENERICO('Il gruppo '||aDGruppoCentro.cd_gruppo_cr||'/'||aDGruppoCentro.cd_regione||'/'||aDGruppoCentro.pg_comune||' non pu? essere ribaltato.');
       	  End If;
          -- Se gi? esiste nel nuovo anno una riga per lo stesso gruppo con STATO = 'I'
	    ...
	    If contatore > 0 Then
	       	IBMERR001.RAISE_ERR_GENERICO('Esiste gi? nell''Esercizio '||To_Char(aEs+1)||' il gruppo '||aDGruppoCentro.cd_gruppo_cr||'/'||aDGruppoCentro.cd_regione||'/'||aDGruppoCentro.pg_comune||' con stato Iniziale.');
	    Else
	    	--Ribalto la PGIRO di spesa di liquid_gruppo_centro nel nuovo esercizio
	 	If aDGruppoCentro.pg_obb_accentr Is Not Null Then
	 	   --verifico se la PGIRO ? stata gi? ribaltata
	 	   Select Count(1)
	 	   Into conta_rib
	 	   From obbligazione
			...
			--PRENDO L'ACCERTAMENTO COLLEGATO ALL'IMPEGNO
	    	   ...
	    	   Else     --conta_rib != 0 e quindi non posso ribaltare
	    	   	IBMERR001.RAISE_ERR_GENERICO('La Partita di giro '||To_Char(aDGruppoCentro.esercizio_obb_accentr)||'/'||To_Char(aDGruppoCentro.esercizio_ori_obb_accentr)||'/'||To_Char(aDGruppoCentro.pg_obb_accentr)||' risulta gi? ribaltata.');
	    	   End If;
