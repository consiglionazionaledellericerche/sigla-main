 ...
 begin
	...
	begin
        ...
        If aComp.FL_COMPENSO_MINICARRIERA='Y' And aComp.FL_COMPENSO_MCARRIERA_TASSEP = 'Y' Then
           --per la tassazione separata non distinguiamo anno corrente e anno precedente (anche perch? per il momento non esistono questi casi)
           aCnrEstr.TI_PAGAMENTO:=4;
           aCnrEstr.ESERCIZIO_COMPENSO:=To_Char(DT_FI_COMP,'YYYY');
        Else
           --se la competenza del compenso ? di anni precedenti
           ...
           Else
              --anche se la competenza ? a cavallo tra anno precedente e anno corrente, la considero Anno Corrente
	...
	/*Eliminato perch? gestite le date di competenza 23/11/2006*/
	/*
...
--              CODICE_FISCALE,
 ...
 begin
  begin
   ...
   IBMERR001.RAISE_ERR_GENERICO('Estrazione gi? effettuata per liquidazione n.'||aLiquid.pg_liquidazione||' es.'||aLiquid.esercizio||' cds:'||aLiquid.cd_cds||' uo:'||aLiquid.cd_unita_organizzativa);
  exception when NO_DATA_FOUND then
   null;
  end;
   ...
   begin
      ...
      Where esercizio = aLiquidFitt.esercizio
        ...
        And pg_liquidazione = aLiquidFitt.pg_liquidazione;
            IBMERR001.RAISE_ERR_GENERICO('Estrazione gi? effettuata per compensi non liquidati es.'||aLiquidFitt.esercizio);
   exception when NO_DATA_FOUND then
       null;
   end;
