...
-- Date: 04/07/2002
-- Version: 1.1
-- Completata la gestione della parte relativa alla linea di attivit?  e cdr in obbligazione
--
-- Date: 04/07/2002
-- Version: 1.2
...
-- Date: 27/02/2003
-- Version: 3.4
-- Gestione chiusura LIQUIDAZIONE_IVA_CENTRO anche quando il debito IVA dell'ente per periodo ?¨ <= 0
--
-- Date: 28/02/2003
-- Version: 3.5
-- Non annullabilit?  della liquidazione IVA locale e del centro
--
-- Date: 07/03/2003
-- Version: 3.6
...
-- pre: Nella liquidazione in processo l'importo iva_da_versare ?¨ non valorizzato
-- post: viene sollevata un'eccezione
--
...
-- post: viene sollevata un'eccezione
--
-- pre-post-name: Estrazione senza successo della linea di attivit?  COMUNE per versamento IVA specificata come codice in configurazione CNR
...
-- pre: Nessuna delle precodizioni precedenti ?¨ verificata
-- post:
--  1.  Caso di unit?  organizzativa non liquidata via interfaccia, liquidazione propria
--  1.1  Viene creata un'obbligazione, generico e mandato di versamento all'UO di versamento IVA
--        Se la liquidazione ?¨ di tipo commerciale -> viene creata un'obbligazione su partite normali, generico di trasferimento e mandato di versamento
--        Se la liquidazione ?¨ di tipo intraue o Sna MArino senz'iva -> viene creata un'obbligazione su partita di giro tronca, generico di versamento cori e mandato di versamento
...
--        Il riferimento a tale pratica ?¨ mantenuto nella tabella LIQUIDAZIONE_IVA_CENTRO dove per ogni mese e tipo di liquidazione esiste un record
...
--  2.  Caso di unit?  organizzativa  liquidata via interfaccia, liquidazione propria
...
--        La gestione del riferimento alla pratica di accantonamento al centro ?¨ lo stesso del caso 1.2
...
--        numero della liquidazione effettuata al centro che chiude la partita finanziaria IVA: lo stato di tale riferimento ?¨ posto a chiuso?©
--        Se tale riferimento non ?¨ trovato, il processo continua senza interruzioni di sorta (nessuna UO risultava a debito IVA nel periodo per
...
-- Controlla l'assunzione di Impegni, se ?¨ presente una riga si Saldi non quadrata
-- sulla linea dell'IVA non ?¨ possibile assumere obbligazioni
...
CREATE OR REPLACE PACKAGE BODY CNRCTB270 AS
 ...
 Begin
   -- Estrazione della linea di attivit??  comune da utilizzare per la creazione della variazione
 ...
 Exception
   When NO_DATA_FOUND Then
     IBMERR001.RAISE_ERR_GENERICO('Linea di attivit??  comune per '||cnrutil.getLabelObbligazioneMin()||' versamento IVA non trovata ('||
aCDRUO.cd_centro_responsabilita||'/'||codLASpecVersIva||') per l''esercizio '||to_char(aEs));
 ...
 Exception
   When NO_DATA_FOUND Then
     IBMERR001.RAISE_ERR_GENERICO('Linea di attivit??  per '||cnrutil.getLabelObbligazioneMin()||' versamento IVA non trovata ('||
aCDRUOVERSIVA.cd_centro_responsabilita||'/'||codLASpecVersIvaSAC||') per l''esercizio '||to_char(aEs));
 ...
 Begin
   ...
   -- Estrazione della linea di attivit??  SAC da utilizzare per la creazione della variazione
         ...
         Begin
           ...
           For recGAE in (select a.cd_linea_attivita, nvl(sum(a.im_variazione), 0) im_variazione
             ...
             Begin
               ...
               --Effettuo il controllo disponibilit??  per le GAE diverse dalla C0000020
               If currentGAE != aLASpecVersIva.cd_linea_attivita Then
                 --Leggo la disponibilit??  residua
 ...
 Begin
  ...
  Begin
    ...
    -- Se lo stato di liquidazione del centro ??¨ gi??  chiuso solleva eccezione
    If aLIC.stato = STATO_LIQUID_CENTRO_CHIUSO then
      IBMERR001.RAISE_ERR_GENERICO('Liquidazione finanziaria al centro gi??  effettuata. '||getDescLiquidazione(aliq));
    End if;
  ...
  Exception when NO_DATA_FOUND then
    ...
    -- La pratica al centro non esiste ancora, devo definirla solo se non ??¨ attiva la tesoreria unica
    If aTesoreriaUnica='N' Then
       ...
       -- La competenza economica ??¨ il periodo della liquidazione
 ...
 begin
  ...
  Begin
    ...
    -- Estraggo il CDR da utilizzare per estrarre la linea di attivit??  per obbligazione di versamento  (centro e periferia)
    ...
    If not isIvaInterfaccia then
      ...
      -- Estrazione della linea di attivit??  comune da utilizzare per la creazione dell'obbligazione di versamento
      Begin
      ...
      If aLiquidIva.iva_da_versare <  (-1)*26 Then
        ...
        Begin
          ...
          If aGruppoCoriDet.pg_banca Is null Or aGruppoCoriDet.cd_modalita_pagamento Is Null Or aGruppoCoriDet.cd_terzo_versamento Is Null Then
            IBMERR001.RAISE_ERR_GENERICO('Terzo o sue modalit??  di pagamento non specificate per GRUPPO CORI IVA');
          End If;
          ...
          If Nvl(recParametriCNR.fl_tesoreria_unica,'N') = 'Y' And aLiquidIva.tipo_liquidazione = CNRCTB250.TI_LIQ_IVA_COMMERC Then
            ...
            Begin
              ...
              If impLiqResiduo>0 Then
                IBMERR001.RAISE_ERR_GENERICO('Disponibilit??  del CDR '||aCDRUOVERSIVA.CD_CENTRO_RESPONSABILITA||' sulla GAE '||
     ...
     Where ESERCIZIO = aEs
       ...
       If Esiste > 0 Then
        ...
        Exception
         ...
         IBMERR001.RAISE_ERR_GENERICO('Registrazione non possibile.\nEsistono pi??¹ impegni che gravano ancora sulla G.A.E. dell''IVA.');
        End;
       End If;
   End If;
 End;
       ...
       If Esiste > 0 Then
        ...
        Exception
          When No_Data_Found Then
            Null;
          When Too_Many_Rows Then
         IBMERR001.RAISE_ERR_GENERICO('Registrazione non possibile.\nEsistono pi??¹ impegni residui che gravano ancora sulla G.A.E. dell''IVA.');
        End;
       End If;
   End If;
 End;
