...
-- Date: 16/10/2001
-- Version: 1.4
-- Gestione del tipo di unit? organizzativa ENTE
--
-- Date: 21/10/2001
-- Version: 1.5
...
-- 2.0.2 Gestione di TI_GESTIONE in creazione linea di attivit? AUTOMATICA
--
-- Date: 28/02/2002
-- Version: 2.3
...
-- A livello di quadratura Entrate figurative/Spese, viene utilizzato l'insieme automatico e le modalit? rimangono quelle
-- precedenti.
--
-- Date: 17/06/2002
-- Version: 2.8
...
-- pre: Il CDR specificato non ? di primo livello
...
-- post: Viene sollevata un'eccezione
--
-- pre-post-name: Verifica che il PDG del CDR non sia gi? stato ribaltato sull'area
-- pre: Il PDG del cdr ? gi? stato ribaltato sull'area (fl_ribaltato_su_area = 'Y')
-- post: Viene sollevata un'eccezione
--
-- pre-post-name: Verifica che il Bilancio finanziario dell'ENTE sia stato approvato
-- pre: Il bilancio finanziario dell'ente NON ? stato ancora approvato
-- post: Viene sollevata un'eccezione
--
-- pre-post-name: Verifica che il pdg del cdr specificato sia in stato di chiusura definitiva (F)
-- pre: Il pdg del CDR specificato non ? in stato di chiusura definitiva
-- post: Viene sollevata un'eccezione
--
-- pre-post-name: Ribaltamento su area
-- pre: Nessuna delle condizioni precedenti ? verificata
-- post:
-- Vengono estratti tutti i PDG della linea del CDR di primo livello
-- Per ogni PDG di CDR "CDRLINEA" afferente ad AREA tramite unit? organizzativa di afferenza:
--     Estraggo il conto di ricavo figurativo collegato
--     Ciclo sui dettagli del PDG con natura 5 di "CDRLINEA"
--       Viene letta la linea di attivit? del dettaglio del centro servito
--       Ricerca o creazione della nuova linea di attivit? nel servente AREA
--      Viene slezionata la linea di attivit? di entrata CSSAC nel CDR AREA
--          Se tale linea non viene trovata, viene creata nel CDR AREA
--          Viene automaticamente creato un insieme di linee di attivit? per legare il ricavo figurativo alla spalmatura in spesa di tale ricavo in AREA
...
--   Viene cercata nel sevrvente la linea di attivit? collegata a quella del servito.
--        Se tale linea di attivit? non viene trovata, viene creata automaticamente ereditando funz./natura da quella originale e viene legata
--        a quella CSSAC del ricavo figurativo attraverso l'insieme precedentemente creato.
--   Viene creato nell'AREA un dettaglio di spesa simmetrico a quello del CDR che ribalta che utilizza la linea di attivit? collegata a quella del servito.
--   Se tale linea di attivit? gi? esiste viene utilizzata altrimenti creata contestualmente.
...
-- Se almeno un CDR appartenente alla linea del CDR di primo livello ? stato ribaltato su AREA viene aggiornato a Y il flag fl_ribaltato_su_area del piano di gestione del CDR di primo livello.
--
--
--
-- Parametri:
...
begin
    -- Leggo la linea di attivit? del dettaglio del centro servito
  ...
  begin
       ...
       IBMERR001.RAISE_ERR_GENERICO('Linea di attivit? del dettaglio del centro servito non trovata!');
         end;
  ...
  begin
     ...
     -- Per prima cosa ? necessario creare la linea di attivit? collegata a quella di partenza (spesa servito)
     begin
      -- Devo selezionare la linea di spesa collegata
      -- Si tratta di una linea di attivit? di spesa propria dell'AREA
...
--         aDestS.PG_ENTRATA_CLGE:=aDett.PG_ENTRATA_CLGE;
  ...
  -- Mantengo il collegamento con il dettaglio verso altra UO esistente eventualmente nel servito
     ...
     -- Azzero il collegamento con il centro servito - ? stato scaricato sull'area
...
Begin
    -- Leggo la linea di attivit? del dettaglio del CDR Assegnatario che deve scaricare sull'Area

-- L'nvl c'? perch? vedendo se ? pieno il record del gestionale o delle variazioni capisco di cosa si tratta,
-- se dello scarico del PdG o dei dettagli di variazione
  ...
  begin
      ...
      exception
        when NO_DATA_FOUND then
       IBMERR001.RAISE_ERR_GENERICO('Linea di attivit? di entrata del dettaglio del CdR che scarica su Area non trovata!');
        End;

  -- Ricerca o creazione della nuova linea di attivit? dell'AREA creata dalla Linea
...
If aDett.esercizio Is Not Null Then     -- scarico del Piano di Gestione gestionale, non per variazione
     ...
     -- Inserisco la riga per L'Area (scaricata)
         ...
         aDestE.ORIGINE                          := aDett.ORIGINE;             -- cio? DIR
         aDestE.CATEGORIA_DETTAGLIO              := aDett.CATEGORIA_DETTAGLIO; -- cio? PRE
         aDestE.FL_SOLA_LETTURA                  := 'Y';
...
-- se dello scarico del PdG o dei dettagli di variazione
  ...
  begin
      ...
      exception
        when NO_DATA_FOUND then
       IBMERR001.RAISE_ERR_GENERICO('Linea di attivit? di spesa del dettaglio del CdR che scarica su Area non trovata!');
        End;
  ...
  begin
        -- verifico se per l'esercizio ? necessario gestire il cofog
...
If aDett.esercizio Is Not Null Then -- scarico del Piano di Gestione gestionale, non per variazione
     ...
     -- Inserisco la riga per L'Area (scaricata)
         ...
         aDestS.DESCRIZIONE                   := aDett.DESCRIZIONE;
         aDestS.ORIGINE                       := aDett.ORIGINE;              -- cio? DIR
         aDestS.CATEGORIA_DETTAGLIO           := aDett.CATEGORIA_DETTAGLIO;  -- cio? PRE
...
-- RECUPERO LA CLASSIFICAZIONE O DAL DETTAGLIO DEL PDGG O DALLA VOCE (PER LE VARIAZIONI)
    ...
    -- Leggo la linea di attivit? creata dal CdR Assegnatario (che scarica su Accentratore)
    ...
    Exception
        when NO_DATA_FOUND then
         IBMERR001.RAISE_ERR_GENERICO('Linea di attivit? del dettaglio del PdG gestionale ('||
          ...
          Exception
            When No_Data_Found Then
              If aCV.CDR_ACCENTRATORE = aCDRPersonale.CD_CENTRO_RESPONSABILITA Then
                IBMERR001.RAISE_ERR_GENERICO('Linea di attivit? per il CDR accentratore '||aCV.CDR_ACCENTRATORE||' con Natura '||aLaTipoNatura||
                                             ', Programma '||aLa_origine.cd_programma||' e Missione '||aLa_origine.cd_missione||' non presente! Operazione non possibile!');
              Else
                Raise No_Data_Found;
              End If;
            When Too_Many_Rows Then
              IBMERR001.RAISE_ERR_GENERICO('Esiste pi? di una linea di attivit? per il CDR accentratore '||aCV.CDR_ACCENTRATORE||' con Natura '||aLaTipoNatura||
...
-- vedo se esiste gi? la riga dell'accentratore
         ...
         Exception
           When No_Data_Found Then
             ...
             Begin
                ...
                aDettaglioNuovo.ORIGINE                       := aDett.ORIGINE;              -- cio? DIR
                aDettaglioNuovo.CATEGORIA_DETTAGLIO           := aDett.CATEGORIA_DETTAGLIO;  -- cio? PRE
...
End;
 ...
 begin
  ...
  -- Verifica che il PDG del CDR non sia gi? stato ribaltato sull'area
  ...
  if aStatoRibaltamento = 'Y' then
   IBMERR001.RAISE_ERR_GENERICO('Ribaltamento del cdr '||aCdCentroResponsabilita||' gi? effettuato');
  end if;
  ...
  if CNRCTB020.ISCDRENTE(aCDRPrimo) then
   IBMERR001.RAISE_ERR_GENERICO('Il CDR '||aCdCentroResponsabilita||' ? un CDR ENTE!');
  end if;
  ...
  if not (CNRCTB050.getStato(aEsercizio, aCDRPrimo.cd_centro_responsabilita)=CNRCTB050.STATO_PDG_FINALE) then
   IBMERR001.raise_err_generico('Il piano di gestione di primo livello non ? in stato definitivo!');
  end if;
  ...
  for aPDG in CNRCTB055.TUTTI_PDG_LINEA(aEsercizio, aCDRPrimo) loop
   ...
   -- se l'area non viene trovata non c'? niente da ribaltare
  ...
  -- che la sua linea ? stata ribaltata su AREA
...
-- FORSE ? MEGLIO CHE LO CAMBI ALL'INIZIO.
  if not (CNRCTB050.getStato_PDG_ESERCIZIO(aEsercizio, aCDRPrimo.cd_centro_responsabilita)=CNRCTB050.STATO_PDG2_APERTO_GEST) then
   IBMERR001.raise_err_generico('1. Il piano di gestione del CdR '||aCDRPrimo.cd_centro_responsabilita||' non ? in stato definitivo !');
  end if;
  ...
  -- Da qui in poi comincia la vera operazione di ribaltamento del CDR
   ...
   CNRCTB050.lockPdg_esercizio(aEsercizio, aCdCentroResponsabilita);
    ...
    -- Ciclo sui dettagli del PDGG ENTRATA da ribaltare su AREA (non c'entra pi? nulla la Natura 5)
    ...
    for aDett in CNRCTB050.PDGG_ETR_DA_RIB_SU_AREA(aEsercizio, aCDRPrimo.cd_centro_responsabilita) loop
       ...
       Exception
        ...
        When Too_Many_Rows Then
             IBMERR001.RAISE_ERR_GENERICO('1. Il CDS Area '||aDett.Cd_cds_Area||' possiede pi? di un CdR !');
       End;
  ...
  ribaltaSuAreaPDGG_etr_Int(aEsercizio, aCdCentroResponsabilita, aCDR, aCDRArea, aDett, Null, recParametriCNR, aTSNow, aUser);
    ...
    -- Ciclo sui dettagli del PDGG SPESA da ribaltare su AREA (non c'entra pi? nulla la Natura 5)
    ...
    for aDett in CNRCTB050.PDGG_SPE_DA_RIB_SU_AREA(aEsercizio, aCdCentroResponsabilita) loop
       ...
       Exception
        When No_Data_Found Then
             IBMERR001.RAISE_ERR_GENERICO('Il CDS Area '||aDett.Cd_cds_Area||' non possiede alcun CdR !');
        When Too_Many_Rows Then
             IBMERR001.RAISE_ERR_GENERICO('2. Il CDS Area '||aDett.Cd_cds_Area||' possiede pi? di un CdR !');
       End;
...
-- aCdCdr -> cd centro responsabilit?
...
For RIGHE_VAR_ENT In (Select *
  ...
  if CNRCTB020.ISCDRENTE(aCDRPrimo) then
   IBMERR001.RAISE_ERR_GENERICO('Il CDR '||RIGHE_VAR_ENT.CD_CDR_ASSEGNATARIO||' ? un CDR ENTE!');
  end if;
  ...
  if not (CNRCTB050.getStato_PDG_ESERCIZIO(aEsercizio, aCDRPrimo.cd_centro_responsabilita) = CNRCTB050.STATO_PDG2_CHIUSO_GEST) then
   IBMERR001.raise_err_generico('Il piano di gestione del CdR '||aCDRPrimo.cd_centro_responsabilita||' non ? in stato definitivo !');
  end if;
  ...
  -- Da qui in poi comincia la vera operazione di ribaltamento del CDR
       ...
       Exception
        ...
        When Too_Many_Rows Then
             IBMERR001.RAISE_ERR_GENERICO('1. Il CDS Area '||RIGHE_VAR_ENT.Cd_cds_Area||' possiede pi? di un CdR !');
       End;
...
For RIGHE_VAR_SPE In (Select *
  ...
  if CNRCTB020.ISCDRENTE(aCDRPrimo) then
   IBMERR001.RAISE_ERR_GENERICO('Il CDR '||RIGHE_VAR_SPE.CD_CDR_ASSEGNATARIO||' ? un CDR ENTE!');
  end if;
  ...
  -- Da qui in poi comincia la vera operazione di ribaltamento del CDR
       ...
       Exception
        ...
        When Too_Many_Rows Then
             IBMERR001.RAISE_ERR_GENERICO('2. Il CDS Area '||RIGHE_VAR_SPE.Cd_cds_Area||' possiede pi? di un CdR !');
       End;
...
--  end if;
  ...
  if not (CNRCTB050.getStato_PDG_ESERCIZIO(aEsercizio, aCDRPrimo.cd_centro_responsabilita)=CNRCTB050.STATO_PDG2_APERTO_GEST) then
   IBMERR001.raise_err_generico('2. Il piano di gestione del CdR '||aCDRPrimo.cd_centro_responsabilita||' non ? in stato definitivo !');
  end if;
...
-- se l'assegnatario ? anche l'accentratore stanzia tutte le colonne (acc + dec)
-- se l'assegnatario non ? anche l'accentratore stanzia solo il decentrato
-- perch? si tratta di voce sia accentrata che decentrata
...
Begin
			   ...
			   aTSNow := sysdate;
   	recParametriCNR := CNRUTL001.getRecParametriCnr(aEsercizio);
   --27/11/2015: RafPag: Se ? attiva la nuova gestione esco perch? non deve essere pi? creata
                  ...
                  exception when no_data_found then
                   begin
                    ...
                    where
                      ...
                      fonteAgg:='*';
                    exception when no_data_found then
                      -- Presupponiamo che se non ? in testata non ? soggetta ai limite(per evitare di inserire dei  limiti fittizi per le fonte esterne)
