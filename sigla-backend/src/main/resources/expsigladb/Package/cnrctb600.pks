...
-- ==================================================================================================
--
-- CNRCTB600 - package di utilit? per gestione MINICARRIERE
--
-- Date: 21/07/2003
-- Version: 1.25
--
...
-- una minicarriera se il compenso ? creato contestualmente alla minicarriera stessa (questa
...
-- Inserito controllo di sollevare eccezione se l'importo recuperato ? pari a zero
--
-- Date: 28/02/2003
-- Version: 1.16
--
...
-- i valori delle sole rate che risultano associate a compensi (Questa gestione ? estesa anche alle
...
-- minicarriera a tassazione separata, ? stata modificata aDataAccessoScaglione inserendo aEsercizioMCarriera
...
BEGIN
   ...
   --    distanza di un giorno
   -- 3) In ogni caso il numero delle rate indicate coincide con quelle calcolate
   -- Per la tassazione separata il numero delle rate ? calcolato in base alle seguenti regole:
   ...
   --       minicarriera ? < della data di inizio esercizio di registrazione esiste solo la prima rata. In
   --       questo caso la data di fine della prima rata ? pari alla data di fine della minicarriera stessa.
   ...
   BEGIN
      ...
      LOOP
         ...
         IF i = 1 THEN
            ...
            -- Se la data di fine della prima rata corrisponde a quella di fine della minicarriere ? prevista
            -- una sola rata
            ...
            IF aDataScadPrimaRata = aRecMinicarriera.dt_fine_minicarriera THEN
               ...
               ELSE
                  IBMERR001.RAISE_ERR_GENERICO
                     ('La minicarriera in calcolo prevede solo 1 rata ma ne ? stato indicato un numero diverso ' ||
                      aRecMinicarriera.numero_rate);
               END IF;
            END IF;

            -- Se la data di fine della prima rata ? < della data di inizio della minicarriera si solleva errore
            ...
            -- Se la data di fine della prima rata ? < della data di fine della minicarriera ? stato
            ...
            IF (aDataScadPrimaRata < aRecMinicarriera.dt_fine_minicarriera AND
               ...
               IBMERR001.RAISE_ERR_GENERICO
                  ('E'' stato indicato un numero di rate pari ad 1 per una minicarriera che deve generare pi? rate');
            END IF;
         ...
         ELSE
            ...
            IF aDataScadSecondaRata = aRecMinicarriera.dt_fine_minicarriera THEN
                ...
                ELSE
                   IBMERR001.RAISE_ERR_GENERICO
                      ('La minicarriera in calcolo prevede solo 2 rate ma ne ? stato indicato un numero diverso ' ||
                       aRecMinicarriera.numero_rate);
                END IF;
            END IF;
...
BEGIN
   ...
   BEGIN
      ...
      IF aRataCalcolo = 1 THEN
         ...
         -- Se no anticipo/positicipo o solo anticipo la data di fine della prima rata ? sempre pari al
         -- 31/12 o alla data di scadenza della minicarriera se questa ? tutta di competenza di esercizi passati
         ...
         -- Se posticipo la data di fine della prima rata ? calcolata per differenza tra la data di sistema
         -- della registrazione della minicarriera e i mesi di posticipo. Se il periodo di competenza della
         -- minicarriera indica un intervallo di mesi interi ritorna un fine mese altrimenti ? pari al GG -1
         -- della data inizio. In ogni caso se questa ? > del 31/12 si torna 31/12.
         -- Se la data di fine cos? calcolata ? maggiore della data di fine della minicarriera si torna la
      ...
      ELSE
         ...
         IF    isRataMeseIntero = 'Y' THEN
               ...
               -- No anticipo posticipo. La data di fine ? la fine mese del mese successivo la prima rata

               IF    aRecMinicarriera.ti_anticipo_posticipo = 'N' THEN
                     aDataControllo:=IBMUTL001.getAddMonth(aDataScadPrimaRata,1);

               -- Anticipo. La data di fine ? la data fine della prima rata + mesi anticipo
               ...
               -- Posticipo. La data di fine ? la fine mese del mese successivo la prima rata
         ...
         ELSIF isRataMensile = 'Y' THEN
               ...
               -- No anticipo posticipo. La data di fine ? il GG -1 della data di inizio della minicarriera nel
               -- mese + 2 della data di prima scadenza

               IF    aRecMinicarriera.ti_anticipo_posticipo = 'N' THEN
                     aDataControllo:=IBMUTL001.getAddMonth((aDataScadPrimaRata + 1),1);

               -- Anticipo. La data di fine ? il GG -1 della data di inizio della minicarriera nel
               ...
               -- Posticipo. La data di fine ? il GG -1 della data di inizio della minicarriera nel
...
BEGIN
   ...
   BEGIN
      ...
      -- Impedisco la cancellazione se lo stato della minicarriera ? sospesa o cessata
      ...
      LOOP
         ...
         BEGIN
            ...
            LOOP
               ...
               -- Sommo le rate.
               -- Se la matrice trattamento importo ? vuota valorizzo il primo elemento
               -- Se la matrice trattamento importo ? piena allora se il codice trattamento ? gi?
   ...
   IF matriceImTrattamento_tab.COUNT = 0 THEN
      IBMERR001.RAISE_ERR_GENERICO
         ('E'' stato recuperato un valore delle rate pari a 0. ' ||
          'Probabilmente sono state pagata anticipatamente o in ritardo una o pi? rate con scadenza ' ||
...
BEGIN
   ...
   IF (aPgMcarriera IS NOT NULL AND
      ...
      BEGIN
         ...
         -- quelle che, pur scadenti nel periodo in esame, hanno data fine anteriore all'inizio
         -- dell'esercizio della minicarriera.
         -- La lettura ? eseguita solo se aRecTipoTrattamento.fl_soggetto_conguaglio = 'Y'
   ...
   -- Eliminato il test in quanto ? possibile avere valore 0 nel numero giorni
...
BEGIN
   ...
   END IF;
   -- Se il Compenso ? un compenso da Conguaglio vanno presi in considerazione
            ...
            LOOP
               ...
               BEGIN
                  ...
                  LOOP
                     ...
                     -- sel la rata ? esterna al periodo di competenza in input la riconduco ad esso
...
BEGIN
   ...
   -- Se l'importo irpef ? uguale a zero si torna, come aliquota media, quella del primo scaglione
   -- altrimenti ? calcolata come valore irpef moltiplicato 100 diviso per l'imponibile base
