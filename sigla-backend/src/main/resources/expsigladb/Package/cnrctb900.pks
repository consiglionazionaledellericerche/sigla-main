...
-- - La data fine cococo non ? la massima data fine ma la minima
...
--   momento e poi perch? occorre verificare in Sigla come vengono gestiti
--
-- Date: 02/03/2010
-- Version: 1.12
--
...
Begin
    ...
    -- restituisco 'Y' (c'? anche il conguaglio)
...
BEGIN
   ...
   -- Non faccio nulla se ? richiesta l'estrazione per tutte le anagrafiche
        ...
        -- Annotazione di default prima parte quota deduzione standard
         ...
         ELSIF i = 18 THEN
               ...
               parametri_tab(i).stringa:='La quota di deduzione riconosciuta su tutte le tipologie di reddito ' ||
                                         'ai sensi dell''art. 10 bis comma 1 del TUIR ? pari a euro ';
        ...
        -- Annotazione di default seconda parte quota deduzione standard
         ...
         ELSIF i = 19 THEN
               ...
               parametri_tab(i).stringa:=', la quota di deduzione riconosciuta sui redditi di lavoro dipendente ' ||
                                         'o assimilati ai sensi dell''art. 10 bis comma 2 del TUIR ? pari a euro ';
        ...
        -- Annotazione prima parte quota deduzione fissa intera
         ...
         ELSIF i = 20 THEN
               ...
               parametri_tab(i).stringa:='Su richiesta del sostituito la deduzione riconosciuta su tutte le ' ||
                                         'tipologie di reddito ai sensi dell''art. 10 bis comma 1 del TUIR, ? ' ||
   ...
   BEGIN
      ...
      LOOP
         ...
         VALUES (inRepID,
                 ...
                 0,
                 0,
                 0,          --verr? valorizzato da CONTRIBUTO_RITENUTA
                 0,	     --verr? valorizzato da CONGUAGLIO
                 0,	     --verr? valorizzato da CONGUAGLIO
                 0,      --verr? valorizzato da CONTRIBUTO_RITENUTA o da CONGUAGLIO
...
--pipe.send_message('aRecEstrazioneCudDett.im_bonus_erogato = '||aRecEstrazioneCudDett.im_bonus_erogato);
                  ELSE
                     ...
                     IF aRecTipoContributoRitenuta.fl_assistenza_fiscale = 'Y' Then
                        --QUESTI CAMPI VALORIZZATI NON VENGONO PI? UTILIZZATI, IN OGNI CASO I CORI PER L'ASSISTENZA
            ...
            ELSIF aRecTipoContributoRitenuta.cd_classificazione_cori = CNRCTB545.isCoriPrevid THEN
                  ...
                  -- Se ? stato superato lo scaglione (cio? esiste il record in contributo_ritenuta_det),
                  -- l'imponibile previdenziale deve essere preso da li, perch? su contributo_ritenuta c'?
...
BEGIN
   ...
   BEGIN
      ...
      LOOP
         ...
         -- Il codice anagrafico in lettura ? diverso dal precedente
         ...
         BEGIN
            ...
            IF isScritto = 'N' THEN
               isScritto:= 'Y';
	             ...
	             -- i campi successivi non sono pi? utilizzati, quindi li valorizzo a zero
	             add_reg_anno_prec := 0;
               add_com_anno_prec := 0;
               /*
   ...
   -- Se il terzo in esame ? un cervellone, devo ricalcolare l'imponibile (leggendolo anche dal conguaglio)
   -- poich? il conguaglio stesso potrebbe averlo modificato
...
--pipe.send_message('anag = '||aRecEstrazioneCudDett.cd_anag);
            ...
            Begin
               Select --Sum(DECODE(A.fl_detrazioni,'Y', Decode(A.fl_tassazione_separata,'N',A.imponibile_fisc_lordo_compenso, 0),0)),
                      ...
                      --dei giorni lavorati) e quindi l'imponibile che leggo dal conguaglio non pu? tener conto di tale flag
...
BEGIN
   ...
   BEGIN
	 ...
	 -- Poich? esistono dati sporchi sul db (residenza anagrafico diversa da residenza terzo)
	 -- e poich? nei compensi e nell'acconto prendiamo la residenza dal terzo, anche nel CUD
	 -- i dati della residenza li prendiamo dal terzo e poich? potrebbero esserci pi? terzi validi
	 ...
	 Begin
   	 ...
   	 Exception
              ...
              When Too_Many_Rows then
                  IBMERR001.RAISE_ERR_GENERICO('Esistono pi? terzi validi per Anagrafico:'||aRecEstrazioneCud.dip_cd_anag);
         End;
         -- Aggiornamento dati da ESTRAZIONE_CUD_DETT su ESTRAZIONE_CUD inclusi dati di testata

         aRecEstrazioneCud.dip_cognome:=NULL;
         aRecEstrazioneCud.dip_nome:=NULL;
         --03/03/2009 - Prendiamo sempre il Cognome ed il Nome, cos? se non sono valorizzati il CUD
	 ...
	 -- Se il comune della residenza fiscale ? estero, si prende il comune della UO
...
BEGIN
   ...
   BEGIN
      ...
      -- Apertura cursore e determinazione degli intervalli date di lavoro a parit? di codice e tipo anagrafico
            ...
            LOOP
               ...
               IF aRecEstrazioneCudAnnotaz.pg_tipo_annotazione = 1 THEN
                  IF scrittoArt1 = 'N' THEN
                     ...
                     memAnnotazione:=memAnnotazione ||
                                     'L''importo indicato al punto 1 della parte B dati fiscali ? cos? composto: ' || CHR(10);
                  END IF;
               ...
               ELSE
                  IF scrittoArt2 = 'N' THEN
                     scrittoArt2:= 'Y';
                     memAnnotazione:=memAnnotazione ||
                                     'L''importo indicato al punto 2 della parte B dati fiscali ? cos? composto: ' || CHR(10);
                  END IF;
                  aStringaImporto:=LTRIM(TO_CHAR(aRecEstrazioneCudAnnotaz.imponibile,'9999G999G999G999D99'));
                  IF LENGTH(aStringaImporto) - INSTR(aStringaImporto,',',-1) != 2 THEN
                     aStringaImporto:=TRANSLATE(aStringaImporto,aTrnslFrom,aTrnslTo);
      ...
      LOOP
         ...
         BEGIN
            ...
            IF aRecEstrazioneCud.deduzione_dovuta != 0 THEN
               ...
               -- Se esistono solo imponibili per puto 2 tutto ? sempre gestito nelle 3000
...
BEGIN
   ...
   --Esistono diversi conguagli non contabilizzati che quindi non vengono inseriti nel cud (il che ? giusto)
   --per? se per essi esiste una rateizzazione (calcolata con il conguaglio), essa non viene presa
   /*
   Declare
      ...
      LOOP
         ...
         LOOP
            ...
            IF i = 1 THEN
               ...
               --(? il comune con cui ? stato calcolato l'acconto per l'esercizio a cui si riferisce il CUD
               --e se non c'? nessun acconto ? quello dell'anagrafica)
	       ...
	       -- Codice comune con cui ? stato calcolato l'acconto addizionale comunale per l'esercizio successivo
	       -- Altrimenti ? quello dell'anagrafica
	    ...
	    Else         -- i = 3
	       ...
	       -- se il terzo ? SOLO non residente (per come abbiamo inteso noi i non residenti)i primi cinque campi sono a zero
	       If aRecEstrazioneCud.fl_soggetto_estero = 'Y' And
                  Nvl(aRecEstrazioneCud.imponibile_si_detr,0) = 0 And
                  Nvl(aRecEstrazioneCud.imponibile_no_detr,0) = 0 Then
         ...
         Else
   		  ...
   		  --Esistono diversi conguagli non contabilizzati che quindi non vengono inseriti nel cud (il che ? giusto)
   		  --per? se per essi esiste l'acconto, poich? aRecEstrazioneCud.addizionale_comunale ? <0, invece dell'acconto
   		  --viene passato zero
                  ...
                  --Else
                       --SE DOVUTO - ACCONTO >= 0 PASSO L'ACCONTO COS? COME CALCOLATO ALTRIMENTI LO FORZO UGUALE AL SALDO
         ...
         -- se il terzo ? residente i successivi due campi sono a zero
         aStringa:=aStringa ||
