...
-- Date: 07/01/2003
-- Version: 2.9
-- Fix errore controllo consistenza unit? organizzativa con UO pdersonale obbligazioni stipendi
--
-- Date: 07/01/2003
-- Version: 3.0
...
-- essere recuperata dal personale; inoltre, poich? vanno versati solo i comuni con addizionale positiva,
...
-- pre: Il mese specificato ? maggiore di 1, e la liquidazione degli stipendi del mese precedente non ? stata fatta
-- post: viene sollevata un'eccezione
--
-- pre-post-name: Controllo liquidazione mese precedente
-- pre: Il mese specificato ? maggiore di 1, e la liquidazione degli stipendi del mese precedente non ? stata fatta
...
--       1. Il terzo ? specificato in configurazione CNR sotto la voce TERZO_SPECIALE DIVERSI_STIPENDI
...
--        Altrimenti
--         viene aperta una normale partita giro che verr? chiusa dal versamento CORI come per i compensi
--
...
-- =============================================================================
 ...
 begin
  ...
  -- Estrae il terzo associato all'UO di stipendi_cofi e le sue modalit? di pagamento di tipo bancario pi? recenti
...
--   aManRiga.CD_TERZO_UO:=0;
 ...
 BEGIN
  aTSNow:=sysdate;
  --Il flusso per il mese 15 pu? essere contabilizzato in qualunque momento
  If aMese != 15 Then
        if aMese < 1 or aMese > 13 then
         IBMERR001.RAISE_ERR_GENERICO('Il mese deve essere compreso tra 1 (Gennaio) e 13 (Mese Tredicesima)');
        end if;

        -- se il mese ? > 1 verifico che la liquidazione del mese precedente sia stata fatta
        if aMese > 1 then
           Begin
               Select * into aStip
               From   stipendi_cofi
  ...
  Begin
      ...
      For Update nowait;
  Exception when NO_DATA_FOUND then
   IBMERR001.RAISE_ERR_GENERICO('Dati stipendiali non trovati o contabilizzazione gi? effettuata per mese n.'||aMese||' es:'||aEs);
  End;
...
--   aGen.PG_LETTERA:=0;
  ...
  for aStipObb in (Select * from stipendi_cofi_obb_scad
   ...
   If aObbScad.im_associato_doc_amm <> 0 or aObbScad.im_associato_doc_contabile <> 0 then
      If cnrutil.isLabelObbligazione() Then
        IBMERR001.RAISE_ERR_GENERICO('Scadenza di obbligazione n. '||aStipObb.pg_obbligazione||' associata a liquidazione stipendi mese n. '||aMese||' es. '||aEs||' gi? associata a documenti contabili o amministrativi');
      Else
        IBMERR001.RAISE_ERR_GENERICO('Scadenza di impegno n. '||aStipObb.pg_obbligazione||' associato a liquidazione stipendi mese n. '||aMese||' es. '||aEs||' gi? associata a documenti contabili o amministrativi');
      End If;
   End If;
   If aMese != 15 Then
...
-- crea il compenso fittizio degli stipendi, crea CONTRIBUTO_RITENUTA e mette il compenso su STIPENDI_COFI
  ...
  Begin
    ...
    For aStipObbDett In cObb(FLUSSO_ANNULLI) Loop
          ...
          If sql%Rowcount = 0 Then
          	errore := 'Y';
          	ibmutl200.logErr(gPgLog, '6', 'Nel Flusso Annulli Stipendiali ? presente l''obbligazione '
    ...
    For aStipCoriDett In cCori(FLUSSO_ANNULLI) Loop
          ...
          If sql%Rowcount = 0 Then
          	errore := 'Y';
          	ibmutl200.logErr(gPgLog, '6', 'Nel Flusso Annulli Stipendiali ? presente la ritenuta '
    ...
    For aStipCoriDett In cCori(FLUSSO_RIMBORSI) Loop
          ...
          If sql%Rowcount = 0 Then
          	errore := 'Y';
          	ibmutl200.logErr(gPgLog, '6', 'Nel Flusso Rimborsi ? presente la ritenuta '
          	||aStipCoriDett.cd_contributo_ritenuta||' che non esiste nel flusso degli Stipendi Principale', '');
          	-- inserisco la riga
          	aStipCori.esercizio 		:= aStipCoriDett.esercizio;
          	aStipCori.mese 			:= aStipCoriDett.mese;
    ...
    --? uguale alla somma in STIPENDI_COFI_CORI_DETT dei dettagli per singola regione
    ...
    For aStipObbDett In cObb(FLUSSO_CONTR_FIGURATIVI) Loop
          ...
          If sql%Rowcount = 0 Then
          	errore := 'Y';
          	ibmutl200.logErr(gPgLog, '6', 'Nel Flusso Contributi Figurativi ? presente l''obbligazione '
          	||To_Char(aStipObbDett.esercizio_obbligazione)||'/'||To_Char(aStipObbDett.esercizio_ori_obbligazione)||'/'||aStipObbDett.cd_cds_obbligazione||'/'||To_Char(aStipObbDett.pg_obbligazione)
          	||' che non esiste nel flusso degli Stipendi Principale', '');
          End If;
    End Loop;
    ...
    For aStipCoriDett In cCori(FLUSSO_CONTR_FIGURATIVI) Loop
          ...
          If sql%Rowcount = 0 Then
          	errore := 'Y';
          	ibmutl200.logErr(gPgLog, '6', 'Nel Flusso Contributi Figurativi ? presente la ritenuta '
          	||aStipCoriDett.cd_contributo_ritenuta||' che non esiste nel flusso degli Stipendi Principale', '');
          End If;
    End Loop;
    If esistonoDati = 'Y' Then
      ...
      Else
                ...
                Begin
                   for rec in (select cd_catastale
                              ...
                              Where esercizio = aEs
                                ...
                                And cd_catastale Not In (Select cd_catastale From comune Where ti_italiano_estero = 'I'))
                   loop
                         ibmutl200.logErr(gPgLog, '9', 'Errore nel Flusso delle Addizionali Comunali. Il Codice Catastale '||rec.cd_catastale||' non ? corretto', '');
                   end loop;
                End;
           ...
           Else
             If esistonoDati = 'Y' Then
	   	    ...
	   	    -- cio? con quello che realmente verso con F24
	   	    Select Nvl(Sum(saldo),0)
	            ...
	            From STIPENDI_COFI_CORI_DETT
    	   	    Where esercizio = aEs
       	            ...
       	            -- che realmente verser? con F24 (cio? la somma dei positivi della colonna saldo in STIPENDI_COFI_CORI_DETT
       	            differenza := sommaSaldiPosAddCom - Nvl(aStipCori.ammontare,0);
       	            -- se la differenza ? positiva vuol dire che in F24 ho di pi? di quello che mi risulta da versare
       	            -- e quindi aumento l'importo da versare esattamente della differenza
       	            -- se la differenza ? negativa vuol dire che in F24 ho di meno di quello che mi risulta da versare
       		        ...
       		        If differenza > 0 Then
       		           ibmutl200.logInf(gPgLog, '5', 'Per il cori '||To_Char(aStipCori.cd_contributo_ritenuta)||' ? stato aggiunto all''importo '||
	   	           To_Char(aStipCori.ammontare, '999G999G999G999G990D00')||', l''importo '||To_Char(Abs(differenza), '999G999G999G999G990D00'), '');
	   	        Else
	   	           ibmutl200.logInf(gPgLog, '5', 'Per il cori '||To_Char(aStipCori.cd_contributo_ritenuta)||' ? stato diminuito l''importo '||
 ...
 Begin
    --controllo che per il mese scelto non sia stata gi? effettuata la liquidazione
    Select stato
    ...
    If stato_liquidazione = 'P' Then
         IBMERR001.RAISE_ERR_GENERICO('Non ? possibile procedere all''annullamento dell''elaborazione poich? ? stata gi? effettuata la liquidazione del mese selezionato');
    Else
