...
') ha una tipologia ('||aAcc.cd_tipo_documento_cont||') non compatibile.');
  ...
  --Se l'esercizio originale non ? valorizzato lo inizializzo con quello dell'esercizio
 ...
 -- Prepara un dettaglio di scadenza di accertamento CNR per CDS arricchito nel caso siano specificate linee di attivit? comuni
 ...
 begin
  for i in 1..aDettScadenza.count loop
   ...
   exception when NO_DATA_FOUND then
    IBMERR001.RAISE_ERR_GENERICO('Linea di attivit?: '||aDettScadenza(i).cd_linea_attivita||' non valida o con natura non compatibile con la voce del piano: '||aEV.cd_elemento_voce||' per l''esercizio '||aDettScadenza(i).ESERCIZIO_ORIGINALE);
   end;
   select * into aTipoLa from tipo_linea_attivita where
       cd_tipo_linea_attivita = aLA.cd_tipo_linea_attivita;

   -- Estrae tutte le linee di attivit? comuni finalizzate ai cdr
...
') ha una tipologia ('||aAcc.cd_tipo_documento_cont||') non compatibile.');
  ...
  if aAcc.fl_pgiro is null or aAcc.fl_pgiro = 'Y' then
   IBMERR001.RAISE_ERR_GENERICO('L''accertamento che si sta tentando di generare ('||aAcc.cd_cds||'/'||aAcc.esercizio||'/'||aAcc.esercizio_originale||'/'||aAcc.pg_accertamento||
') ? una partita di giro e quindi non ? non compatibile.');
  end if;
  ...
  -- Check di compatibilit? tra natura linea di attivit? ed elemento voce
