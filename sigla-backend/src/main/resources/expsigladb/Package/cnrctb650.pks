...
-- bens? annullato e quindi rigenerato magari ad un terzo riferito ad altro anagrafico
-- Errore interno 2831
--
-- Date: 18/11/2002
-- Version: 1.8
...
-- ? rimasta nel blocco interno BEGIN END
--
-- Date: 29/01/2003
-- Version: 1.13
--
...
-- - Non ? possibile attivare la gestione dell'accantonamento se questo risulta negativo.
...
--    a) Se il conguaglio viene effettuato nel periodo in cui la sospensione ? ancora attiva,
--       la ritenuta viene sospesa (IRPEF=0) e lo stesso importo viene aggiunto al sospeso precedente.
--    b) Se il conguaglio viene effettuato nel periodo in cui la sospensione non ? ancora attiva,
...
-- Verifica ammissibibilita del conguaglio per un dato soggetto anagrafico. Se il conguaglio ?
...
BEGIN
   ...
   BEGIN
      ...
      IF esisteCompenso = 0 THEN
         IBMERR001.RAISE_ERR_GENERICO
            ('Non ? stato trovato alcun compenso eleggibile a conguaglio per anagrafico ' ||
             aRecAnagrafico.cd_anag );
      END IF;
   ...
   BEGIN
      ...
      IF esisteCompenso = 0 THEN
         IBMERR001.RAISE_ERR_GENERICO
            ('Tutti i compensi eleggibili a conguaglio risultano essere gi? stati conguagliati per anagrafico ' ||
             aRecAnagrafico.cd_anag);
      END IF;
   ...
   BEGIN
      ...
      BEGIN
         ...
         IF aRecCompensoConguaglioBase.stato_cofi != 'P' THEN
            IBMERR001.RAISE_ERR_GENERICO
               ('Non ? possibile eseguire un conguaglio se il precedente non risulta essere in stato pagato');
         END IF;
...
BEGIN
   ...
   -- ? compreso anche l'eventuale imponibile esterno
...
--pipe.send_message('aImportoRifDetrazioniFam = '||aImportoRifDetrazioniFam);
   ...
   -- per conguaglio con aliquota massima in anagrafico. Il valore minimo ? utilizzato, in questi casi,
...
--pipe.send_message('aTotRedditoComplessivo = '||aTotRedditoComplessivo);
   ...
   */
         ...
         End;
         -- Se non ci sono figli senza coniuge oppure ce ne sono pi? di uno, calcolo le detrazioni normalmente
         If aContaFiSenzaConiuge != 1 Then --cio? se non ci sono figli senza coniuge oppure ce ne sono pi? di uno
...
--                                           glbDataMinCompetenza + numeroGG - 1,
         ...
         End;
	       ...
	       --se il reddito ? inferiore a 8000 euro e se il terzo lo ha richiesto,
...
--pipe.send_message('aRecCoriConguaglio.CD_CONTRIBUTO_RITENUTA '||aRecCoriConguaglio.CD_CONTRIBUTO_RITENUTA);
         ...
         IF CNRCTB545.IsCoriCreditoIrpef(aRecCoriConguaglio.cd_contributo_ritenuta) = 'Y' then
            --per ora passo glbNumeroGiorni che ? anche il numero gg del cud
--pipe.send_message('3 ');
...
--pipe.send_message('glbImportoCreditoIrpefDovuto '||glbImportoCreditoIrpefDovuto);
   ...
   Begin
      ...
      /* Se aImCompCongIrpefNetto  ? maggiore di -0.49 e minore di -0.01 (cio? verrebbe emesso un mandato
...
--pipe.send_message('glbFlDetrazioniAltriTipi = '||glbFlDetrazioniAltriTipi);
   ...
   BEGIN
      ...
      LOOP
         ...
         -- dovuto/goduto l'eventuale importo precedentemente accantonato. Se questo risulta gi? in parte
         -- pagato il conguaglio non ? ammesso
         ...
         IF CNRCTB545.getIsIrpefScaglioni(aRecCoriConguaglio.cd_classificazione_cori,
               ...
               -- Solo se l'ammontare netto ? positivo lo sospendo
               Declare
                 ...
                 If aAmmontareCompCongNetto > 0 Then
	                  If aRecAnagrafico.fl_sospensione_irpef = 'Y' And
                       ...
                       If  FL_SOSP Is Not Null And fl_sosp = 'Y' Then
	                     ...
	                     End If;
	                  Else  -- non devo applicare la sospensione quindi me la valorizzo con quella della testata che ? la precedente
	               ...
	               Elsif aAmmontareCompCongNetto < 0 Then
	                  ...
	                  -- per? ridulo la sospensione gi? calcolata
...
--pipe.send_message('glbFlNoCreditoIrpef = '||glbFlNoCreditoIrpef);
   --? prevista la gestione ed il terzo non ha chiesto che non sia applicato
...
--pipe.send_message('aCoefficiente = '||aCoefficiente);
             -- calcolo il credito totale rapportato agli effettivi giorni che lavorer? nell'anno
   ...
   BEGIN
      ...
      LOOP
	    ...
	    End If;
            ...
            -- elaborazione non ? stato precedentemente incluso in alcun conguaglio, si incrementa il
...
BEGIN
   ...
   BEGIN
      ...
      LOOP
         ...
         -- Se la collezione delle date da stornare ? vuota allora esco dal ciclo
      ...
      -- Esco se la collezione delle date da stornare ? vuota ed azzero inoltre entrambe le matrici
      ...
      -- Esco se la matrice degli intervalli da stornare ? vuota
   ...
   BEGIN
      ...
      LOOP
         ...
         LOOP
            ...
            LOOP
               ...
               IF intervallo_date_neg(i).tDataDa IS NOT NULL THEN
                  ...
                  -- L'intervallo da recuperare ? contenuto in quello da stornare
         ...
         -- Esco se la collezione delle date da stornare ? vuota
...
BEGIN
   ...
   -- Esco se la collezione delle date ? vuota
...
PROCEDURE unisciMatriceDate
   ...
   numeroGG INTEGER;
   salta   CHAR(1);
   intervallo_date_appoggio CNRCTB545.intervalloDateTab;                 --date unite (cio? raggruppo gli intervalli contigui)
...
BEGIN
   ...
   -- Gi? so che non esistano intervalli di date con sovrapposizione
         ...
         LOOP
		  ...
		  j:=1;
                  If To_Char(intervallo_date_appoggio(1).tDataDa,'yyyy') = To_Char(intervallo_date_appoggio(1).tDataA,'yyyy') Then
                     --tutto l'intervallo ? nell'anno precedente
                     intervallo_date_annoprec(j).tDataDa:=intervallo_date_appoggio(1).tDataDa;
                     intervallo_date_annoprec(j).tDataA:=intervallo_date_appoggio(1).tDataA;
                     ...
                     salta:='N';
                  Else
                     --la prima data ? nell'esercizio precedente, la seconda ? nell'esercizio del conguaglio
                     intervallo_date_annoprec(j).tDataDa:=intervallo_date_appoggio(1).tDataDa;
...
BEGIN
   ...
   -------------------------------------------------------------------------------------------------
   -- Segnalo errore se la collezione delle date non ? vuota
...
BEGIN
   ...
   BEGIN
      ...
      -- Se per il terzo in esame ? previsto l'acconto aggiorno il riferimento del conguaglio
...
BEGIN
   ...
   BEGIN
      ...
      IF (aRecConguaglio.cd_cds != aRecCompensoConguaglioBase.cd_cds_conguaglio OR
         ...
         IBMERR001.RAISE_ERR_GENERICO
            ('Impossibile eliminare un conguaglio che non ? l''ultimo generato per il codice anagrafico ' ||
             aRecAnagrafico.cd_anag);
      END IF;
      ...
      -- Il compenso risulta essere gi? in stato annullato
      IF aRecCompenso.stato_cofi = 'A' THEN
         IBMERR001.RAISE_ERR_GENERICO
            ('Il compenso conguaglio risulta essere gi? stato annullato in data ' ||
...
--pipe.send_message('1tabella_date_fam_ok_unite(i).tDataA = '||tabella_date_fam_ok_unite(i).tDataA);
	        --devo escludere gli intervalli dell'anno precedente
                If To_Char(tabella_date_fam_ok_unite(i).tDataDa,'yyyy') = inEsercizioConguaglio Then
                   ...
                   aImDetrazioniAl:=0;
                   aImDetrazioniFiS:=0;
                   --devo fare in modo che in totale non applico le detrazioni per pi? di 12 mesi
         ...
         If numero_mesi_totali < 12 And numero_mesi_anno_precedente > 0 Then
             ...
             --se ? stato gi? aggiunto prima, ora devo toglierlo
             If numero_mesi_totali > 0 --vuol dire che ho gi? aggiunto la maggiorazione e quindi ora la tolgo
         ...
         If (numero_mesi_totali + numero_mesi_anno_precedente) < 12 And numero_mesi_anno_successivo > 0 Then
             ...
             --l'importo di maggiorazione del coniuge, che non deve essere rapportato al periodo,
             --se ? stato gi? aggiunto prima, ora devo toglierlo
             If (numero_mesi_totali > 0 Or numero_mesi_anno_precedente > 0) --vuol dire che ho gi? aggiunto la maggiorazione e quindi ora la tolgo
                And Nvl(aImDetrazioniCo,0) > 0 Then
                Declare
                  maggCo   NUMBER;
                Begin
