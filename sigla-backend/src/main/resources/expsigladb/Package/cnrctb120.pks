...
-- Version: 4.1
--
-- Inserita routine di recupero dell'informazione che una fattura attiva ? gestita ad esigibilit?
...
-- Recupero dell'informazione che una fattura attiva ? gestita ad esigibilit? differita con i dati
...
BEGIN
   ...
   -- Se la fattura non ? per iva differita esco

   IF aRecFatturaAttiva.fl_liquidazione_differita = 'N' THEN
      RETURN attivaGestioneDifferita;
   END IF;

   -- Verifico se la fattura ? gi? stata stampata sul registro IVA o ? stata liquidata
   ...
   -- Se la fattura ? gi? stata liquidata non attivo la gestione per iva differita
...
BEGIN
    ...
    RETURN aRecFatturaPassiva;
	 --Non intercetto il no_data_Found perch? se non presenti i riferimenti al compenso richiamo la vecchia CNRCTB120.getTstFatturaDaRifTerzo
