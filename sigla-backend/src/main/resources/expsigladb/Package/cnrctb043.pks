...
-- Se l'obbligazione di contropartita dell'accertamento è cancellata, non viene aggiornata del delta
...
-- per fix errore che impediva le liquidazioni IVA su più U.O.
--
-- Date: 19/04/2006
-- Version: 2.4
-- Aggiunto nella procedura modificaPraticaObb il parametro "aAccConScad" che per default è 'N' e solo se chiamato
...
CREATE OR REPLACE package body CNRCTB043 is
 ...
 begin
  ...
  if aObbScad.im_associato_doc_amm <> 0 or aObbScad.im_associato_doc_contabile <> 0 then
   IBMERR001.RAISE_ERR_GENERICO(cnrutil.getLabelObbligazione()||' di versamento CORI al centro già utilizzata '||CNRCTB035.getDesc(aObb));
  end if;
  ...
  -- poichè possono esserci più scadenze, devo controllare che se aImDelta è negativo (annullamento liq)
  ...
  exception
   ...
   when TOO_MANY_ROWS then
    IBMERR001.RAISE_ERR_GENERICO('Documento generico con più di una riga non compatibile');
  end;
...
-- vecchio (stani 28.04.2005)
   ...
   when TOO_MANY_ROWS then
    IBMERR001.RAISE_ERR_GENERICO('Reversale con più di una riga non compatibile '||CNRCTB038.getDesc(aRev));
  end;
 ...
 begin
  ...
  if aObbScad.im_associato_doc_amm <> 0 or aObbScad.im_associato_doc_contabile <> 0 then
    If cnrutil.isLabelObbligazione() Then
      IBMERR001.RAISE_ERR_GENERICO('Obbligazione da troncare già utilizzata '||CNRCTB035.getDesc(aObb));
    Else
      IBMERR001.RAISE_ERR_GENERICO('Impegno da troncare già utilizzato '||CNRCTB035.getDesc(aObb));
    End If;
  end if;

  if aObb.stato_obbligazione = CNRCTB035.STATO_STORNATO then
    If cnrutil.isLabelObbligazione() Then
      IBMERR001.RAISE_ERR_GENERICO('Obbligazione da troncare già stornata '||CNRCTB035.getDesc(aObb));
    Else
      IBMERR001.RAISE_ERR_GENERICO('Impegno da troncare già stornato '||CNRCTB035.getDesc(aObb));
    End If;
  end if;
  update obbligazione
    set
...
procedure troncaPraticaAccPgiroInv(aEs number,aCdCds varchar2,aEsOri number,aPgAcc number,aTSNow date,aUser varchar2) is
 ...
 begin
  ...
  if aAccScad.im_associato_doc_amm <> 0 or aAccScad.im_associato_doc_contabile <> 0 then
    IBMERR001.RAISE_ERR_GENERICO('Accertamento da troncare già utilizzato '||CNRCTB035.getDesc(aAcc));
  end if;

  if aAcc.dt_cancellazione is not null then
    IBMERR001.RAISE_ERR_GENERICO('Accertamento da troncare già cancellato '||CNRCTB035.getDesc(aAcc));
  end if;
  update accertamento
    set
