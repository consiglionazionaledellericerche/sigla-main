...
--	indicato per pi? righe di Fattura, viene creata una sola riga di Dettaglio per il bene. E' stata corretta
--	anche l'operazione di UPDATE su INVENTARIO_BENI: nello stesso caso sopra citato, il VALORE_ALIENAZIONE
--	sar? la somma dei VALORE_ALIENAZIONE, indicati per il bene nelle diverse righe di Fattura.
--
-- Date: 31/07/2003
-- Version: 1.12
...
-- di aggiornamento con dettaglio dei beni per i quali questo aggiornamento non ? stato possibile.
-- Procedure: updScaricoInventarioBeni, updCaricoBeniAumentoValFtPas
-- messaggio: attenzione, per questo bene non verr? modificato automaticamente l'imponibile ammortamento
...
-- La regola generale ? che solo i beni principali hanno questa valorizzazione; gli accessori ereditano
...
-- 	Pre:  L'inventario ? scaricato in modo diretto
...
-- 	Pre:  L'inventario ? scaricato tramite la registrazione di una  fattura attiva
-- 	Post: Viene generata prima la testata del documento di scarico quindi una riga di dettaglio per ogni bene scaricato.
--
-- 	Pre-post name: Scarico dell'inventario da fattura Attive o in modo diretto.
-- 	Pre:  L'inventario ? scaricato.
...
--            DT_VALIDITA_VARIAZIONE  =	data in cui ? avvenuta la variazione
--
...
-- Pre:           Il bene non ? associato ad un tipo di ammortamento
-- Post:          Viene sollevata un'eccezione
--
-- Pre-post name: Controllo se il bene risulta gi? ammortizzato nell'esercio in esame
-- Pre:           Esiste almeno un bene risulta gi? ammortizzato nell'anno specificato
-- Post:          Viene sollevata un'eccezione
--
...
--
--         Controllo se esiste un tipo di ammortamento associato al gruppo categoria. Se tale tipo non ? definito per
...
--               Se la categoria.gruppo ? associata a pi? di un tipo ammortamento allora il sistema segnala l'impossibilit?
--                  di procedere.
--
--         Stabilisco se devo usare aliquota del primo anno o di anni successivi.
--            Se l'aliquota non viene trovata o questa ? zero vedi Pre-post name: Stabilisco l'aliquota di ammortamento
...
--            Se il bene presenta il flag totalmente scaricato = Y ed ? stato movimentato in esercizi successivi si
--               porta il flag = N
--            Si assestano i saldi (imponibile ammortamento) togliendo tutto quanto ? stato movimentato da buoni
--            di carico e scarico in esercizi successivi. Questa funzione ? svolta solo per i beni per cui risulta
--            verificata l'uguaglianza valore_iniziale + variazioni pi? - variazioni meno = imponibile ammortamento.
--
--         Si registra nel log un warning se il valore ammortizzato ? maggiore dell'imponibile ammortamento. La
--         segnalazione non ? bloccante.
--         Si registra nel log un warning se il valore assestato del bene vale zero e il flag totalmente scaricato = N
--         e imponibile ammortamento > 0. La segnalazione non ? bloccante.
--
...
--            Se la rata di ammortamento calcolata con la percentuale stabilita ? troppo alta in quanto risulta
...
-- Verifica se il bene ? gi? stato ammortizzato nell'anno specificato
...
--                parametri in input:
--                1) aPgInventarioDest se ? NULL ? posto uguale a inPgInventarioOri (trasferimento intra inventario)
--                2) aFlTrasferisciTutto se ? NULL ? posto uguale a N, non si esegue il trasferimento totale
--
-- Parametri:
--   inLocalTransId       -> Identificativo della chiave di lettura sulla tabella INVENTARIO_BENI_APG utilizzata
--                           quando il trasferimento non ? totale e leggo i beni da trasferire selezionati dall'utente
...
-- Pre:           L'esercizio ? minore del 2004
-- Post:          Viene sollevata un'eccezione
--
...
-- Post:          Viene sollevata un'eccezione
--
-- Pre-post name: Controllo cardinalit? della matrice beni da trasferire (blocco omogeneo di bene principale e suoi
--                accessori o accessori stand alone
-- Pre:           La matrice ? vuota
-- Post:          Viene sollevata un'eccezione
--
...
--                   2) Tutti i beni devono diventare accessori dello stesso bene di destinazione e questo non pu?
--                      essere nullo.
--                Non esiste un bene principale
--                   1) Se l'accessorio non ? da trasferire come principale deve esistere l'indicazione del bene
--                      di destinazione
-- Post:          Viene sollevata un'eccezione
--
...
--            Se il numero inventario ? diverso da quello memorizzato allora si esegue il trasferimento per il blocco.
--            Se ho gi? portato dei beni sulla matrice di appoggio e la prima occorrenza non ? un bene principale,
--               anche se il numero inventario ? uguale al precedente, si esegue il trasferimento per il blocco.
...
--            Si memorizzano i valori dei buoni di carico e scarico generati. La testata ? generata ogni 150 beni
...
--               Si crea la testata del buono di scarico. E' generata solo se ? la prima volta o se il numero delle
--                  righe di dettaglio ? maggiore di 150. Si legge con lock il prossimo numeratore del buono di scarico
--                  creando lo stesso se non esiste per esercizio e pg_inventario.
--               Si crea il dettaglio del buono di scarico, settando se questo avr? impatto nella gestione coge (solo
--                  per trasferimenti altro inventario).
--               Si aggiorna il bene origine portandolo a totalmente scaricato.
--               Si crea la testata del buono di carico. E' generata solo se ? la prima volta o se il numero delle
--                  righe di dettaglio ? maggiore di 150. Si legge con lock il prossimo numeratore del buono di ccarico
--                  creando lo stesso se non esiste per esercizio e pg_inventario.
--               Si crea il dettaglio del buono di carico, settando se questo avr? impatto nella gestione coge (solo
--                  per trasferimenti altro inventario).
...
--                     un dato pg_inventario. Se il valore tornato ? zero allora si torna il numero iniziale a partire
...
--         In uscita, se la matrice errori ? piena, si segnala la terminazione con successo con alcuni errori. Il
--         trasferimento ? sempre processato per intero ed ? quindi possibile che alcuni beni siano trasferiti mentre
...
BEGIN
   ...
   aMessaggioBaseSegnala:='Attenzione, per i seguenti beni non ? stato modificato automaticamente ' ||
		 ...
		 End If;
            ...
            ELSE
               ...
               WHERE  pg_inventario= riga_group_invetario_beni_apg.pg_inventario AND
                      nr_inventario= riga_group_invetario_beni_apg.nr_inventario AND
                      progressivo = riga_group_invetario_beni_apg.progressivo;
                 -- r.p. 13/02/2006 da verificare per segnalare i beni per cui non ? stato possibile
                 -- aggiornare l'imponibile ammortamento
                 -- r.p. 7/09/2006 che non abbiano gi? completato l'ammortamento
...
-- Si elimina il flag totalmente scaricato se l'azione ? avvenuta in esercizi successivi
-- Si assestano i saldi (imponibile ammortamento) togliendo tutto quanto ? stato movimentato da buoni
-- di carico e scarico in esercizio successivo. Questa funzione ? fatta solo nel caso per i beni per
-- i quali risulta verificata l'uguaglianza valore_iniziale + variazioni pi? - variazioni meno = imponibile ammortamento
-- =================================================================================================
PROCEDURE upgRecBenePerAmmortamento
   (
    aRecVAmmortamentoBeni IN OUT V_AMMORTAMENTO_BENI%ROWTYPE,
...
exception when no_data_found then
  aStorno:=0;
   ...
   -- Se l'uguaglianza valore_iniziale + variazioni pi? - variazioni meno = imponibile ammortamento ?
...
BEGIN
 ...
 For aBeneInv In(Select * From INVENTARIO_BENI
  ...
  End Loop;
   ...
   -------------------------------------------------------------------------------------------------
   -- Controllo eseguibilit? dell'ammortamento
   ...
   BEGIN
      ...
      LOOP
         ...
         BEGIN
            ...
            LOOP
	    	  ...
	    	  IF ChkBeneAmmortato(aRecVAmmortamentoBeni, aEs) THEN
                     ...
                     -- tale tipo non ? definito per il bene si verifica che lo sia e univocamente per
                     ...
                     IF (aRecVAmmortamentoBeni.valore_ammortizzato > aRecVAmmortamentoBeni.imponibile_ammortamento) And
                        ...
                        IBMERR001.RAISE_WRN_GENERICO('Il valore ammortizzato ? maggiore dell''imponibile ammortamento');
                     END IF;
		...
		begin
                        ...
                        -- Controllare se il bene pu? essere ammortizzato dell'importo della rata calcolata precedentemente.
                        -- Si posssono verificare i seguenti casi:
                        --   a) la rata di ammortamento calcolata con la percentuale stabilita ? troppo
                        ...
                        --   b) la rata di ammortamento calcolata con la percentuale stabilita ? consistente in quanto risulta:
 		...
 		EXCEPTION  When others THEN
			...
			dbms_output.put_line('errore '||aRecVAmmortamentoBeni.nr_inventario);
	    	  ...
	    	  ELSE -- Controllo se il bene risulta gi? ammortizzato nell'esercio in esame;

                     IBMERR001.RAISE_ERR_GENERICO('Il bene risulta aver subito ammortamento nell''anno ' || aEs );

                  END IF; -- Controllo se il bene risulta gi? ammortizzato nell'esercio in esame;
...
-- =================================================================================================
-- Controllo se esiste un tipo di ammortamento associato al gruppo categoria. Se tale tipo non ?
   ...
   IF aRecVAmmortamentoBeni.cd_tipo_ammortamento IS NULL THEN
      ...
      EXCEPTION
         ...
         WHEN too_many_rows THEN
              IBMERR001.RAISE_ERR_GENERICO('Tipo ammortamento non definito per il bene: il gruppo inventariale a cui il bene appartiene ha pi? di un tipo di ammortamento definito');
         WHEN no_data_found THEN
      ...
      LOOP
         ...
         IF inPgInventarioOri = aPgInventarioDest THEN
            ...
            IF esisteBenePrincipale = 'Y' THEN
               ...
               -- Tutti i beni devono diventare accessori dello stesso bene di destinazione e questo non pu?
               -- essere nullo

               IF (beniDaTrasf_tab(i).tPgInventarioPrincipale IS NULL OR
                   beniDaTrasf_tab(i).tNrInventarioPrincipale IS NULL) THEN
                  IBMERR001.RAISE_ERR_GENERICO
                     ('Trasferimento nello stesso inventario. E'' indicato un bene principale e non ? richiesto ' ||
                      'il trasferimento come bene principale e non ? indicato il bene di destinazione');
               END IF;
               ...
               IF (beniDaTrasf_tab(i).tPgInventarioPrincipale != beniDaTrasf_tab(1).tPgInventarioPrincipale OR
                  ...
                  IBMERR001.RAISE_ERR_GENERICO
                     ('Trasferimento nello stesso inventario. E'' indicato un bene principale e non ? richiesto ' ||
                      'il trasferimento come bene principale e il bene di destinazione non ? lo stesso per tutti i records');
               END IF;
            ...
            ELSE
               ...
               -- Se l'accessorio non ? da trasferire come principale deve esistere l'indicazione del bene
               -- di destinazione
               ...
               IF beniDaTrasf_tab(i).tFlTrasfComePrincipale = 'N' THEN
                  IF (beniDaTrasf_tab(i).tPgInventarioPrincipale IS NULL OR
                      beniDaTrasf_tab(i).tNrInventarioPrincipale IS NULL) THEN
                     IBMERR001.RAISE_ERR_GENERICO
                        ('Trasferimento nello stesso inventario. Non ? indicato un bene principale e non ? richiesto ' ||
                         'il trasferimento come bene principale e non ? indicato il bene di destinazione');
                  END IF;
               END IF;
            END IF;
         ...
         -- La testata del buono di scarico ? generata solo se ? la prima volta o se il numero delle
         -- righe di dettaglio ? maggiore di 200
         ...
         -- Riporti dati utilizzatori (solo per trasferimento stessa UO e se la destinazione ? bene principale)
...
BEGIN
   ...
   -- Se l'esercizio di riferimento ? 2003 sollevo errore

   IF inEsercizio < 2004 THEN
      IBMERR001.RAISE_ERR_GENERICO
         ('Errore generale. Non ? possibile eseguire il trasferimento beni in esercizio anteriore al 2004');
   END IF;
   ...
   BEGIN
      ...
      LOOP
         ...
         BEGIN
            ...
            -- Se il numero inventario ? diverso da quello memorizzato allora indico l'esecuzione del
            -- trasferimento
            ...
            -- Se ho gi? portato dei beni sulla tabella di appoggio e la prima occorrenza non ? un bene
            -- principale, anche se il numero inventario ? uguale al precedente, indico l'esecuzione del
...
BEGIN
   ...
   -------------------------------------------------------------------------------------------------
   -- La funzione di trasferimento beni ? attivata come batch
   ...
   IBMERR001.RAISE_ERR_GENERICO
       ('Operazione in corso. Al completamento l''utente ricever? un messaggio di notifica ' ||
...
Begin
   ...
   -------------------------------------------------------------------------------------------------
   -- La funzione di ammortamento dei beni ? attivata come batch
