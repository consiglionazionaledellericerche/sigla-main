--------------------------------------------------------
--  DDL for Package Body CNRCTB240
--------------------------------------------------------
...
BEGIN
   ...
   -- L'attributo LIQUID_IVA_INTERF.pg_caricamento è posto uguale a 0 (ZERO)
   ...
   -- è eseguito solo in caso di liquidazione definitiva in quanto quella provvisoria è riferita ad una
   ...
   BEGIN
      ...
      IF flEsistePeriodo = 1 THEN
         IBMERR001.RAISE_ERR_GENERICO
            ('La liquidazione non è ammessa in quanto esiste già la liquidazione del centro in stato ' ||
      ...
      IF (flEsistePeriodoPrecedente = 0 AND
         ...
         IBMERR001.RAISE_ERR_GENERICO
            ('La liquidazione non è ammessa in quanto la liquidazione centro, pur non risultando ' ||
   ...
   BEGIN
      ...
      LOOP
         ...
         -- La liquidazione può essere eseguita se :
      ...
      IF flFattaLiquidazione = 'N' THEN
         IBMERR001.RAISE_ERR_GENERICO
           ('Non è presente alcun record di liquidazione iva interfaccia in stato da elaborare');
      END IF;
