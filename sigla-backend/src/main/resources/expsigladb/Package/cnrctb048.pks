...
-- porta avanti/indietro di residui e compatibilit? con storico
--
-- Date: 22/07/2003
-- Version: 1.4
-- Fix getPgVerRec
...
--
-- Controllo sulla validit? del riporto dall'esercizio successivo di documento contabile
--
-- pre-post-name: annotazione su partita di giro non valida
-- pre: l'annotazione ? su partita di giro, ma risultano pi? di un dettaglio o di una scadenza,
...
-- pre: il documento contabile ? stato modificato nel nuovo esercizio, (v. CNRCTB048.isDocModificato)
--	il documento contabile risulta modificato se:
--	   - l'importo o lo stato dell'obbligazione ? stato modificato, oppure il documento ? stato annullato;
--	   - se al documento nel nuovo es. ? stata aggiunta una scadenza, (la scadenza non ha ori riporto);
--	   - la dt_scadenza di una delle scadenze ? cambiata;
--	   - ? cambiata la testata di una scadenza.
--         - se al documento nel nuovo es. ? stata aggiunta una modifica, (record in OBBLIGAZIONE_MODIFICA/ACCERTAMENTO_MODIFICA);
-- post: viene sollevata un'eccezione
--
-- pre-post-name: controllo sulla validit? del riporto dall'esercizio successivo di documento contabile.
...
--
-- Controllo sulla validit? del riporto dall'esercizio successivo delle scadenze di documento contabile
--
...
-- pre: il documento contabile ? associato a documenti amministrativi emessi nel nuovo esercizio
-- post: viene sollevata un'eccezione
--
-- pre-post-name: documento contabile associato a documento amministrativo pagato
-- pre: il documento contabile ? associato a documenti amministrativi (in parte) pagato nel nuovo esercizio
-- post: viene sollevata un'eccezione
--
...
-- post: viene sollevata un'eccezione
--
-- pre-post-name: controllo sulla validit? del riporto dall'esercizio successivo di scadenze di un documento contabile
-- pre: Nessun'altra precondizione verificata
...
-- pre: l'accertamento ? associato almeno ad un documento generico di versamento cori
-- post: viene sollevato un errore
--
-- pre-post: controlli per doc amm associati ad accertamento da riportare al nuovo esercizio -
--           doc amm da contabilizzare in economica
-- pre: l'accertamento ? associato almeno ad un doc amm da contabilizzare in economica
-- post: viene sollevato un errore
--
-- pre-post: controlli per doc amm associati ad accertamento da riportare al nuovo esercizio -
--           doc amm da contabilizzare in analitica
-- pre: l'accertamento ? associato almeno ad un doc amm da contabilizzare in analitica
-- post: viene sollevato un errore
--
-- pre-post: controlli per doc amm associati ad accertamento da riportare al nuovo esercizio -
...
--                documento gi? riportato
-- pre: l'accertamento che deve essere riportato risulta gi? riportato
-- post: viene sollevato un errore
--
-- pre-post name: controlli per accertamento da riportare all'esercizio successivo -
--                documento ha importo nullo
-- pre: il documento non ? su partita di giro, oppure apre la partita di giro, ed ha
--      importo nullo in testata
-- post: viene sollevato un errore
--
-- pre-post name: controlli per accertamento da riportare all'esercizio successivo -
--                documento non ribaltabile
-- pre: il documento contabile non ? ribaltabile (vd. IsEligibileRibalt)
-- post: viene sollevato un errore
--
-- pre-post name: controlli per accertamento da riportare all'esercizio successivo -
--                documenti amministrativi associati non ribaltabili
-- pre: il documento contabile ? associato a documenti amministrativi particolari per
--      cui non ? ribaltabile (vd. pre-post checkNoRiporta() )
-- post: viene sollevato un errore
--
-- pre-post name: controlli per accertamento da riportare all'esercizio successivo -
--                terzo non valido
-- pre: il terzo associato al documento contabile ha un data di fine validit? precedente
--      all'esercizio su cui si ribalta
-- post: viene sollevato un errore
--
-- pre-post name: controlli per accertamento da riportare all'esercizio successivo -
--                capitolo finanziario non valido sul nuovo esercizio
-- pre: deve essere controllata la validit? della voce_f nel nuovo esercizio, non esiste
--      una voce_f corrispondente nel nuovo esercizio
-- post: viene sollevato un errore
--
-- pre-post name: controlli per accertamento da riportare all'esercizio successivo -
--                unit? organizzativa non valida nel nuovo esercizio
-- pre: l'unit? organizzativa del documento non ? valida nel nuovo esercizio
-- post: viene sollevato un errore
--
-- pre-post name: controlli per accertamento da riportare all'esercizio successivo
-- pre: nessuna delle precedenti precondizioni
...
-- controlloVoce -> boolean, se cambia imputazione finanziaria non ? necessario verificare
--                  la validit? del capitolo voce_f
...
--                documento gi? riportato
-- pre: l'obbligazione che deve essere riportata risulta gi? riportata
-- post: viene sollevato un errore
--
-- pre-post name: controlli per obbligazione da riportare all'esercizio successivo -
--                documento ha importo nullo
...
-- restituisce true se l'accertamento ? stato completamente incassato
     ...
     --Verifico se sono state aggiunte modifiche ai residui (record in OBBLIGAZIONE_MODIFICA)
	  ...
	  for aScadNew in (select * from obbligazione_scadenzario
		...
		if aScadNew.pg_obbl_scad_ori_riporto is null then
		   return 'Y';  -- se una scadenza non ha ori riporto, ? stata aggiunta
		end if;
		...
		then
			  if not (aScadNew.im_scadenza = aScad.im_scadenza - aScad.im_associato_doc_contabile
                ...
                -- S.F. 25/02/2009 Eliminato il controllo perch? la data di scadenza non ? una informazione sostanziale per
...
*/
	  ...
	  -- S.F. 25/02/2009 In assenza di uno qualsiasi dei casi in cui il documento ? cambiato ritorna "N"
          return 'N';

  else  -- ? cambiata la testata
  	  return 'Y';
  end if;
end;
...
*/
  ...
  Else  -- ? cambiata la testata (IL TERZO DELLA TESTATA)
  	  Return 'Y';
  End If;
End;
 ...
 -- l'esercizio corrente ? aperto, esiste l'esercizio successivo
...
--    IBMERR001.RAISE_ERR_GENERICO('L''esercizio corrente '||aEs||' ? chiuso per il cds:'||aCdCds);
--   end if;
  begin
  	   select 1 into aNum
	   from dual
	   where exists (select 1 from esercizio
	                 where cd_cds = aCdCds
					   and esercizio = aEs + 1);
  exception when no_data_found then
       ibmerr001.RAISE_ERR_GENERICO('L''esecizio '||(aEs+1)||'non ? definito per il cds '||aCdCds);
  end;
 end;
    ...
    for aDocAmmRiga in (select * from V_DOC_AMM_OBB
          ...
          if not (aDocAmmRiga.ti_associato_manrev = CNRCTB100.TI_NON_ASSOC_MAN_REV) then
		   ...
		   begin
			...
			-- allora l'obbligazione non pu? essere riportata indietro
            If cnrutil.isLabelObbligazione() Then
            ...
            End If;
		   exception when no_data_found then
		   	null; -- se il mandato (annullato) ? sulla vecchia scadenza allora si pu? tornare indietro
		   end;
          end if;
	end loop;
  end loop;
 ...
 begin
  ...
  Begin
   ...
   If FL_PAR = 'N' Then
     ibmerr001.RAISE_ERR_GENERICO('Il CDS '||aObbNext.CD_CDS||' non ? abilitato a riportare indietro documenti dall''esercizio '||aObbNext.ESERCIZIO||'.');
   End If;
  ...
  If aObb.fl_pgiro = 'Y' and not (aObb.cd_tipo_documento_cont in (CNRCTB018.TI_DOC_IMP_RES, CNRCTB018.TI_DOC_IMP)) Then
	  ...
	  if not (aObb.riportato = 'Y' or aAcc.riportato = 'Y') then
	  	 ibmerr001.RAISE_ERR_GENERICO('L''annotazione su partita di giro non ? riportata '||CNRCTB035.getDesc(aObb));
	  end if;
  Else
	  if not (aObb.riportato = 'Y') then
          If cnrutil.isLabelObbligazione() Then
            ibmerr001.RAISE_ERR_GENERICO('L''obbligazione non ? riportata '||CNRCTB035.getDesc(aObb));
       	  Else
            ibmerr001.RAISE_ERR_GENERICO('L''impegno non ? riportato '||CNRCTB035.getDesc(aObb));
      	  End If;
	  end if;
  End If;
  ...
  -- verifico se il documento ? stato modificato nel nuovo esercizio rispetto a quanto riportato

  if CNRCTB048.isDocModificato(aObb,aObbNext) = 'Y' then
    If cnrutil.isLabelObbligazione() Then
      ibmerr001.RAISE_ERR_GENERICO('L''obbligazione ? stata modificata nel nuovo esercizio, non pu? essere riportata indietro '||CNRCTB035.GETDESC(aObbNext));
    Else
      ibmerr001.RAISE_ERR_GENERICO('L''impegno ? stata modificato nel nuovo esercizio, non pu? essere riportato indietro '||CNRCTB035.GETDESC(aObbNext));
    End If;
  end if;
    ...
    for aDocAmmRiga in (select * from V_DOC_AMM_ACC
        ...
        if not (aDocAmmRiga.ti_associato_manrev = CNRCTB100.TI_NON_ASSOC_MAN_REV) then
		 ...
		 begin
		  ...
		  -- nel nuovo esercizio, non ? possibile tornare indietro
		 ...
		 exception when NO_DATA_FOUND then
		  null;  -- se la reversale (annullata) ? sulla vecchia scadenza allora posso tornare indietro
		 end;
        end if;
	end loop;
  end loop;
 ...
 begin
  ...
  Begin
   ...
   If FL_PAR = 'N' Then
     ibmerr001.RAISE_ERR_GENERICO('Il CDS '||aAccNext.CD_CDS||' non ? abilitato a riportare indietro documenti dall''esercizio '||aAccNext.ESERCIZIO||'.');
   End If;
  ...
  then
	  ...
	  if not (aAcc.riportato = 'Y') then
	   ibmerr001.RAISE_ERR_GENERICO('L'''||cnrutil.getLabelObbligazioneMin()||' non ? riportata '||CNRCTB035.getDesc(aObb));
	  end if;
  end if;
  ...
  -- verifico se il documento ? stato modificato nel nuovo esercizio
  -- rispetto a quanto riportato
  if CNRCTB048.isDocModificato(aAcc,aAccNext) = 'Y' then
    ibmerr001.RAISE_ERR_GENERICO('L''accertamento ? stato modificato nel nuovo esercizio, non pu? essere riportato indietro '||CNRCTB035.GETDESC(aAccNext));
  end if;
 end;
...
For aDocAmm In (Select Distinct cd_tipo_documento_amm, cd_cds, esercizio, cd_unita_organizzativa, pg_documento_amm,
    ...
    If aDocAmm.stato_pagamento_fondo_eco <> CNRCTB100.STATO_NO_PFONDOECO Then
	 -- se il doc amm ? assegnato a fondo ma non ancora associato deve essere
	 -- possibile riportare il documento contabile
	 -- se il doc amm ? gi? legato a spesa del fondo, non si pu? riportare
	 Begin
	 	 select 1 into aNum
		 from dual
		 where exists (select 1
    ...
    If aDocAmm.cd_tipo_documento_amm in (CNRCTB100.TI_GEN_CORI_VER_SPESA) then
           IBMERR001.RAISE_ERR_GENERICO('L'''||cnrutil.getLabelObbligazioniMin()||' '||CNRCTB035.GETDESC(aObb)||' ? gestito in automatico alla liquidazione CORI o ? di liquidazione IVA');
    End If;
...
--    End If;
   ...
   For aGruppoCentro in (Select *
     ...
     IBMERR001.RAISE_ERR_GENERICO('L'''||cnrutil.getLabelObbligazioneMin()||' '||CNRCTB035.GETDESC(aObb)||' risulta parte di liquidazione CORI accentrata (l''obbligazione ? presente in LIQUID_GRUPPO_CENTRO)');
   End Loop;
   ...
   For aGruppoCentro in (Select *
                         ...
                         Where cd_cds_obb_accentr = aObb.cd_cds And
                               ...
                               esercizio_ori_obb_accentr=aObb.esercizio_originale And
                               pg_obb_accentr=aObb.pg_obbligazione) Loop
    IBMERR001.RAISE_ERR_GENERICO('L'''||cnrutil.getLabelObbligazioneMin()||' '||CNRCTB035.GETDESC(aObb)||' risulta parte di liquidazione IVA al centro (l''obbligazione ? presente in LIQUIDAZIONE_IVA_CENTRO)');
   End Loop;
 ...
 begin
  for aDocAmm in (select distinct
    ...
    if aDocAmm.cd_tipo_documento_amm in (CNRCTB100.TI_GEN_CORI_VER_ENTRATA) then
     IBMERR001.RAISE_ERR_GENERICO('L''accertamento '||CNRCTB035.GETDESC(aAcc)||' ? gestito in automatico alla liquidazione CORI');
	end if;
...
begin
  Begin
   ...
   If FL_PAR = 'N' Then
     ibmerr001.RAISE_ERR_GENERICO('Il CDS '||aAcc.CD_CDS||' non ? abilitato a riportare avanti documenti dall''esercizio '||aAcc.ESERCIZIO||'.');
   End If;
   ...
   If aAcc.cd_tipo_documento_cont = CNRCTB018.TI_DOC_ACC_RES And FL_RIA = 'Y' And aAcc.STATO_RESIDUO is null and aAcc.FL_PGIRO='N' Then
     ibmerr001.RAISE_ERR_GENERICO('L''accertamento '||CNRCTB035.GETDESC(aAcc)||' ? un documento privo dello stato. Ribaltamento non possibile!');
   End If;
  ...
  End;
	...
	if aAcc.riportato = 'Y' then
	   aErrMsn := 'L''accertamento '||CNRCTB035.GETDESC(aAcc)||' ? un documento gi? riportato nel nuovo esercizio';
	   ibmerr001.RAISE_ERR_GENERICO(aErrMsn);
	end if;

	-- verifico se il documento ha importo nullo
	-- se non ? pgiro, o se origine di pgiro
	...
	if controlloRibalt then
		if isEligibileRibalt(CNRCTB001.GESTIONE_ENTRATE,
 		     ...
 		     aErrMsn := 'L''accertamento '||CNRCTB035.GETDESC(aAcc)||' non ? un documento ribaltabile. '||
...
' Verificare che le reversali associate alle scadenze riscosse dell''Accertamento siano totalmente riscontrate. La quota non riscontrata risulta di '||Ltrim(Rtrim(To_Char(getImPagRiscNonRiscontrato(aAcc.cd_cds, aAcc.esercizio, aAcc.esercizio_originale, aAcc.pg_accertamento, 'E'), '999g999g999g999g999g990d00')))||'.';
		     ibmerr001.RAISE_ERR_GENERICO(aErrMsn);
                  Else
		   aErrMsn := 'L''accertamento '||CNRCTB035.GETDESC(aAcc)||' non ? un documento ribaltabile per cause generiche.';
		   ibmerr001.RAISE_ERR_GENERICO(aErrMsn);
                  End If;
		end if;
	end if;
	...
	-- verifico se il documento ? associato a pratiche CORI, IVA, FONDO ECONOMALE
	checkNoRiporta(aAcc);

	-- verifico la validit? del terzo nell'anno su cui si ribalta
	select dt_fine_rapporto into aDt
	from terzo
	where cd_terzo = aAcc.cd_terzo;
	if to_number(to_char(aDt,'YYYY')) < aEs then
	   aErrMsn := 'L''accertamento '||CNRCTB035.GETDESC(aAcc)||' non pu? essere riportato: terzo '||aAcc.cd_terzo||' non valido nell''esercizio '||aEs;
	   ibmerr001.RAISE_ERR_GENERICO(aErrMsn);
	end if;

	-- verifico la validit? della voce_f sul nuovo esercizio
	...
	if controlloVoce then
        ...
        begin
		  ...
		  End If;
		exception when NO_DATA_FOUND then
			aErrMsn := 'L''accertamento '||CNRCTB035.GETDESC(aAcc)||' non pu? essere riportato: capitolo '||aAcc.cd_voce||' non esistente sull''esercizio '||aEs;
			ibmerr001.RAISE_ERR_GENERICO(aErrMsn);
		end;
	end if;

	-- verifico la validit? dell'unit? organizzativa sul nuovo esercizio
...
Begin
  ...
  Begin
   ...
   If FL_PAR = 'N' Then
     ibmerr001.RAISE_ERR_GENERICO('Il CDS '||aObb.CD_CDS||' non ? abilitato a riportare avanti documenti dall''esercizio '||aObb.ESERCIZIO||'.');
   End If;
  ...
  End;
	...
	if aObb.riportato = 'Y' then
	   aErrMsn := 'L'''||cnrutil.getLabelObbligazioneMin()||' '||CNRCTB035.GETDESC(aObb)||' ? un documento gi? riportato nel nuovo esercizio';
	   ibmerr001.RAISE_ERR_GENERICO(aErrMsn);
	end if;
	...
	then
        ...
        -- verifico se il documento ? provvisorio
        If aObb.stato_obbligazione = cnrctb035.STATO_PROVVISORIO and aObb.FL_GARA_IN_CORSO ='N' then
         aErrMsn := 'L'''||cnrutil.getLabelObbligazioneMin()||' '||CNRCTB035.GETDESC(aObb)||' ? un documento Provvisorio';
         ibmerr001.RAISE_ERR_GENERICO(aErrMsn);
        End If;
	...
	if controlloRibalt then
		if isEligibileRibalt(CNRCTB001.GESTIONE_SPESE,
		   ...
		   aErrMsn := 'L'''||cnrutil.getLabelObbligazioneMin()||' '||CNRCTB035.GETDESC(aObb)||' non ? un documento ribaltabile. '||
...
' Verificare che i mandati associati alle scadenze pagate dell''Impegno siano totalmente riscontrati. La quota non riscontrata risulta di '||Ltrim(Rtrim(To_Char(getImPagRiscNonRiscontrato(aObb.cd_cds, aObb.esercizio, aObb.esercizio_originale, aObb.pg_obbligazione, 'S'), '999g999g999g999g999g990d00')))||'.';
		   ibmerr001.RAISE_ERR_GENERICO(aErrMsn);
                  Else
		   aErrMsn := 'L'''||cnrutil.getLabelObbligazioneMin()||' '||CNRCTB035.GETDESC(aObb)||' non ? un documento ribaltabile per cause generiche.';
		   ibmerr001.RAISE_ERR_GENERICO(aErrMsn);
                  End If;
		end if;
	end if;
	...
	-- verifico la validit? del terzo sull'esercizio su cui si ribalta
	select dt_fine_rapporto into aDt
	from terzo
	where cd_terzo = aObb.cd_terzo;

	if to_number(to_char(aDt,'YYYY')) < aEs then
	   aErrMsn := 'L'''||cnrutil.getLabelObbligazioneMin()||' '||CNRCTB035.GETDESC(aObb)||' non pu? essere riportata: terzo '||aObb.cd_terzo||' non valido nell''esercizio '||aEs;
	   ibmerr001.RAISE_ERR_GENERICO(aErrMsn);
	end if;
...
aPgNext number;
begin
-- SF 21.11.2006 con la nuova gestione dell'esercizio originale il numero del documento non cambia pi?.
...
begin
	...
	if aObb.cd_tipo_documento_cont in (CNRCTB018.TI_DOC_IMP, CNRCTB018.TI_DOC_IMP_RES, CNRCTB018.TI_DOC_OBB_PGIRO, CNRCTB018.TI_DOC_OBB_PGIRO_RES) then
	    -- non dipende dalla linea di attivit?, che ? fittizia
	    select * into aVoce
		from voce_f
		where esercizio    	  = aEs
...
begin
 if aObbNext.cd_tipo_documento_cont In (CNRCTB018.TI_DOC_IMP_RES, CNRCTB018.TI_DOC_OBB_RES_PRO, CNRCTB018.TI_DOC_OBB_PGIRO_RES) then
   ...
   where cd_cds    = aObbNext.cd_cds
     ...
     aNum := 1; -- per competenza la numerazione ? sempre rigenerata
                -- non capiter? pi? ! 04.01.2006 stani
 end if;
 return aNum;
end;
...
-- dal 2006 restituisce sempre lo stesso pg_documento dell'esercizio precedente (praticamente ? inutile la chiamata)
...
-- 	aAccDettScadNext.CD_LINEA_ATTIVITA		  	 :=aDettScad.CD_LINEA_ATTIVITA;
 ...
 begin
  ...
  exception
   ...
   when TOO_MANY_ROWS then
    IBMERR001.RAISE_ERR_GENERICO('Esiste pi? di un documento nel nuovo esercizio corrispondente al documento:'||CNRCTB035.getDesc(aObb));
  end;
  return aObbNew;
 end;
 ...
 begin
  ...
  exception
   ...
   when TOO_MANY_ROWS then
    IBMERR001.RAISE_ERR_GENERICO('Esiste pi? di un documento nel nuovo esercizio corrispondente al documento:'||CNRCTB035.getDesc(aAcc));
  end;
  return aAccNew;
 end;
 ...
 begin
  ...
  exception
   ...
   when TOO_MANY_ROWS then
    IBMERR001.RAISE_ERR_GENERICO('Esiste pi? di un documento di origine corrispondente al documento:'||CNRCTB035.getDesc(aObbNew));
  end;
  return aObb;
 end;
 ...
 begin
  ...
  exception
   ...
   when TOO_MANY_ROWS then
    IBMERR001.RAISE_ERR_GENERICO('Esiste pi? di un documento di origine corrispondente al documento:'||CNRCTB035.getDesc(aAccNew));
  end;
  return aAcc;
 end;
