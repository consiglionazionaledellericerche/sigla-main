...
-- pre: è stato creato un batch dinamico
...
-- -----------------------
-- pre-post name: annullamento massivo di documenti contabili - risorsa non più valida
-- pre: il documento contabile in processo è stato nel frattempo modificato
...
-- pre: l'esercizio del documento non è aperto per il cds
-- post: viene creata una riga di log di errore. La procedura procede al successivo,
--       se esiste, documento da processare
--
--
...
-- pre: il documento contabile in processo è ancora valido, risulta riportato all'esercizio
...
-- pre: il documento contabile in processo è ancora valido, non può essere annullato
-- post: il documento viene riportato all'esercizio di partenza se risulta già riportato, viene
...
-- pre: nessuna delle precedenti precondizioni è soddisfatta
-- post: il documento viene riportato all'esercizio di partenza se risulta già riportato,
		...
		for aDocCont in (select * from vsx_chiusura
			...
			begin
				if aDocCont.TI_GESTIONE = CNRCTB001.GESTIONE_ENTRATE then
					...
					-- verifica della validità della risorsa
					-- lock del documento
					...
					-- se il documento è riportato, viene riportato indietro
					-- e quindi annullato
				...
				commit;
				ibmutl200.loginf(aPg_exec,DS_ANNULLAMENTO||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ',aDescDocCont||': è stato annullato','');
...
-- ----------------------
-- pre-post name: riporto all'esercizio successivo - risorsa non più valida
-- pre: il documento contabile in processo è stato nel frattempo modificato
...
-- pre: il documento contabile in processo è ancora valido, viene sollevato un errore
...
Begin
  	...
  	if aDocCont.TI_GESTIONE = CNRCTB001.GESTIONE_ENTRATE then  -- accertamenti
 		aAcc.cd_cds := aDocCont.cd_cds;
  		...
  		aAcc.esercizio_originale := aDocCont.esercizio_ori_acc_obb;
		aAcc.pg_accertamento := aDocCont.pg_acc_obb;
  		-- verifica della validità del documento e lock della risorsa
  		CNRCTB035.LOCKDOCFULLCHECK(aAcc,aDocCont.pg_ver_rec_doc);
  		aDescDocCont := 'Accertamento '||CNRCTB035.GETDESC(aAcc);
	  ...
	  aAccNew := cnrctb048.GETDOCRIPORTATO(aAcc);
		...
		begin
		  ...
		  where exists (select 1 from accertamento_scad_voce
  		  ...
  		  ibmutl200.LOGINF_TEMP(aPg_exec,DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ','Accertamento '||aDescDocCont||': è stato riportato al nuovo esercizio con linea di attività di sistema. Importo accertamento origine '||aAcc.im_accertamento||' importo riportato '||aAccNew.im_accertamento,'', 'N');
		exception when NO_DATA_FOUND then
		  ibmutl200.LOGINF_TEMP(aPg_exec,DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ','Accertamento '||aDescDocCont||': è stato riportato al nuovo esercizio. Importo accertamento origine '||aAcc.im_accertamento||' importo riportato '||aAccNew.im_accertamento,'', 'N');
		end;
	     Elsif aAcc.cd_tipo_documento_cont in (CNRCTB018.TI_DOC_ACC, CNRCTB018.TI_DOC_ACC_PLUR) then
		   ibmutl200.LOGINF_TEMP(aPg_exec,DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ','Accertamento '||aDescDocCont||': è stato riportato al nuovo esercizio. Importo accertamento origine '||aAcc.im_accertamento||' importo riportato '||aAccNew.im_accertamento,'', 'N');
	     Else  -- ACR_PGIRO
	           ibmutl200.LOGINF_TEMP(aPg_exec,DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ','Annotazione su partita di giro '||aDescDocCont||': è stato riportata al nuovo esercizio. Importo accertamento origine '||aAcc.im_accertamento||' importo riportato '||aAccNew.im_accertamento,'', 'N');
	     End if;

  	else -- spese
		aObbNew := cnrctb048.GETDOCRIPORTATO(aObb);
		if aObb.cd_tipo_documento_cont = CNRCTB018.TI_DOC_OBB_PGIRO then
		   ibmutl200.LOGINF_TEMP(aPg_exec,DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ','Annotazione su partita di giro '||aDescDocCont||': è stato riportata al nuovo esercizio con numero '||aObbNew.pg_obbligazione||'. Importo '||cnrutil.getLabelObbligazioneMin()||' origine '||aObb.im_obbligazione||' importo riportato '||aObbNew.im_obbligazione,'', 'N');
		elsif aObb.cd_tipo_documento_cont = CNRCTB018.TI_DOC_OBB then
		   ibmutl200.LOGINF_TEMP(aPg_exec,DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ',cnrutil.getLabelObbligazione()||' '||aDescDocCont||': è stato riportata al nuovo esercizio con numero '||aObbNew.pg_obbligazione||'. Importo '||cnrutil.getLabelObbligazioneMin()||' origine '||aObb.im_obbligazione||' importo riportato '||aObbNew.im_obbligazione,'', 'N');
		elsif aObb.cd_tipo_documento_cont = CNRCTB018.TI_DOC_OBB_RES_PRO then
		   ibmutl200.LOGINF_TEMP(aPg_exec,DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ',cnrutil.getLabelObbligazione()||' di tipo "Residuo proprio" '||aDescDocCont||': è stato riportata al nuovo esercizio con numero '||aObbNew.pg_obbligazione||'. Importo '||cnrutil.getLabelObbligazioneMin()||' origine '||aObb.im_obbligazione||' importo riportato '||aObbNew.im_obbligazione,'', 'N');
		elsif aObb.cd_tipo_documento_cont = CNRCTB018.TI_DOC_OBB_RES_IMPRO then
		   ibmutl200.LOGINF_TEMP(aPg_exec,DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ',cnrutil.getLabelObbligazione()||' di tipo "Residuo improprio" '||aDescDocCont||': è stato riportata al nuovo esercizio con numero '||aObbNew.pg_obbligazione||'. Importo '||cnrutil.getLabelObbligazioneMin()||' origine '||aObb.im_obbligazione||' importo riportato '||aObbNew.im_obbligazione,'', 'N');
		else -- impegni
		   ibmutl200.LOGINF_TEMP(aPg_exec,DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ','Impegno '||aDescDocCont||': è stato riportata al nuovo esercizio con numero '||aObbNew.pg_obbligazione||'. Importo '||cnrutil.getLabelObbligazioneMin()||' origine '||aObb.im_obbligazione||' importo riportato '||aObbNew.im_obbligazione,'', 'N');
		end if;
	end if;
...
-- mette una serie di variabili a 0
	...
	begin
		...
		delete_vsx(aPgCall);
-- stani: perchè non committa dentro
                IBMUTL001.DEFERRED_COMMIT;
		Str := riassunto;
		-- Messaggio all'utente di operazione completata
...
' "Funzionalità di servizio/Logs Applicativi/Visualizzazione" per il Progressivo Esecuzione '||
		    pg_exec||')',
		    aUser);
  Else
		IBMUTL205.LOGINF(DS_RIPORTO_NEXT,
...
-- ----------------------
-- pre-post name: riporto all'esercizio origine - risorsa non più valida
-- pre: il documento contabile in processo è stato nel frattempo modificato
-- post: viene creata una riga di log di errore. La procedura procede al successivo,
--       se esiste, documento da processare
--
	...
	for aCds in (select * from v_unita_organizzativa_valida
		...
		for aDocCont in (select * from vsx_chiusura
			...
			begin
				...
				commit;
				ibmutl200.LOGINF(aPg_exec,DS_RIPORTO_PREV||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ',aDescDocCont||': è stato riportato all''esercizio precedente','');
			exception when OTHERS then
...
begin
	...
	else -- è una voce_f, solo per impegni non pgiro
	   begin
	   	   select * into aVoceF
		   from voce_f
		   where esercizio        = aEsDoc + 1
				...
				if aDocCont.TI_GESTIONE = CNRCTB001.GESTIONE_ENTRATE then
					...
					if aAcc.cd_tipo_documento_cont = CNRCTB018.TI_DOC_ACC_RES then
						...
						begin
							...
							ibmutl200.LOGINF(aPg_exec,DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ','Accertamento '||aDescDocCont||': è stato riportato al nuovo esercizio con linea di attività di sistema. Importo accertamento origine '||aAcc.im_accertamento||' importo riportato '||aAccNew.im_accertamento,'');
						exception when NO_DATA_FOUND then
							ibmutl200.LOGINF(aPg_exec,DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ','Accertamento '||aDescDocCont||': è stato riportato al nuovo esercizio. Importo accertamento origine '||aAcc.im_accertamento||' importo riportato '||aAccNew.im_accertamento,'');
						end;
					elsif aAcc.cd_tipo_documento_cont in (CNRCTB018.TI_DOC_ACC, CNRCTB018.TI_DOC_ACC_PLUR) then
						ibmutl200.LOGINF(aPg_exec,DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ','Accertamento '||aDescDocCont||': è stato riportato al nuovo esercizio. Importo accertamento origine '||aAcc.im_accertamento||' importo riportato '||aAccNew.im_accertamento,'');
					else  -- ACR_PGIRO
						ibmutl200.LOGINF(aPg_exec,DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ','Annotazione su partita di giro '||aDescDocCont||': è stato riportata al nuovo esercizio. Importo accertamento origine '||aAcc.im_accertamento||' importo riportato '||aAccNew.im_accertamento,'');
					end if;
				else -- spese
					aObbNew := cnrctb048.GETDOCRIPORTATO(aObb);
					if aObb.cd_tipo_documento_cont = CNRCTB018.TI_DOC_OBB_PGIRO then
					   ibmutl200.LOGINF(aPg_exec,DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ','Annotazione su partita di giro '||aDescDocCont||': è stata riportata al nuovo esercizio. Importo '||cnrutil.getLabelObbligazioneMin()||' origine '||aObb.im_obbligazione||' importo riportato '||aObbNew.im_obbligazione,'');
					elsif aObb.cd_tipo_documento_cont = CNRCTB018.TI_DOC_OBB_RES_PRO then
					   ibmutl200.LOGINF(aPg_exec,DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ',cnrutil.getLabelObbligazione()||' '||aDescDocCont||': è stata riportata al nuovo esercizio. Importo '||cnrutil.getLabelObbligazioneMin()||' origine '||aObb.im_obbligazione||' importo riportato '||aObbNew.im_obbligazione,'');
					else -- impegni
					   ibmutl200.LOGINF(aPg_exec,DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ','Impegno '||aDescDocCont||': è stato riportato al nuovo esercizio. Importo '||cnrutil.getLabelObbligazioneMin()||' origine '||aObb.im_obbligazione||' importo riportato '||aObbNew.im_obbligazione,'');
					end if;
				end if;
...
' L''operazione di ribaltamento andrà a buon fine solo se tutti i documenti saranno ribaltati. '||
' In caso contrario verificare le cause del mancato ribaltamento nella funzione: '||
' "Funzionalità di servizio/Logs Applicativi/Gestione" per il Progressivo Esecuzione '||To_Char(pg_exec+1)||
				...
				if aAcc.cd_tipo_documento_cont = CNRCTB018.TI_DOC_ACC_RES then
					...
					begin
					    ...
					    where exists (select 1 from accertamento_scad_voce
							    ...
							    And cd_centro_responsabilita  = CNRCTB015.GETVAL01PERCHIAVE(CNRCTB035.LA_SPECIALE,CNRCTB035.LA_ENTRATA_ENTE)
						            And cd_linea_attivita = CNRCTB015.GETVAL02PERCHIAVE(CNRCTB035.LA_SPECIALE,CNRCTB035.LA_ENTRATA_ENTE));
						ibmutl200.LOGINF_TEMP(aPg_exec,cnrctb047.DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ','Accertamento '||aDescDocCont||': è stato riportato al nuovo esercizio con linea di attività di sistema. Importo accertamento origine '||aAcc.im_accertamento||' importo riportato '||aAccNew.im_accertamento,'', 'N');
					exception when NO_DATA_FOUND then
						ibmutl200.LOGINF_TEMP(aPg_exec,cnrctb047.DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ','Accertamento '||aDescDocCont||': è stato riportato al nuovo esercizio. Importo accertamento origine '||aAcc.im_accertamento||' importo riportato '||aAccNew.im_accertamento,'', 'N');
					end;
				elsif aAcc.cd_tipo_documento_cont in (CNRCTB018.TI_DOC_ACC, CNRCTB018.TI_DOC_ACC_PLUR) then
					ibmutl200.LOGINF_TEMP(aPg_exec,cnrctb047.DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ','Accertamento '||aDescDocCont||': è stato riportato al nuovo esercizio. Importo accertamento origine '||aAcc.im_accertamento||' importo riportato '||aAccNew.im_accertamento,'', 'N');
				else  -- ACR_PGIRO
					ibmutl200.LOGINF_TEMP(aPg_exec,cnrctb047.DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ','Annotazione su partita di giro '||aDescDocCont||': è stato riportata al nuovo esercizio. Importo accertamento origine '||aAcc.im_accertamento||' importo riportato '||aAccNew.im_accertamento,'', 'N');
				end if;
			else -- spese
				aObbNew := cnrctb048.GETDOCRIPORTATO(aObb);
				if aObb.cd_tipo_documento_cont = CNRCTB018.TI_DOC_OBB_PGIRO then
				   ibmutl200.LOGINF_TEMP(aPg_exec,cnrctb047.DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ','Annotazione su partita di giro '||aDescDocCont||': è stato riportata al nuovo esercizio. Importo '||cnrutil.getLabelObbligazioneMin()||' origine '||aObb.im_obbligazione||' importo riportato '||aObbNew.im_obbligazione,'', 'N');
				elsif aObb.cd_tipo_documento_cont = CNRCTB018.TI_DOC_OBB_RES_PRO then
				   ibmutl200.LOGINF_TEMP(aPg_exec,cnrctb047.DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ',cnrutil.getLabelObbligazione()||' '||aDescDocCont||': è stata riportata al nuovo esercizio. Importo '||cnrutil.getLabelObbligazioneMin()||' origine '||aObb.im_obbligazione||' importo riportato '||aObbNew.im_obbligazione,'', 'N');
				else -- impegni
				   ibmutl200.LOGINF_TEMP(aPg_exec,cnrctb047.DS_RIPORTO_NEXT||aEs||' '||to_char(sysdate,'DD/MM/YYYY HH:MI:SS')||' (pg_exec='||aPg_exec||') ','Impegno '||aDescDocCont||': è stato riportato al nuovo esercizio. Importo '||cnrutil.getLabelObbligazioneMin()||' origine '||aObb.im_obbligazione||' importo riportato '||aObbNew.im_obbligazione,'', 'N');
				end if;
			end if;
			commit;
		  exception when OTHERS then
...
if recParametriCNR.esercizio is null Then
  ibmerr001.RAISE_ERR_GENERICO('Ribaltamento disponibilità improprie fallito, manca la configurazione per esercizio '||to_char(aEs+1));
End If;
