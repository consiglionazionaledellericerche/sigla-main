...
ROUND(COMP.IM_LORDO_PERCIPIENTE*RIGA.IM_TOTALE_RIGA_COMPENSO/COMP.IM_TOTALE_COMPENSO,2) IM_LORDO_PERCIPIENTE,
ROUND(COMP.IM_NETTO_PERCIPIENTE*RIGA.IM_TOTALE_RIGA_COMPENSO/COMP.IM_TOTALE_COMPENSO,2) IM_NETTO_PERCIPIENTE,
RIGA.IM_TOTALE_RIGA_COMPENSO IM_TOTALE_COMPENSO
from COMPENSO COMP, COMPENSO_RIGA RIGA, OBBLIGAZIONE OBB, VOCE_IVA V
where 
COMP.CD_CDS = RIGA.CD_CDS AND 
COMP.CD_UNITA_ORGANIZZATIVA = RIGA.CD_UNITA_ORGANIZZATIVA AND 
COMP.ESERCIZIO = RIGA.ESERCIZIO AND
COMP.PG_COMPENSO = RIGA.PG_COMPENSO AND
RIGA.PG_TROVATO IS NOT NULL AND
RIGA.ESERCIZIO_OBBLIGAZIONE = OBB.ESERCIZIO AND
RIGA.ESERCIZIO_ORI_OBBLIGAZIONE = OBB.ESERCIZIO_ORIGINALE AND
RIGA.PG_OBBLIGAZIONE = OBB.PG_OBBLIGAZIONE AND
RIGA.CD_CDS_OBBLIGAZIONE = OBB.CD_CDS AND
