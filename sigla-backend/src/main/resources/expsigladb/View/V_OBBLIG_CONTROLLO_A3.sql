--------------------------------------------------------
--  DDL for View V_OBBLIG_CONTROLLO_A3
--------------------------------------------------------

  CREATE OR REPLACE FORCE VIEW "V_OBBLIG_CONTROLLO_A3" ("ESERCIZIO", "CD_CDS", "IM_STANZIAMENTO_A3", "IM_SALDO_OBBLIG_A3") AS 
  select
--
-- Date: 26/09/2002
-- Version: 1.2
...
-- I parametri sono:
-- 1. Stanziamento presente su articoli ENTE relativi al CDS cd_cds per il terzo anno
...
-- 2. Saldo obbligazioni dell'anno 3
--
-- History:
--
-- Date: 24/01/2002
...
-- Date: 26/09/2002
-- Version: 1.2
-- Fix estraz. obbligazioni
--
-- Body:
--
  esercizio,
  cd_cds,
  sum(im_stanziamento_a3) im_stanziamento_a3,
  sum(im_saldo_obblig_a3) im_saldo_obblig_a3
 from (
  select -- Stanziamento competenza anno 2
   a.esercizio esercizio,
   b.cd_cds cd_cds,
   sum(a.im_stanz_iniziale_a3) im_stanziamento_a3,
   0 im_saldo_obblig_a3
 ...
 union
  select -- Saldo obbligazioni pluriennali
   esercizio,
   cd_cds,
   0 im_stanziamento_a3,
   sum(im_obbligazione) im_saldo_obblig_a3
  from obbligazione where
       esercizio_competenza = esercizio + 2
  group by
   esercizio,
   cd_cds
 union
...
;
   ...
   COMMENT ON TABLE "V_OBBLIG_CONTROLLO_A3"  IS 'Vista di estrazione dei parametri di controllo assunzione obbligazione
I parametri sono:
 1. Stanziamento presente su articoli ENTE relativi al CDS cd_cds per il terzo anno
    Tali articoli spaccano il capitolo per natura ed appartengono a sezioni (funzioni) diverse
    E'' quindi necessario effettuare una aggregazione a livello del capitolo (CDS) considerando solo la parte propria (non AREA)
