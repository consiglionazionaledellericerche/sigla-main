--------------------------------------------------------
--  DDL for View V_OBBLIG_CONTROLLO_RESIDUI
--------------------------------------------------------

  CREATE OR REPLACE FORCE VIEW "V_OBBLIG_CONTROLLO_RESIDUI" ("ESERCIZIO", "CD_CDS", "IM_STANZIAMENTO_A1", "IM_VAR_PIU_A1", "IM_VAR_MENO_A1", "IM_RESIDUO", "IM_RES_VAR_PIU", "IM_RES_VAR_MENO", "IM_CASSA_INIZIALE", "IM_SALDO_MAN_REV") AS 
  select
--
-- Date: 17/06/2005
-- Version: 1.0
--
-- Vista di estrazione derivata dalla V_OBBLIG_CONTROLLO_A1
-- UTILE SOLO nella mappa Ricostruzione dei Residui per il 2005
-- e richiamata indirettamente in RicostruzioneResiduiComponent
--
-- History:
--
-- Body:
--
  ...
  sum(im_saldo_man_rev) im_saldo_man_rev
 from (
 ...
 union
  select -- Saldo obbligazioni di competenza
   ...
   sum(im_mandati_reversali) im_saldo_man_rev
  from voce_f_saldi_cmp a, voce_f b where
       a.ti_competenza_residuo = 'C' -- CNRCTB055.TI_COMPETENZE
...
;
   ...
   COMMENT ON TABLE "V_OBBLIG_CONTROLLO_RESIDUI"  IS 'Vista di estrazione dei parametri di controllo assunzione obbligazione per la ricostruzione dei residui al 2005
I parametri sono:
