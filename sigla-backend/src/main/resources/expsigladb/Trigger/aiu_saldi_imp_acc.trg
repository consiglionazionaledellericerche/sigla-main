CREATE OR REPLACE TRIGGER aiu_saldi_imp_acc
AFTER INSERT OR UPDATE OF ESERCIZIO, TI_APPARTENENZA, TI_GESTIONE, CD_VOCE
 ON SALDI_IMP_ACC REFERENCING NEW AS NEW OLD AS OLD
FOR EACH ROW
declare
 recParametriCNR parametri_cnr%rowtype;
begin
--
-- Trigger attivato su inserimento  e aggiornamento voce nella tabella SALDI_IMP_ACC (After)
--
-- Date: 08/01/2016
-- Version: 1.0
--
