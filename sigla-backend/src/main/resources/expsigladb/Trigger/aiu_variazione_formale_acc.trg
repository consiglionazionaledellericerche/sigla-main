CREATE OR REPLACE TRIGGER aiu_variazione_formale_acc
AFTER INSERT OR UPDATE OF ESERCIZIO, TI_APPARTENENZA, TI_GESTIONE, CD_VOCE
 ON variazione_formale_acc REFERENCING NEW AS NEW OLD AS OLD
FOR EACH ROW
declare
 recParametriCNR parametri_cnr%rowtype;
begin
--
-- Trigger attivato su inserimento  e aggiornamento voce nella tabella variazione_formale_acc (After)
--
-- Date: 15/01/2016
-- Version: 1.0
--
