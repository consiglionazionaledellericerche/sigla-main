CREATE OR REPLACE procedure ins_progetto_sic(p_esercizio number,p_pg_progetto number, p_esercizio_crea number) IS
Begin
...
Select p_esercizio_crea, PG_PROGETTO, TIPO_FASE, p_esercizio_crea, PG_PROGETTO_PADRE, 
 TIPO_FASE_PROGETTO_PADRE, CD_PROGETTO, DS_PROGETTO, CD_TIPO_PROGETTO, 
 Nvl(CD_UNITA_ORGANIZZATIVA,'999.000'), Nvl(CD_RESPONSABILE_TERZO,1), 
 Nvl(DT_INIZIO,To_Date('0101'||p_esercizio_crea,'ddmmyyyy')), DT_FINE, DT_PROROGA, IMPORTO_PROGETTO, 
 IMPORTO_DIVISA, CD_DIVISA, NOTE, STATO, CONDIVISO, DURATA_PROGETTO, 
 LIVELLO, DACR, UTCR, DUVA, UTUV, PG_VER_REC, CD_DIPARTIMENTO, 
 FL_PIANO_TRIENNALE, FL_UTILIZZABILE 
 From PROGETTO_SIP
 Where ESERCIZIO = p_esercizio
   And PG_PROGETTO = p_pg_progetto;   
End;
/
